#!/usr/bin/env python3
"""
Domain Decoder — Pipeline Progress Tracker
Version: 1.0.0

Progress tracking for the 6-phase business rules extraction pipeline.
Tracks rules extracted, bounded contexts, cost, and tokens per phase.

Features:
- Rich terminal progress with live updates (when available)
...
class ProgressTracker:
    """Rich terminal progress tracker for domain decoder pipeline."""

    PANEL_TITLE = "[bold blue]Domain Decoder Pipeline[/bold blue]"
    ...
    def _build_display(self) -> "Panel":
        ...
        progress_text = Text()
        progress_text.append(f"[Phase {self.state.current_phase}/6] ", style="yellow")
        ...
        rules_text = Text()
        rules_text.append(f"{self.state.total_rules_extracted} rules", style="bold green")
        rules_text.append(f" across {len(self.state.bounded_contexts)} contexts", style="dim")
        table.add_row("Extracted:", rules_text)
    ...
    def start(self) -> None:
        if not is_rich_supported():
            print(f"\n{'='*60}")
            print(f"Domain Decoder Pipeline: {self.state.total_items} {self.state.item_noun}")
            print(f"Phases: discovery > characterization > extraction > modeling > expression > validation")
            print(f"{'='*60}\n")
    ...
    def stop(self) -> None:
        if not is_rich_supported():
            ...
            print(f"  Systems: {self.state.completed_items}/{self.state.total_items}")
            print(f"  Rules: {self.state.total_rules_extracted}")
            print(f"  Contexts: {len(self.state.bounded_contexts)}")
    ...
    def start_item(self, item_num: int, item_label: str) -> None:
        ...
        if not is_rich_supported():
            print(f"\n[{item_num}/{self.state.total_items}] Decoding: {item_label}")
        self._refresh()
    ...
    def complete_phase(
        ...
        if not is_rich_supported():
            ...
            extra = f", +{rules_extracted} rules" if rules_extracted else ""
            print(f"    [{status}] +{tokens_in + tokens_out:,} tokens{extra}")
        self._refresh()
    ...
    def complete_item(self, success: bool = True) -> None:
        ...
        if not is_rich_supported():
            ...
            print(f"  [{status}] Total rules: {self.state.total_rules_extracted}")
        self._refresh()

    def add_contexts(self, contexts: List[str]) -> None:
        for ctx in contexts:
...
class SimpleProgress:
    ...
    def __init__(self, total_items: int, total_phases: int = 6, item_noun: str = "systems", **kwargs):
    ...
    def stop(self) -> None:
        elapsed = self.state.elapsed_seconds
        print(f"\n{'='*60}")
        print(f"Complete in {elapsed:.1f}s | Rules: {self.state.total_rules_extracted} | Cost: ${self.state.total_cost_usd:.4f}")
        print(f"{'='*60}\n")
    ...
    def complete_phase(self, tokens_in: int = 0, tokens_out: int = 0, cost_usd: float = 0.0, rules_extracted: int = 0, success: bool = True) -> None:
        self.state.completed_phases += 1
        self.state.total_tokens_in += tokens_in
        self.state.total_tokens_out += tokens_out
        self.state.total_cost_usd += cost_usd
...
def create_progress_tracker(
    ...
    """Create the appropriate progress tracker based on environment."""
...
if __name__ == "__main__":
    ...
    tracker = create_progress_tracker(total_items=1, item_noun="systems")
    tracker.start()

    try:
        tracker.start_item(1, "test-system")
        phases = ["discovery", "characterization", "extraction", "modeling", "expression", "validation"]
        for i, name in enumerate(phases):
            tracker.start_phase(i, name)
            time.sleep(0.3)
            rules = 15 if i == 2 else 0
            tracker.complete_phase(tokens_in=2000, tokens_out=1000, cost_usd=0.02, rules_extracted=rules)
