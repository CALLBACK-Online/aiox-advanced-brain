...
framework_task_metadata:
  task_id: define-north-star
  task_name: Define North Star Metric
  status: active
  ...
  atomic_layer: Atom
  inputs:
    - product_understanding document
    - user_behavior_data
    - business_model_clarity (monetization + value delivery)
    - stakeholder availability for workshop
  outputs:
    - outputs/data/{slug}/north-star-definition.yaml
    - outputs/data/{slug}/input-metrics-tree.yaml
    - outputs/data/{slug}/nsm-validation-results.yaml
    - outputs/data/{slug}/team-alignment-doc.md
  pre_conditions:
    - product has reached measurable user activity
    - leadership agrees to operationalize a single metric
  post_conditions:
    - NSM defined with clear formula and input tree
    - validation against long-term value produced
  acceptance_criteria:
    - NSM captures core customer value (not revenue proxy)
    - input metrics tree traces cause -> NSM
    - NSM validated against CLV/retention correlation
    - dashboard spec and team alignment doc produced
  ...
  output_path: "outputs/data/{slug}/north-star-definition.yaml"
```
