...
sinkra_task_metadata:
  ...
  status: active
  responsible_executor: '@avinash-kaushik'
  ...
  inputs:
    - business_objectives and KPIs
    - outputs/data/{slug}/dmmm.yaml
    - stakeholder_personas (who consumes the dashboard)
    - available data sources and freshness SLAs
  outputs:
    - outputs/data/{slug}/dashboard-spec.yaml
    - outputs/data/{slug}/executive-report.yaml
    - outputs/data/{slug}/metric-hierarchy.yaml
  pre_conditions:
    - business objectives documented
    - data sources identified and accessible
  post_conditions:
    - dashboard specification ready for implementation
    - each metric passes So What Test with recommended action
  acceptance_criteria:
    - metrics organized by acquisition | behavior | outcomes
    - DMMM model applied to select KPIs
    - every metric has owner, threshold, and recommended action
    - no vanity metrics; every metric must change a decision
