...
framework_task_metadata:
  ...
  task_name: Measure Community
  status: active
  responsible_executor: '@david-spinks'
  ...
  atomic_layer: Atom
  inputs:
    - community platform data (posts, replies, members)
    - member onboarding and churn records
    - community investment and program catalog
    - benchmark data for similar communities (when available)
  outputs:
    - outputs/data/{slug}/community-health-score.yaml
    - outputs/data/{slug}/spaces-value-assessment.yaml
    - outputs/data/{slug}/member-value-calculation.yaml
    - outputs/data/{slug}/community-engagement-dashboard.yaml
  pre_conditions:
    - community platform accessible with export capability
    - business outcomes linked to community identified
  post_conditions:
    - SPACES dimensions scored with evidence
    - community ROI and member value quantified
  acceptance_criteria:
    - SPACES (Support, Product, Acquisition, Content, Engagement, Success) assessed
    - Three-Level Strategy Framework applied
    - health indicators computed vs. benchmarks
  ...
  output_path: "outputs/data/{slug}/community-health-report.yaml"
```
