...
framework_task_metadata:
  task_id: calculate-clv
  task_name: Calculate Customer Lifetime Value
  status: active
  responsible_executor: '@peter-fader'
  ...
  atomic_layer: Atom
  inputs:
    - outputs/data/{slug}/raw_transactions.csv
    - customer_ids with timestamps and monetary values
    - minimum 6 months of transactional history
    - cost_of_acquisition (CAC) if available
  outputs:
    - outputs/data/{slug}/clv-report.yaml
    - outputs/data/{slug}/clv-per-customer.csv
    - outputs/data/{slug}/pareto-distribution.yaml
  pre_conditions:
    - unique customer identifiers present
    - transactions span at least 6 months
  post_conditions:
    - CLV computed at individual and segment level
    - value distribution quantified (top decile share)
  acceptance_criteria:
    - at least one method applied (historical | deterministic | BG/NBD + Gamma-Gamma)
    - method selection justified by data maturity
    - CLV > CAC check produced
    - future revenue projection and retention vs acquisition recommendation generated
  performance_sla: "completes within estimated_time, logs all errors"
  error_handling: "on failure, log to outputs/data/{slug}/errors.log and escalate to data-chief"
  output_path: "outputs/data/{slug}/clv-report.yaml"
```
...
aiox_contract:
  ...
  atomic_layer: Atom
  executor: Clone
  pre_condition: "inputs and dependencies resolved"
...
- Output canônico (template-first): `docs/analytics/clv-report.yaml`
- Output custom (ad-hoc): `docs/data/{slug}/clv-investigation-{date}.md`
- Regra: escrita canônica só se existir template em `docs/templates/analytics/clv-report.yaml`
