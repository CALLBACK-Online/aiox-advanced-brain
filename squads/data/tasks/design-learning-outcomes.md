...
sinkra_task_metadata:
  ...
  task_name: Design Learning Outcomes
  status: active
  responsible_executor: '@wes-kao'
  ...
  atomic_layer: Atom
  inputs:
    - course_or_program_definition
    - target_learner_profile (ICP for learners)
    - outputs/data/{slug}/cohort-analysis.yaml
    - outputs/data/{slug}/community-health-report.yaml
  outputs:
    - outputs/data/{slug}/learning-outcomes-framework.yaml
    - outputs/data/{slug}/completion-rate-optimization-plan.yaml
    - outputs/data/{slug}/engagement-score-design.yaml
    - outputs/data/{slug}/outcome-dashboard-spec.yaml
  pre_conditions:
    - course mechanics defined (cohort-based or self-paced)
    - measurable transformation goal established
  post_conditions:
    - completion rate methodology documented beyond binary
    - engagement score and learning progression tracking specified
  acceptance_criteria:
    - Course Mechanics Canvas (12 levers) applied
    - State Change Method used to design engagement touchpoints
    - NPS and skill-assessment outcome measurement defined
    - cohort-based benchmarks established with drop-off diagnostics
  ...
  output_path: "outputs/data/{slug}/learning-outcomes-framework.yaml"
```
