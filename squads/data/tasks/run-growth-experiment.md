...
sinkra_task_metadata:
  ...
  inputs:
    ...
    - baseline_metric and target_lift
    - traffic_or_user_volume estimate
    - outputs/data/{slug}/north-star-definition.yaml
  outputs:
    - outputs/data/{slug}/experiment-spec.yaml
    - outputs/data/{slug}/experiment-results.yaml
    - outputs/data/{slug}/experiment-learnings.yaml
    - outputs/data/{slug}/ice-backlog.yaml
  pre_conditions:
    - hypothesis prioritized via ICE
    - sample size calculation completed with required power
  post_conditions:
    - experiment executed and statistical significance computed
    - learnings documented regardless of win/loss
  acceptance_criteria:
    - ICE score documented for prioritization
    - sample size and runtime calculated a priori
    - statistical significance and effect size reported
    - learnings written and stored in experiment library
