...
sinkra_task_metadata:
  ...
  task_name: Analyze Cohort
  status: active
  responsible_executor: '@peter-fader'
  execution_type: Clone
  estimated_time: 6-10h
  domain: Tactical
  responsavel_type: Clone
  atomic_layer: Molecule
  inputs:
    - outputs/data/{slug}/raw_transactions.csv
    - cohort_definition_dimensions (time|channel|product|behavior)
    - outputs/data/{slug}/clv-report.yaml
    - outputs/data/{slug}/rfm-segmentation.yaml
  outputs:
    - outputs/data/{slug}/cohort-definition-matrix.yaml
    - outputs/data/{slug}/retention-curves.yaml
    - outputs/data/{slug}/cohort-comparison-report.yaml
    - outputs/data/{slug}/cohort-action-plan.yaml
  pre_conditions:
    - transactional history available (min 3 months, 6+ preferred)
    - customer-level identifiers and timestamps present
  post_conditions:
    - retention curves constructed per cohort
    - comparative insights extracted and prioritized
  acceptance_criteria:
    - cohorts defined across at least 2 dimensions
    - retention curve computed per cohort with statistical baseline
    - actionable insights tied to acquisition/activation/retention levers
    - cohort projections modeled (sBG or equivalent)
  ...
  output_path: "outputs/data/{slug}/cohort-analysis.yaml"
```
