...
sinkra_task_metadata:
  ...
  task_name: Load Workspace Context
  status: active
  responsible_executor: '@data-chief'
  ...
  outputs:
    - outputs/data/{slug}/data-workspace-context.yaml
    - canonical_outputs_allowed[]
    - custom_outputs_allowed[]
  pre_conditions:
    - bootstrap-data-workspace.sh passes
    - validate-data-essentials.sh passes
  post_conditions:
    - workspace context summary persisted
    - output routes (canonical|custom) decided
  acceptance_criteria:
    - preflight scripts return pass
    - business and domain context files loaded
    - canonical template catalog enumerated
    - routing decision documented before analytical execution
  performance_sla: "completes within estimated_time, logs all errors"
  error_handling: "on preflight failure, block downstream tasks and escalate to data-chief"
  output_path: "outputs/data/{slug}/data-workspace-context.yaml"
```
