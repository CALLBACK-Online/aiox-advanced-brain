...
sinkra_task_metadata:
  task_id: segment-rfm
  task_name: RFM Customer Segmentation
  status: active
  ...
  inputs:
    ...
    - monetary values per transaction
  outputs:
    - outputs/data/{slug}/rfm-scores.csv
    - outputs/data/{slug}/rfm-segmentation.yaml
    - outputs/data/{slug}/rfm-segment-playbooks.yaml
  pre_conditions:
    - minimum 3 months of transactional data (6+ preferred)
    - customer identifiers deduplicated
  post_conditions:
    - each customer assigned an R, F, M score
    - 11 behavioral segments populated with action playbooks
  acceptance_criteria:
    - scoring methodology selected (quintile | percentile | absolute)
    - 11 segments (Champions, Loyal, At Risk, Can't Lose, etc.) defined
    - playbook per segment with retention/growth actions
    - base composition summary produced (% per segment)
  ...
  output_path: "outputs/data/{slug}/rfm-segmentation.yaml"
```
