...
sinkra_task_metadata:
  ...
  status: active
  responsible_executor: '@nick-mehta'
  ...
  outputs:
    - outputs/data/{slug}/health-score-report.yaml
    - outputs/data/{slug}/health-score-components.yaml
    - outputs/data/{slug}/alert-triggers.yaml
    - outputs/data/{slug}/health-score-implementation-guide.md
  pre_conditions:
    - stakeholder alignment on intent (intervention vs reporting)
    - data sources accessible and reliable
  post_conditions:
    - composite health score defined with weights and thresholds
    - alerting rules established for proactive CS intervention
  acceptance_criteria:
    - DEAR framework applied (Deployment, Engagement, Adoption, ROI)
    - weights calibrated via correlation with churn/CLV outcomes
    - thresholds defined per score band (green/yellow/red)
    - integration path with CLV for prioritization documented
