...
framework_task_metadata:
  ...
  inputs:
    ...
    - channels_list
    - outputs/data/{slug}/historical_marketing_data.csv
    - budget_by_channel
  outputs:
    - outputs/data/{slug}/attribution-report.yaml
    - outputs/data/{slug}/dmmm.yaml
    - outputs/data/{slug}/channel-mapping.yaml
  pre_conditions:
    - minimum 3 months of channel/conversion history
    - conversion event and window defined
    - executive buy-in to act on insights
  post_conditions:
    - attribution model(s) selected and documented
    - budget reallocation recommendation produced
  acceptance_criteria:
    - tier 0 diagnostic completed
    - at least 2 attribution models compared (first/last/linear/time-decay/position-based/data-driven)
    - conversion window justified with data
    - recommendations tied to budget decisions, not just reporting
