...
sinkra_task_metadata:
  task_id: run-pmf-test
  task_name: Run PMF Test
  status: active
  responsible_executor: '@sean-ellis'
  ...
  atomic_layer: Atom
  inputs:
    - active_user_base with recent usage
    - email_access_to_users
    - user_segmentation attributes
    - minimum_sample_size (target ~100 responses)
  outputs:
    - outputs/data/{slug}/pmf-survey-template.yaml
    - outputs/data/{slug}/pmf-survey-results.yaml
    - outputs/data/{slug}/pmf-segment-breakdown.yaml
    - outputs/data/{slug}/pmf-improvement-roadmap.yaml
  pre_conditions:
    - product has active users with recent engagement
    - survey channel available (email or in-app)
  post_conditions:
    - "% very disappointed computed overall and per segment"
    - recommendations generated based on 40% threshold
  acceptance_criteria:
    - Sean Ellis "very disappointed" question used verbatim
    - minimum sample size achieved for statistical confidence
    - segment-level PMF analyzed to find high-expectation customers
    - action recommendation (iterate vs. scale) justified by data
  ...
  output_path: "outputs/data/{slug}/pmf-survey-results.yaml"
```
