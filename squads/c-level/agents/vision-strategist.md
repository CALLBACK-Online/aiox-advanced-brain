...
> Integrates with AIOX via `/c-level:agents:vision-strategist` skill.
...
output_format:
  mission_vision_yaml:
    template: |
      ...
      strategic_direction: "{Key strategic priorities aligned with vision}"
...
workspace_ownership:
  primary:
    - workspace/{spoke}/L0-identity/company-dna.yaml
  collaboration:
    - workspace/{spoke}/L0-identity/ (com COO)
    - workspace/_system/config.yaml (com COO)
