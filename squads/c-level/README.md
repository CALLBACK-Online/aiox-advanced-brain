...
O Squad C-Level nasceu da necessidade de ter um "dono" responsavel por estruturar toda a informacao da empresa dentro do workspace. Nao e um unico profissional — e um squad composto por executivos C-Level, porque o trabalho cruza visao estrategica, operacao, tecnologia e inteligencia artificial simultaneamente.

**Preserva o modelo canônico `workspace/{spoke}` do SINKRA. No hub atual, cada `spoke` é materializado em `workspace/businesses/{slug}` com 5 layers (L0-L4).**
...
| **CSO** (cso) | Sinkra Governor | 1 | Governanca SINKRA, compliance, gap-zero, cascading |
...
Ambas compartilham o mesmo `workspace/{spoke}/`.
No Sinkra Hub atual, isso é resolvido fisicamente para `workspace/businesses/{slug}/`.

---

## Estrutura do Workspace (5-Layer Model)

```
workspace/
└── businesses/
    └── {spoke}/                         # spoke namespace no hub
        ├── document-registry.yaml       # Governance registry
        │
        ├── L0-identity/                 # TTL: 365d — CEO + COO
        │   ├── company-dna.yaml         # Layer 2 (COO/CEO)
        │   ├── founder-dna.yaml         # Layer 2 (CEO)
        │   ├── credentials.yaml         # Layer 2 (CEO)
        │   └── core-processes.yaml      # Layer 2 (COO)
        │
        ├── L1-strategy/                 # TTL: 90d — CMO + CTO + CAIO
        │   ├── icp.yaml                 # Layer 2 (CMO)
        │   ├── diagnosis.yaml           # Layer 2 (CMO)
        │   ├── pricing-strategy.yaml    # Layer 2 (CMO)
        │   ├── bmc.yaml                 # Layer 2 (CMO)
        │   ├── lean-canvas.yaml         # Layer 2 (CMO)
        │   ├── offerbook.yaml           # Layer 2 (CMO)
        │   ├── tech-strategy.yaml       # Layer 2 (CTO)
        │   ├── tech-stack.yaml          # Layer 2 (CIO)
        │   └── ai-strategy.yaml         # Layer 2 (CAIO)
        │
        ├── L2-tactical/                 # TTL: 60d — CMO + Brand Squad
        │   └── brand/
        │       └── brandbook.yaml       # Layer 2 (CMO → brand-squad)
        │
...
Binding físico no hub:
- `workspace/businesses/{slug}/...`

Arquivos de configuração do sistema:
- `workspace/_system/config.yaml` — Configuração global do hub
- `workspace/_templates/business-template/` — Templates canônicos
- `workspace/scripts/scaffold-workspace.js` — Scaffold determinístico de novos businesses
...
FASE 1: Formulário Básico (~15 min)
  ...
  → Output: company-dna.yaml (parcial)
...
FASE 3: Empresa + Time (~30 min)
  ...
  → Output: company-dna.yaml (completo) + team-structure.yaml
...
*elicit-company-profile meu_negocio    # → company-dna.yaml
...
Gera arquivos `.md` narrativos em `L1-strategy/tech/`, `L1-strategy/ai/` e `L0-identity/`. Coexiste com Layer 2.
...
1. `workspace/{spoke}/document-registry.yaml`
2. `workspace/_system/config.yaml`
3. `squads/c-level/config.yaml`
4. `squads/c-level/workflows/`
5. `workspace/_templates/business-template/*`
6. `workspace/{spoke}/L0-identity/*`, `L1-strategy/*`, `L2-tactical/*`, `L3-product/*` (quando existir)
...
|---------|-------|--------|-----------|
| `*elicit-company-profile {slug}` | COO | company-dna.yaml | ~30 |
