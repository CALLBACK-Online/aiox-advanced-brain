...
#
# Usage:
#   ./squads/c-level/scripts/validate-product.sh businesses/academia_lendaria/L3-product/comunidade
#   ./squads/c-level/scripts/validate-product.sh businesses/your_business/L3-product/your_product
#
# Output:
#   ✅ overview.md exists
...
if [ "$TOTAL_ISSUES" -eq 0 ] && [ -f "$ICP_FILE" ] && [ -f "$BRAND_FILE" ] && [ -f "$OVERVIEW_FILE" ] && [ -f "$OFFERBOOK_FILE" ] && [ -f "$ANALYTICS_FILE" ]; then
    ...
    echo "     ./squads/c-level/scripts/validate-product.sh $PRODUCT_PATH"
    echo ""
    exit 1
fi
