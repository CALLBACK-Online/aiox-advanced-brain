...
task:
  ...
  elicit: true
  output_format: yaml
  target_template: L0-identity/founder-dna.yaml
```
## SINKRA Contract
...
- Recomendado: `company-dna.yaml` já preenchido (para contexto)
...
1. Ler `workspace/{spoke}/L0-identity/founder-dna.yaml`:
...
2. Ler `workspace/{spoke}/L0-identity/company-dna.yaml` (se existir) para contexto sobre a empresa.
...
4. **Calcular completude:**
   ...
   metadata:
     ...
     last_updated: "{date}"
   ```
5. **Salvar** em `workspace/{spoke}/L0-identity/founder-dna.yaml`.
6. **Relatório:**
   ```
...
- [ ] Arquivo salvo em `workspace/{spoke}/L0-identity/founder-dna.yaml`
