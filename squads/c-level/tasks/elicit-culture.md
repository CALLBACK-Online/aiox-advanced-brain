...
- Recomendado: `L0-identity/company-dna.yaml` já preenchido (dados de missão/visão são input)
...
2. Ler fontes complementares (se existirem):
   - `L0-identity/company-dna.yaml` → pré-popular mission/vision/values
   - `L0-identity/founder-dna.yaml` → pré-popular história do founder
   - `L2-tactical/brand/brandbook.yaml` → pré-popular posicionamento e voz
...
elicitation:
  ...
  pre_populate_from:
    - L0-identity/company-dna.yaml → mission, vision, positioning
  questions:
    - id: mission
      text: >
...
elicitation:
  ...
  pre_populate_from:
    - L0-identity/company-dna.yaml → values
  questions:
    - id: values_list
      text: >
