...
1. Verificar se `.user/user.md` existe (bootstrap completo).
2. Verificar se `workspace/{spoke}/L0-identity/company-dna.yaml` já existe:
...
3. Ler `squads/c-level/config.yaml` (se existir) para alinhar com entidades de Decision/ActionItem do domínio.
...
Criar `workspace/{spoke}/L0-identity/company-dna.yaml` (seção mission-vision):
