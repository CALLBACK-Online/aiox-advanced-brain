...
task:
  ...
  commands:
  - '*etl-batch-run'
  - '*etl-batch-run --slugs {list}'
  - '*etl-batch-run --resume'
...
Output:
- workspace/businesses/_batch/etl-deep-pass-batch-status.yaml
pre_condition:
...
3. Para cada business, ler `L4-operational/evidence/completeness-manifest.yaml` se existir.
