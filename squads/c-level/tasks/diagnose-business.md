...
- Pelo menos `L0-identity/company-dna.yaml` presente
...
weight: 0.14
files:
  - path: "L1-strategy/diagnosis.yaml"
    role: primary
  - path: "L1-strategy/icp.yaml"
    role: primary
  - path: "L1-strategy/analytics.yaml"
    role: secondary
```
...
weight: 0.14
files:
  - path: "L3-product/*/offerbook.yaml"
    role: primary
  - path: "L3-product/*/proof.yaml"
    role: primary
  - path: "L3-product/*/testimonials.yaml"
    role: primary
```
...
weight: 0.09
files:
  - path: "L3-product/*/narrative/brandscript.yaml"
    role: primary
  - path: "L3-product/*/narrative/product-story.yaml"
    role: primary
  - path: "L3-product/*/narrative/pitch-narrative.yaml"
    role: secondary
  - path: "L3-product/*/narrative/objection-destroyers.yaml"
    role: secondary
  - path: "L0-identity/authority-story.yaml"
    role: secondary
  - path: "L0-identity/founder-dna.yaml"
    role: primary
```
...
weight: 0.07
files:
  - path: "L3-product/*/marketing/buyer-journey.yaml"
    role: primary
  - path: "L3-product/*/marketing/conversion-funnel.yaml"
    role: primary
  - path: "L3-product/*/marketing/email-sequences.yaml"
    role: secondary
  - path: "L3-product/*/marketing/campaign-brief.yaml"
    role: secondary
```
...
weight: 0.07
files:
  - path: "L3-product/*/curriculum.yaml"
    role: primary
  - path: "L3-product/*/onboarding/onboarding-flow.yaml"
    role: secondary
  - path: "L3-product/*/retention/churn-prevention.yaml"
    role: secondary
  - path: "L3-product/*/retention/nps-feedback-loop.yaml"
    role: secondary
```
...
files:
  ...
  - path: "L3-product/*/proof.yaml"
    role: primary
  - path: "L0-identity/credentials.yaml"
    role: primary
  - path: "L0-identity/authority-story.yaml"
    role: secondary
```
...
|----------------|---------------|---------|----------|
| Customer | c-level (CMO) | `*elicit-icp-yaml {slug}` | deep-research |
| Brand | brand | `/brand` | c-level `*elicit-brand-yaml` |
| Offer | hormozi | `/hormozi` | copy `/copy-workflow` |
| Narrative | storytelling | `/storytelling` | copy |
| Traffic | traffic-masters | `/traffic-masters` | c-level (CMO) |
| Operations | c-level (COO) | `*elicit-operations {slug}` | aiox-sop `*create-sop-operations-suite {slug}` |
| Success | course-creator | squad course-creator | c-level (COO) |
| Evidence | deep-research | `/deep-research` | spy `/spy` |
| Movement | movement | `/movement` | brand |
| Culture | c-level (COO) | `*elicit-culture {slug}` | c-level (Vision Chief) |
...
*Diagnóstico gerado por COO (workspace-chief) - Squad AIOX Workspace*
*Data: {YYYY-MM-DD}*
```
...
```yaml
backlog_gate:
  path: "workspace/businesses/{slug}/L1-strategy/diagnostic-backlog.yaml"
  template: "workspace/_templates/business-template/L1-strategy/diagnostic-backlog.yaml"
  steps:
    ...
    - present_to_user: |
        ...
        |---|----------|-------|-----------|-------|---------|
        | 1 | Customer | 45 | CRÍTICO | c-level (CMO) | *elicit-icp-yaml {slug} |
