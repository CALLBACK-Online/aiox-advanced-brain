...
task:
  ...
  elicit: true
  output_format: yaml
  target_template: L1-strategy/icp.yaml
```
## SINKRA Contract
...
- Recomendado: `company-dna.yaml` preenchido (para contexto de mercado)
...
1. Ler `workspace/{spoke}/L1-strategy/icp.yaml`:
...
2. Ler `workspace/{spoke}/L1-strategy/icp.yaml` (Sistema A, se existir): pré-popular campos correspondentes.
3. **Diagnosis Gate:** Ler `workspace/{spoke}/L1-strategy/diagnosis.yaml`:
...
3. **Calcular completude:**
   ...
   ```
4. **Salvar** em `workspace/{spoke}/L1-strategy/icp.yaml`.
...
- [ ] Arquivo salvo em `workspace/{spoke}/L1-strategy/icp.yaml`
