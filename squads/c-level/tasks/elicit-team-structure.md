...
task:
  ...
  elicit: true
  output_format: yaml
  target_template: L1-strategy/team-structure.yaml
```
## SINKRA Contract
...
O COO conduz elicitação para documentar a estrutura atual do time, a estrutura ideal e o plano de contratação. O output popula `team-structure.yaml` em `L1-strategy/`, onde o contrato canônico L0-L4 concentra governança organizacional.

## Prerequisites

- Bootstrap executado
- Templates scaffolded
- Recomendado: `company-dna.yaml` preenchido (para contexto de stage e team size)
...
1. Ler `workspace/{spoke}/L1-strategy/team-structure.yaml`:
...
2. Ler `workspace/{spoke}/L0-identity/company-dna.yaml` (se existir) para team_size e stage.
...
3. **Calcular completude** e salvar em `workspace/{spoke}/L1-strategy/team-structure.yaml`.
...
- [ ] YAML válido salvo em L1-strategy/team-structure.yaml
