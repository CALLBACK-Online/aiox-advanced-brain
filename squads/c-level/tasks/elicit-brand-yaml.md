...
task:
  ...
  elicit: true
  output_format: yaml
  target_template: L2-tactical/brand/brandbook.yaml
```
## SINKRA Contract
...
O CMO conduz elicitação estruturada para popular `brandbook.yaml` com guidelines completos da marca: identidade, essência, posicionamento, arquétipos e voice DNA.

## Prerequisites

- Bootstrap executado
- Negócio criado e templates scaffolded
- Recomendado: `founder-dna.yaml` e `company-dna.yaml` preenchidos
...
1. Ler `workspace/{spoke}/L2-tactical/brand/brandbook.yaml`:
...
2. Ler `workspace/{spoke}/L2-tactical/brand/brandbook.yaml` (Sistema A, se existir): pré-popular campos correspondentes.
3. Ler `workspace/{spoke}/L0-identity/founder-dna.yaml` (se existir): usar personalidade do fundador como base para personalidade da marca.
...
3. **Calcular completude:**
   ...
   ```
4. **Salvar** em `workspace/{spoke}/L2-tactical/brand/brandbook.yaml`.
...
- [ ] Arquivo salvo em `workspace/{spoke}/L2-tactical/brand/brandbook.yaml`
