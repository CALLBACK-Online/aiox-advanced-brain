...
task_id: classify-squad-workspace-fit
```yaml
task:
  task_id: classify-squad-workspace-fit
...
Output:
- squads/c-level/outputs/squad-workspace-fit.yaml
pre_condition:
...
- `workspace/businesses/{slug}/L3-product/*`
- `workspace/businesses/{slug}/L4-operational/*`
...
- brand system da BU
- strategy artifacts da BU
- movement canonizado por business
- assets operacionais específicos do business
...
- ou quando múltiplos squads consumidores reutilizam o mesmo artefato L3/L4
...
- identity-wide -> `workspace/businesses/{business}/L0-identity/`
- strategy-wide -> `workspace/businesses/{business}/L1-strategy/`
- product-specific -> `workspace/businesses/{business}/L3-product/{product}/`
- operational-specific -> `workspace/businesses/{business}/L4-operational/`
...
shared_artifacts:
  - artifact: "{name}"
    ...
    save_scope: identity|strategy|product|operational
...
### Casos legados que exigem remapeamento antes de rollout

1. `franchise`
   - manter como `defer` até existir namespace L0-L4 explícito para o domínio

2. `traffic-masters`
   - tratar como `controlled_runtime_consumer`, consumindo artefatos de `L3-product/` e `L4-operational/`
