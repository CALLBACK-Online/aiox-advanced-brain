...
task_id: elicit-pricing-strategy
```yaml
task:
  task_id: elicit-pricing-strategy
  ...
  elicit: true
  output_format: yaml
  target_template: L1-strategy/pricing-strategy.yaml
```
## SINKRA Contract
...
O CMO conduz elicitação para documentar a estratégia de preços: fundação, psicologia de preço, posição competitiva e experimentos. O output popula `pricing-strategy.yaml` em `L1-strategy/`, que é o contrato canônico para estratégia de negócio.

## Prerequisites

- Bootstrap executado
- Templates scaffolded
- Recomendado: `company-dna.yaml` e `icp.yaml` preenchidos
...
1. Ler `workspace/{spoke}/L1-strategy/pricing-strategy.yaml`:
...
2. Ler `workspace/{spoke}/L0-identity/company-dna.yaml` (se existir) para contexto de produtos e receita.
3. Ler `workspace/{spoke}/L1-strategy/icp.yaml` (se existir) para contexto de poder aquisitivo do ICP.
...
elicitation:
  ...
  questions:
    - id: lowest_competitor
      ...
      required: true
      maps_to: competitive_positioning.market_price_range.lowest_competitor

    - id: average_price
      text: "Qual o preço médio do mercado?"
      required: true
      maps_to: competitive_positioning.market_price_range.average_market_price

    - id: premium_competitor
      text: "Qual o concorrente mais caro e seu preço?"
      required: true
      maps_to: competitive_positioning.market_price_range.premium_competitor

    - id: your_position
      text: "Onde você se posiciona? (Budget / Mid-market / Premium / Ultra-premium). Por quê?"
      required: true
      maps_to: competitive_positioning.your_position
```
...
3. **Calcular completude** e salvar em `workspace/{spoke}/L1-strategy/pricing-strategy.yaml`.
...
- [ ] YAML válido salvo em L1-strategy/pricing-strategy.yaml
