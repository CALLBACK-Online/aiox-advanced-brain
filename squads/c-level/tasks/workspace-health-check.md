...
task_id: workspace-health-check
```yaml
task:
  task_id: workspace-health-check
...
Output:
- docs/qa/workspace-health/YYYY-MM-DD-health-report.md
pre_condition:
...
- [ ] `workspace/` existe
- [ ] `L0-identity/` existe com arquivos obrigatórios
- [ ] `L1-strategy/tech/` existe quando o spoke exige contexto técnico
- [ ] `L1-strategy/ai/` existe quando o spoke exige contexto de IA
- [ ] `L3-product/` existe (pode estar vazio)
- [ ] `workspace/_system/config.yaml` existe
- [ ] `STRUCTURE.md` existe
- [ ] Nenhum pattern proibido encontrado
- [ ] Produtos em `L3-product/` seguem snake_case
...
**Gate:** Se `exit code != 0`, marcar `REVIEW` e bloquear PASS.
...
2. **Gate Decision:** PASS | REVIEW | FAIL
...
- [ ] Gate decision definido (PASS/REVIEW/FAIL)
