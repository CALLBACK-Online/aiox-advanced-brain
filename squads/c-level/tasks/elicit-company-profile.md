...
task_id: elicit-company-profile
```yaml
task:
  task_id: elicit-company-profile
  ...
  elicit: true
  output_format: yaml
  target_template: L0-identity/company-dna.yaml
```
## SINKRA Contract
...
O COO conduz elicitação estruturada para popular `company-dna.yaml` com dados completos da empresa. Este arquivo é o perfil institucional canônico: quem é a empresa, missão, visão, valores, portfólio e métricas.
...
1. Ler `workspace/{spoke}/L0-identity/company-dna.yaml`:
...
2. Ler evidências já consolidadas em `workspace/{spoke}/L0-identity/` e `workspace/{spoke}/L1-strategy/`:
   - **Se existem:** Pré-popular missão, visão, valores e métricas a partir dos artefatos canônicos.
   - Informar ao usuário quais blocos já vieram preenchidos do contexto existente.
...
3. **Calcular completude:**
   ...
   ```
4. **Salvar** em `workspace/{spoke}/L0-identity/company-dna.yaml`.
5. **Relatório:**
   ```
...
- [ ] Arquivo salvo em `workspace/{spoke}/L0-identity/company-dna.yaml`
