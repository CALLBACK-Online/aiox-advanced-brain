...
framework_task_metadata:
  ...
  domain: Operational
  input:
  - 'expert_name do expert cujas heurísticas serão extraídas'
  - 'sources classificadas (ouro/bronze) via *assess-sources ou lista manual'
  output:
  - 'heuristics_yaml com heurísticas formatadas (SE/ENTÃO/NUNCA)'
  - 'decision_cards entries prontas para decision-cards.yaml'
  - 'heuristic_files arquivos .md individuais no formato L3'
  action_items:
  - Validar fontes no Source Gate (mínimo 60% OURO)
  - Varrer fontes com 6 lentes de extração (CDM)
  - Filtrar via Pareto ao Cubo (descartar genéricas)
  - Triangular e validar confiança
  - Formalizar em L2 (YAML) e L3 (.md)
  - Emitir extraction_report e source_map
  acceptance_criteria:
  - Todas heurísticas têm [SOURCE:] rastreável
  - Zero heurísticas genéricas passaram filtro
  - Quality check score >= 6/7
...
aiox_contract:
  ...
  atomic_layer: Atom
  executor: Agent
  pre_condition: "fontes classificadas (ouro/bronze) disponíveis via *assess-sources, expert_name definido"
