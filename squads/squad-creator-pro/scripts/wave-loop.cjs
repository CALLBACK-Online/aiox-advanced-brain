...
// literal "socket connection was closed unexpectedly" phrase that the old
// local regex did not match. See .aiox-core/core/utils/transient-llm-error.js
// and the broad-socket-closed-review workflow (2026-05-29).
const {
  isTransientLlmErrorText,
  isFatalLlmErrorText,
} = require(path.resolve(__dirname, '..', '..', '..', '.aiox-core', 'core', 'utils', 'transient-llm-error.js'));
...
function isTransientRuntimeError(output) {
  // Delegate to the shared source of truth (covers socket-closed, stream-idle,
  // 5xx/429, rate-limit, ECONNRESET, etc.), plus a couple of Codex-runtime
  // specific markers not in the shared list.
  return (
    isTransientLlmErrorText(output) ||
    /connection reset|network error|temporarily unavailable|failed to queue rollout items|failed to shutdown rollout recorder/i.test(
      output || ''
    )
  );
}
...
function isFatalRuntimeError(output) {
  return isFatalLlmErrorText(output);
}
  ...
  try {
    ...
    for (const wave of plan.waves) {
      ...
      for (let attempt = 1; attempt <= runtimeSettings.maxAttempts; attempt += 1) {
        ...
        // Retry on transient errors. Note: a socket-closed drop can surface
        // with exitCode === 0 while the wave did NOT complete in runtime state
        // (the CLI exits 0 but reports the API error in-band). So we retry when
        // the wave is incomplete AND the output looks transient, regardless of
        // exit code — not only when exitCode !== 0. (broad-socket-closed-review)
        const shouldRetry = attempt < runtimeSettings.maxAttempts && isTransientRuntimeError(result.output);
        if (shouldRetry) {
