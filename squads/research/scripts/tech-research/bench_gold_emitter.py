...
def _normalize_groups_to_dimensions(groups):
    """Normalize matrix.groups (list or dict) to dimensions list. Robust to both formats."""
    if isinstance(groups, list):
        return [
            {
                "id": g.get("id", str(i)),
                "name": g.get("label", g.get("name", g.get("id", str(i)))),
                "weight": g.get("weight", 5),
                "rationale": g.get("rationale", g.get("shortform_meaning", "")),
            }
        ...
        return [
            {
                "id": k.lower().replace(" ", "-"),
                "name": k,
                "weight": v.get("weight", 5) if isinstance(v, dict) else 5,
...
def emit_decision_rubric(bench_dir, metadata, matrix, scorecard):
    ...
    rubric = {
        ...
        "dimensions": _normalize_groups_to_dimensions(matrix.get("groups", {})),
...
def emit_bench_contract(bench_dir, metadata, matrix):
    ...
    Respects MERGE_EXISTING: if bench-contract.json exists and merge mode is on,
    skip writing to preserve hand-enriched fields (scoring_model.method_family,
    taxonomy.row_unit, decision_rows, etc).
    """
    target = bench_dir / "bench-contract.json"
    if MERGE_EXISTING and target.exists():
        sys.stderr.write(f"  SKIP (exists, --merge-existing): bench-contract.json\n")
        return
    ...
    contract = {
        ...
        "taxonomy": {
            "groups": (
                [g.get("id") for g in matrix.get("groups", [])]
                if isinstance(matrix.get("groups", {}), list)
                else list(matrix.get("groups", {}).keys())
            ),
            ...
            # NOTE: validator requires categories, row_unit, player_model, dimension_source.
            # Enrich manually after first emission.
            "categories": (
                [g.get("name") or g.get("id") for g in matrix.get("groups", [])]
                if isinstance(matrix.get("groups", {}), list)
                else list(matrix.get("groups", {}).keys())
            ),
            "row_unit": "microdimension",
            "player_model": f"{len(matrix.get('players', []))}-player benchmark",
            "dimension_source": "Derived from comparison-matrix.json",
        },
        "scoring_model": {
            "method_family": "weighted_sum",
            "formula": "total_player = sum(cell_score × group_weight) / sum(group_weights = 100)",
            "scale": "0-100",
            "weight_policy": "Group weights sum to 100. Microdimensions weighted uniformly within group.",
            "normalization": "Weighted mean preserves 0-100 scale",
            "tie_break_policy": "On tie, prefer anchor player",
            "score_range": "0-100",
        },
        "expected_sources": [
