...
CANONICAL_FILES = {
    ...
    "executive-report.md": ("bench_report", "bench-executive-report"),
    "bench-output-dash.json": ("bench_dash", "bench-output-dash"),
    "comparison-matrix.json": ("matrix", "comparison-matrix"),
    "comparison-matrix.md": ("matrix_doc", "comparison-matrix-md"),
    "gap-analysis.md": ("gap_analysis", "gap-analysis"),
    "scorecard.json": ("scorecard", "scorecard"),
    "scorecard.md": ("scorecard_doc", "scorecard-md"),
...
def build_graph(folder: Path, mode: str = "research") -> dict[str, Any]:
    slug = folder.name
    ...
    bench_dash = _read_json(folder / "bench-output-dash.json")
    if mode == "bench" and isinstance(bench_dash, dict):
        dash_players = bench_dash.get("players")
        if isinstance(dash_players, list):
            for player in dash_players:
                if not isinstance(player, dict):
                    continue
                pid = str(player.get("id") or player.get("key") or "").strip()
                if not pid:
                    continue
                nid = player_nodes.get(pid) or f"player-{pid}"
                player_nodes[pid] = nid
                add_node(
                    {
                        "id": nid,
                        "type": "player",
                        "label": player.get("name") or pid,
                        "category": player.get("category"),
                        "tier": player.get("tier"),
                ...
                add_edge(file_nodes.get("bench-output-dash.json"), nid, "indexes")

        matrix = bench_dash.get("matrix")
        dimensions = matrix.get("dimensions") if isinstance(matrix, dict) else None
        group_nodes: dict[str, str] = {}
        if isinstance(dimensions, list):
            for dimension in dimensions:
                if not isinstance(dimension, dict):
                    continue
                did = str(dimension.get("id") or "").strip()
                if not did:
                    continue
                group = _short(dimension.get("group") or "Ungrouped", 80)
                group_id = "dimension-group-" + re.sub(r"[^a-z0-9]+", "-", group.lower()).strip("-")
                if group not in group_nodes:
                    group_nodes[group] = group_id
                    add_node({"id": group_id, "type": "dimension_group", "label": group})
                    add_edge(file_nodes.get("bench-output-dash.json"), group_id, "contains_dimension_group")
                nid = f"dimension-{did}"
                add_node(
                    {
                        "id": nid,
                        "type": "dimension",
                        "label": _short(dimension.get("label") or dimension.get("question") or did, 96),
                        "dimension_id": did,
                        "parent_id": dimension.get("parent_id"),
                        "group": group,
                        "weight": dimension.get("weight"),
                    }
                )
                add_edge(group_nodes.get(group), nid, "contains_microdim")

        gaps = bench_dash.get("gaps")
                ...
                add_node(
                    {
                        "id": nid,
                        "type": "gap",
                        "label": _short(gap.get("title") or gid, 96),
                        "priority": gap.get("priority"),
                        "complexity": gap.get("complexity"),
                        "summary": gap.get("rationale"),
                    }
                )
                add_edge(file_nodes.get("gap-analysis.md") or file_nodes.get("bench-output-dash.json"), nid, "indexes")

        personas = bench_dash.get("personas")
        if isinstance(personas, list):
            for persona in personas:
                if not isinstance(persona, dict):
                    continue
                persona_id = str(persona.get("id") or persona.get("label") or "").strip()
                if not persona_id:
                    continue
                nid = "persona-" + re.sub(r"[^a-z0-9]+", "-", persona_id.lower()).strip("-")
                add_node({"id": nid, "type": "persona", "label": persona.get("label") or persona_id})
                add_edge(file_nodes.get("bench-output-dash.json"), nid, "declares_persona")
                ranking = persona.get("ranking")
                if isinstance(ranking, list):
                    for row in ranking[:10]:
                        if not isinstance(row, dict):
                            continue
                        add_edge(nid, player_nodes.get(str(row.get("player") or "")), "ranks", rank=row.get("rank"), score=row.get("score"))

        duels = bench_dash.get("duels")
        if isinstance(duels, list):
            for duel in duels:
                if not isinstance(duel, dict):
                    continue
                did = str(duel.get("id") or "").strip()
                if not did:
                    continue
                a = str(duel.get("a") or "").strip()
                b = str(duel.get("b") or "").strip()
                nid = f"duel-{did}"
                add_node(
                    {
                        "id": nid,
                        "type": "duel",
                        "label": _short(duel.get("verdict") or did, 120),
                        "a": a,
                        "b": b,
                        "wins_a": len(duel.get("winsA") or []) if isinstance(duel.get("winsA"), list) else duel.get("winsA"),
                        "wins_b": len(duel.get("winsB") or []) if isinstance(duel.get("winsB"), list) else duel.get("winsB"),
                    }
                )
                add_edge(file_nodes.get("bench-output-dash.json"), nid, "indexes")
                add_edge(nid, player_nodes.get(a), "competes_with", side="a")
                add_edge(nid, player_nodes.get(b), "competes_with", side="b")
...
def main() -> int:
    ...
    if _multi_mode and not args.quiet:
        sys.stderr.write(f"[research-graph] mode={mode} (folder: {folder.name})\n")

    graph = build_graph(folder, mode=mode)
