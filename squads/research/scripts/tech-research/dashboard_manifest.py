...
BENCH_TAB_CONTRACT: dict[str, dict[str, Any]] = {
    "map": {
        "primary_artifacts": ["README.md", "bench-contract.json", "metrics.yaml", "research-profile.yaml", "bench-output-dash.json", "research-graph.json"],
        "expected_value": "Visão executiva do benchmark, tipo de decisão, prontidão e rastreabilidade visual.",
        "tier": "required",
    },
    "matrix": {
        "primary_artifacts": ["comparison-matrix.json", "comparison-matrix.md", "matrices.yaml", "decision-rubric.yaml"],
        "expected_value": "Comparação granular por dimensões, pesos e score interpretável.",
        "tier": "required",
    },
    "duel": {
        "primary_artifacts": ["bench-output-dash.json"],
        "expected_value": "Confrontos par-a-par derivados da matriz para explicar trade-offs.",
        "tier": "gold",
    },
    "score": {
        "primary_artifacts": ["scorecard.json", "scorecard.md", "decision-rubric.yaml", "evaluation-rubric.yaml"],
        "expected_value": "Ranking e semântica de score calibrada para a solicitação.",
        "tier": "required",
    },
    "personas": {
        "primary_artifacts": ["decision-rubric.yaml", "bench-output-dash.json", "players.yaml"],
        "expected_value": "Rubricas/personas que mudam o ranking segundo critérios de decisão.",
        "tier": "gold",
    },
    "tco": {
        "primary_artifacts": ["risk-register.yaml", "sources.yaml", "claims.yaml"],
        "expected_value": "Custos, riscos, licenças e restrições de absorção/adoção.",
        "tier": "optional",
    },
    "decision": {
        "primary_artifacts": ["executive-report.md", "action-plan.yaml", "decision-ledger.yaml", "risk-register.yaml"],
        "expected_value": "Veredito, ações recomendadas, riscos e próximos passos.",
        "tier": "required",
    },
    "evidence": {
        "primary_artifacts": ["sources.yaml", "claims.yaml", "research-graph.json", "validation-report.yaml"],
        "expected_value": "Provas, claims e ligações entre fontes, matriz e decisão.",
        "tier": "required",
    },
    "document": {
        "primary_artifacts": ["README.md", "INDEX.md", "MANIFEST.json"],
        "expected_value": "Auditoria direta dos artefatos brutos do benchmark.",
        "tier": "required",
    },
}
...
def build_manifest(folder: Path, mode: str = "research") -> dict[str, Any]:
    ...
    if mode == "bench" and not research_contract:
        research_contract = read_json(folder / "bench-contract.json")

    tabs = {}
    tab_contract = BENCH_TAB_CONTRACT if mode == "bench" else TAB_CONTRACT
    for tab, contract in tab_contract.items():
        status, present = tab_status(folder, contract["primary_artifacts"])
        tabs[tab] = {
            "status": status,
            "tier": contract.get("tier", "required"),
    ...
    return {
        ...
        "tabs": tabs,
        "tier_per_tab": {tab: data["tier"] for tab, data in tabs.items()},
        "quality_bars": {
...
def build_validation_report(folder: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    decision_rubric = read_yaml(folder / "decision-rubric.yaml")
    research_contract = read_json(folder / "research-contract.json")
    if manifest.get("mode") == "bench" and not research_contract:
        research_contract = read_json(folder / "bench-contract.json")
    ...
    if manifest.get("mode") == "bench":
        required_contract_fields = ["type", "decision_improved", "anchor", "taxonomy", "scoring_model", "completeness_criteria"]
        contract_label = "bench-contract.json"
    else:
        required_contract_fields = ["research_kind", "objective", "decision_context", "taxonomy", "rubric_model", "evidence_model", "stop_rules"]
        contract_label = "research-contract.json"
    ...
    if research_contract and not missing_contract_fields:
        contract_detail = (
            f"present; type: {research_contract.get('type', 'missing')}."
            if manifest.get("mode") == "bench"
            else f"present; kind: {research_contract.get('research_kind', 'missing')}."
        )
    else:
        contract_detail = f"missing or incomplete; missing: {', '.join(missing_contract_fields) or 'file'}."
    checks.append(
        check(
            "RESEARCH-CONTRACT",
            "pass" if research_contract and not missing_contract_fields else "warn",
            f"{contract_label} {contract_detail}",
        )
    )
...
def main() -> int:
    ...
    if _multi_mode and not args.quiet:
        sys.stderr.write(f"[dashboard-manifest] mode={mode} (folder: {folder.name})\n")

    manifest = build_manifest(folder, mode=mode)
