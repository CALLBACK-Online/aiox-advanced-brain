...
def build_action_plan(folder: Path, sections: list[MarkdownSection]) -> dict[str, Any]:
    ...
    for table in markdown_tables(next_steps.body if next_steps else ""):
        for row in table["rows"][:8]:
            ...
            raw_actions.append(clean_text(f"{title}: {details}" if details else title, 220))
    raw_actions = raw_actions or markdown_list_items(next_steps.body if next_steps else "", 8)
...
def collect_sections(folder: Path, mode: str = "research") -> list[MarkdownSection]:
    if mode == "bench":
        files = [
            "executive-report.md",
            "gap-analysis.md",
            "verdict-comparative-final.md",
            "README.md",
        ]
    else:
        files = [
...
def build_assets(folder: Path, mode: str = "research") -> dict[str, dict[str, Any]]:
    sections = collect_sections(folder, mode=mode)
...
def main() -> int:
    ...
    if _multi_mode and not args.quiet:
        sys.stderr.write(f"[action-assets-extractor] mode={mode} (folder: {folder.name})\n")

    assets = build_assets(folder, mode=mode)
