...
def _read_skill_workflow_version() -> str:
    ...
    skill_path = os.path.join(repo_root, ".claude", "skills", "tech-research", "SKILL.md")
    if not os.path.exists(skill_path):
        return "unknown"
    try:
        with open(skill_path, encoding="utf-8") as f:
            content = f.read(1024)
        match = re.search(r"^workflow_version:\s*[\"']?([^\s\"'\r\n]+)[\"']?", content, re.MULTILINE)
        if match:
...
def compute_metrics(data: dict) -> dict:
    ...
    # AC10: resolve workflow_version from SKILL.md if caller did not supply it.
    skill_wv = _read_skill_workflow_version()
    workflow_version = data.get("workflow_version") or skill_wv
...
def save_state(state: dict, output_dir: str) -> dict:
    ...
    # AC10: resolve workflow_version from SKILL.md if caller did not supply it.
    skill_wv = _read_skill_workflow_version()
    workflow_version = state.get("workflow_version") or skill_wv
