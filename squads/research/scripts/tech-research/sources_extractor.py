...
Parses 02-research-report.md (and 04..NN-followup files) and emits a YAML manifest:
  ...
  sources:
    - id: src-001
      ...
      # STORY-RA-F.3 additive fields (optional — omitted when data unavailable):
      extraction_method: "websearch_snippet" | "webfetch" | "etl" | "playwright" | "manual_paste" | "extraction_blocked"
      extraction_quality: "full" | "snippet_only" | "blocked_fallback_used"
      extraction_attempts: N
      extraction_ladder_steps_tried: ["websearch_snippet", "webfetch"]  # only when ladder was used
      extraction_notes: "..."     # optional, describes failure + fallback used
  totals:
...
STORY-RA-F.3 additive schema (AC-B2):
  extraction_* fields are OPTIONAL. They are emitted ONLY when the data is
  available (e.g., when the extraction escalator was used and logged results).
  When absent, the fields are OMITTED entirely (NOT filled with default values).
  This complies with .claude/rules/extraction-no-fallbacks.md.

  To enrich a source with extraction metadata, callers should:
    1. Run extraction_escalator.escalate(url) to get ExtractionResult
    2. Merge ExtractionResult.to_sources_fields() into the source dict
       before passing to extract_sources()
    3. Or provide a pre-built extraction_metadata dict keyed by URL
...
def extract_sources(
    folder: Path,
    extraction_metadata: dict[str, dict[str, Any]] | None = None,
) -> dict[str, Any]:
    """Extract sources from research/bench folder and emit sources.yaml payload.

    Args:
        folder: Path to docs/research/{date}-{slug}/ or docs/bench/{date}-{slug}/
        extraction_metadata: Optional dict mapping URL → extraction_* fields dict.
            Produced by extraction_escalator.ExtractionResult.to_sources_fields().
            When provided, extraction_* fields are merged into matching source entries.
            When absent or URL not in dict, extraction_* fields are OMITTED (no defaults).
            Per .claude/rules/extraction-no-fallbacks.md: never inject universal defaults.
    """
    slug = folder.name
    raw_findings: list[dict[str, Any]] = []
    _extraction_meta: dict[str, dict[str, Any]] = extraction_metadata or {}
    ...
    for idx, item in enumerate(sorted(seen.values(), key=lambda x: (x["first_seen_in"], x["position"])), start=1):
        ...
        # STORY-RA-F.3 AC-B2: merge extraction_* fields when available.
        # Additive only — fields are OMITTED when extraction_metadata not provided
        # or URL not in the dict. Never inject universal defaults (extraction-no-fallbacks).
        ext_fields = _extraction_meta.get(item["url"])
        if ext_fields:
            for ext_key, ext_val in ext_fields.items():
                if ext_key.startswith("extraction_") and ext_val is not None:
                    entry[ext_key] = ext_val
