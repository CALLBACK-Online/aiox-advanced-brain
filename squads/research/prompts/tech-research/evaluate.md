...
## SET REPRESENTATIVENESS RULE (for category/comparison queries)

When the query implies a set of entities (alternatives, tools, builders, frameworks, libraries, platforms), evaluate set completeness:

- Identify the implied category (e.g., "JavaScript bundlers", "cloud storage providers")
- Count mainstream mentions (≥ 5 sources cite it): these are the "expected set"
- Compare included_in_report[] against expected set
- If coverage < 60% of mainstream mentions: add a gap entry with priority "high"

```json
{
  "dimension": "comparison",
  "gap": "Set representativeness: covered 3/8 mainstream {category} entities. Missing: {entity_A}, {entity_B}, ...",
  "priority": "high"
}
```
...
- ❌ Implicitly narrowing comparison universe without declaring scope_declaration
