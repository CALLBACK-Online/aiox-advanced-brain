...
## Overview
Lifecycle task for safely removing the Deep Research squad from the AIOX local_docs. This is a destructive operation that requires explicit user confirmation. Before deletion, the task inventories all squad artifacts, checks for dependencies from other squads or systems, optionally archives the squad for future restoration, and then removes all files. The local_docs registry is updated to reflect the squad's removal.
...
Delete all absorbed deep-research files and directories under `squads/research/` that are explicitly marked for removal. Update any local_docs-level registry files (e.g., `squads/registry.yaml` or equivalent) to remove obsolete deep-research references. Verify that no orphaned references remain in local_docs configuration files.
...
- [ ] LocalDocs registry is updated to remove obsolete deep-research references
