...
META_SESSION_MARKERS = (
    ...
    '"mode": "facet"',
    '"mode": "narrative"',
    # Context-compaction stubs: the engine writes a ~7-line transcript per
    # compacted segment whose only user message is this summarize directive.
    # Counting these as sessions inflates the denominator (observed: 35,337
    # such stubs in one project, 85x the real session count).
    "Summarize this conversation segment into key facts",
)
...
def build_report_summary(all_data, global_metrics, cache_stats):
    ...
    scanned = cache_stats["sessions_scanned"] or 1
    machine_excluded = (
        cache_stats["meta_sessions_filtered"]
        + cache_stats["non_substantive_filtered"]
        + cache_stats["minimal_facet_filtered"]
    )
    machine_ratio = machine_excluded / scanned
    return {
        ...
        "machine_excluded_ratio": round(machine_ratio, 4),
