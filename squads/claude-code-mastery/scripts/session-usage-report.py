...
# Slash-prefixed terms that are CLI control commands, not skills/agents.
_HISTORY_NOISE_COMMANDS = {
    "clear", "export", "model", "context", "compact", "logout", "login",
    "exit", "buddy", "help", "resume", "cost", "status", "vim",
    "terminal-setup", "add-dir", "memory", "config", "init", "agents",
    "mcp", "doctor", "bug", "release-notes", "privacy-settings",
    "permissions", "hooks", "users",
}

# Intent taxonomy for free-text prompts. Free text is ~77% of typed activity
# and is invisible to slash-command counting — this surfaces what the user
# actually asks for in natural language. Order matters: first match wins, so
# more specific intents precede generic ones. Keyword sets are PT-BR + EN.
_INTENT_TAXONOMY = (
    ("commit_git_deploy", ("commit", "push", " pr ", "deploy", "merge", "branch", "git ")),
    ("fix_bug", ("corrige", "corrija", " fix", "erro", "bug", "não funciona",
                 "nao funciona", "quebrou", "falha", "problema")),
    ("stop_cancel", ("para ", "pare", "espera", "cancela", "stop", "aborta")),
    ("create_implement", ("cria ", "crie", "implementa", "implemente", "faça", "faz ",
                          "build", "adiciona", "adicione", "gera ", "gere")),
    ("analyze_review", ("analisa", "analise", "revisa", "revise", "verifica", "verifique",
                        "confere", "review", "check", "audita", "investiga")),
    ("explain_question", ("explica", "explique", "por que", "porque", "como ",
                          "o que é", "qual ", "me mostra", "mostra")),
    ("refactor_improve", ("refatora", "melhora", "melhore", "otimiza", "simplifica",
                          "organiza", "ajusta")),
    ("continue", ("continua", "continue", "segue", "prossiga", "siga", "pode seguir")),
    ("approve_confirm", ("sim", "ok", "aprovado", "perfeito", "exato", "correto",
                         "pode", "vamos", "beleza", "show", "isso")),
)


def classify_prompt_intent(text):
    """Return the dominant intent label for a free-text prompt, or None."""
    lowered = text.lower()
    for label, keywords in _INTENT_TAXONOMY:
        if any(keyword in lowered for keyword in keywords):
            return label
    return None


def analyze_history(history_path, days, allowed_projects=None):
    """Parse ~/.claude/history.jsonl — the durable record of typed prompts.

    Session JSONL files are rotated/pruned by Claude Code, so a sessions-only
    analysis silently loses most historical activity. history.jsonl persists
    every prompt, making it the source of truth for typed skill/command usage.
    Returns None when the file is absent (graceful degradation).
    """
    if not os.path.isfile(history_path):
        return None

    cutoff_ms = (datetime.now() - timedelta(days=days)).timestamp() * 1000
    command_re = re.compile(r"^/([a-zA-Z][\w:-]*)")
    ...
    with open(history_path, "r", errors="replace") as handle:
        for line in handle:
            try:
            ...
            project = (entry.get("project") or "").rstrip("/")
            project_name = os.path.basename(project) or "/"
            ...
            total_prompts += 1
            per_project[project_name] += 1
            if ts_ms is not None:
                month = datetime.fromtimestamp(ts_ms / 1000).strftime("%Y-%m")
                per_month[month] += 1
                earliest = ts_ms if earliest is None else min(earliest, ts_ms)
                latest = ts_ms if latest is None else max(latest, ts_ms)

            match = command_re.match(display)
            if match:
                slash_prompts += 1
                name = match.group(1)
                if ":" in name:
                    name = name.split(":")[-1]
                if name.lower() not in _HISTORY_NOISE_COMMANDS:
                    skill_counts[name] += 1
            else:
                # Free-text prompt: classify intent + length profile.
                free_text_prompts += 1
                intent = classify_prompt_intent(display)
                if intent:
                    intent_counts[intent] += 1
                length = len(display)
                if length < 20:
                    length_buckets["xs (<20)"] += 1
                elif length < 80:
                    length_buckets["short (20-80)"] += 1
                elif length < 300:
                    length_buckets["medium (80-300)"] += 1
                else:
                    length_buckets["long brief (300+)"] += 1
    ...
    return {
        ...
        "free_text_prompts": free_text_prompts,
        "skill_counts": skill_counts,
        "per_project": per_project,
        "per_month": dict(sorted(per_month.items())),
        "intent_counts": intent_counts,
        "length_buckets": length_buckets,
        "date_range": [_fmt(earliest), _fmt(latest)],
    }
...
def generate_report(
    ...
    if allowed_projects:
        ...
        def find_projects_scoped(projects_dir):
            return {
                name: path
                for name, path in find_projects(projects_dir).items()
                if name in allowed
            }
        find_projects_fn = find_projects_scoped
    else:
    ...
    return generate_report_impl(
        ...
        refresh_cache=refresh_cache,
        insights_command=insights_command,
        find_projects_fn=find_projects_fn,
...
def main():
    ...
    parser.add_argument("--all-projects", action="store_true",
                        help="Analyze ALL projects in --projects-dir. By default, when --repo "
                             "is given, metrics are scoped to the matching project(s) to avoid "
                             "cross-project contamination from unrelated repos.")
    ...
    # Scope to the current repo by default (avoids cross-project contamination).
    # --all-projects opts back into the full ~/.claude/projects scan.
    allowed_projects = None
    if not args.all_projects and args.repos:
        allowed_projects = {short_name(os.path.basename(r.rstrip("/"))) for r in args.repos}
    ...
    # GAP-3 sanity gate: when most scanned transcripts are machine artifacts
    # (compaction stubs, facet/narrative enrichment), the real session count is
    # a small fraction of what was scanned. Surfacing this prevents the silent
    # denominator inflation that made skill/tool counts look artificially low.
    machine_ratio = data.get("summary", {}).get("machine_excluded_ratio", 0)
    if machine_ratio > 0.5:
        print(
            f"  WARNING: {machine_ratio:.0%} of scanned transcripts were machine "
            f"artifacts (compaction/enrichment), not real sessions. Real-session "
            f"counts are correct, but verify the analysis window/projects-dir if "
            f"this ratio is unexpected.",
            file=sys.stderr,
        )

    # Enrich with the durable typed-prompt history. Session JSONLs are pruned by
    # Claude Code, so they undercount long-term usage; history.jsonl persists.
    history_path = os.path.join(os.path.dirname(args.projects_dir.rstrip("/")), "history.jsonl")
    data["history"] = analyze_history(history_path, args.days, allowed_projects=allowed_projects)
    ...
    if args.output == "markdown":
        ...
        print(json.dumps({
            ...
            "history": (
                {
                    **data["history"],
                    "skill_counts": dict(data["history"].get("skill_counts", {})),
                    "per_project": dict(data["history"].get("per_project", {})),
                    "intent_counts": dict(data["history"].get("intent_counts", {})),
                    "length_buckets": dict(data["history"].get("length_buckets", {})),
                }
                if data.get("history") else None
            ),
        }, indent=2))
