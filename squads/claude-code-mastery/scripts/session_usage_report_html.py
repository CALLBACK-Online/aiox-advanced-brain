...
def format_html(data, days, project_roots=None, discover_inventory_fn=None, get_recent_commits_fn=None):
    ...
    top_skills_rows = [
        ...
        for skill, _ in skill_rollup["combined"].most_common(10)
    ...
    # Durable history (history.jsonl) — source of truth for typed usage.
    history = data.get("history") or {}
    history_section_html = ""
    if history.get("total_prompts"):
        total_p = history["total_prompts"]
        slash_p = history.get("slash_prompts", 0)
        free_p = history.get("free_text_prompts", 0)
        date_range = history.get("date_range") or [None, None]

        history_kpis = render_html_kv_grid([
            ("Prompts no período", format_number(total_p)),
            ("Comandos /slash", f"{format_number(slash_p)} ({round(100*slash_p/total_p)}%)"),
            ("Texto livre", f"{format_number(free_p)} ({round(100*free_p/total_p)}%)"),
            ("Range coberto", f"{date_range[0] or '?'} → {date_range[1] or '?'}"),
        ])

        month_rows = [
            [html.escape(month), html.escape(format_number(count))]
            for month, count in (history.get("per_month") or {}).items()
        ]
        skill_rows = [
            [f"<code>{html.escape(name)}</code>", html.escape(format_number(count))]
            for name, count in Counter(history.get("skill_counts") or {}).most_common(10)
        ]
        intent_total = sum((history.get("intent_counts") or {}).values()) or 1
        intent_rows = [
            [html.escape(label), html.escape(format_number(count)),
             html.escape(f"{round(100*count/intent_total)}%")]
            for label, count in Counter(history.get("intent_counts") or {}).most_common()
        ]
        length_order = ["xs (<20)", "short (20-80)", "medium (80-300)", "long brief (300+)"]
        length_rows = [
            [html.escape(bucket), html.escape(format_number((history.get("length_buckets") or {})[bucket]))]
            for bucket in length_order if bucket in (history.get("length_buckets") or {})
        ]

        history_section_html = render_html_section(
            "Histórico Real de Uso (history.jsonl)",
            history_kpis
            + render_html_table(["Mês", "Prompts"], month_rows)
            + "<h3>Top 10 Skills/Agents Digitados</h3>"
            + render_html_table(["Skill/Command", "Invocações"], skill_rows)
            + "<h3>Intenção do Texto Livre</h3>"
            + render_html_table(["Intenção", "Prompts", "%"], intent_rows)
            + "<h3>Perfil de Tamanho dos Prompts Livres</h3>"
            + render_html_table(["Faixa (chars)", "Prompts"], length_rows),
            subtitle="Fonte de verdade do uso digitado. Sessões .jsonl são podadas pela CC; "
                     "o history.jsonl persiste todo prompt — inclui os ~77% de texto livre "
                     "invisíveis à contagem de comandos.",
        )
