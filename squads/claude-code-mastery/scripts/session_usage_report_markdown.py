...
def format_markdown(
    ...
    if history and history.get("total_prompts"):
        date_range = history.get("date_range") or [None, None]
        lines.append("## Histórico Real de Uso (history.jsonl)")
        lines.append("")
        lines.append("> Source of truth for typed usage. Session JSONL files are pruned by "
                     "Claude Code over time, so the session metrics above can undercount "
                     "long-term activity. `history.jsonl` persists every typed prompt.")
        lines.append("")
        total_prompts = history["total_prompts"]
        slash_prompts = history.get("slash_prompts", 0)
        free_text = history.get("free_text_prompts", 0)
        lines.append(f"- **Prompts no período:** {format_number(total_prompts)}")
        if total_prompts:
            slash_pct = round(100 * slash_prompts / total_prompts)
            free_pct = round(100 * free_text / total_prompts)
            lines.append(f"- **Comandos `/slash`:** {format_number(slash_prompts)} ({slash_pct}%) · "
                         f"**Texto livre (conversa):** {format_number(free_text)} ({free_pct}%)")
        if date_range[0]:
            lines.append(f"- **Range coberto:** {date_range[0]} → {date_range[1]}")
        lines.append("")
        per_month = history.get("per_month") or {}
        if per_month:
            lines.append("| Mês | Prompts |")
            lines.append("|-----|---------|")
            for month, count in per_month.items():
                lines.append(f"| {month} | {format_number(count)} |")
            lines.append("")
        skill_counts = Counter(history.get("skill_counts") or {})
        if skill_counts:
            lines.append("### Top 10 Skills/Agents Digitados (histórico durável)")
            lines.append("")
            lines.append("| Skill/Command | Invocações |")
            lines.append("|--------------|-----------|")
            for name, count in skill_counts.most_common(10):
                lines.append(f"| `{name}` | {format_number(count)} |")
            lines.append("")

        intent_counts = Counter(history.get("intent_counts") or {})
        if intent_counts:
            classified = sum(intent_counts.values())
            lines.append("### Intenção do Texto Livre")
            lines.append("")
            lines.append(f"> O que você pede em linguagem natural — {format_number(classified)} "
                         f"de {format_number(free_text)} prompts livres classificados. "
                         f"Esta camada é invisível à contagem de comandos.")
            lines.append("")
            lines.append("| Intenção | Prompts | % |")
            lines.append("|----------|---------|---|")
            for label, count in intent_counts.most_common():
                pct = round(100 * count / classified) if classified else 0
                lines.append(f"| {label} | {format_number(count)} | {pct}% |")
            lines.append("")

        length_buckets = history.get("length_buckets") or {}
        if length_buckets:
            order = ["xs (<20)", "short (20-80)", "medium (80-300)", "long brief (300+)"]
            lines.append("### Perfil de Tamanho dos Prompts Livres")
            lines.append("")
            lines.append("| Faixa (chars) | Prompts |")
            lines.append("|---------------|---------|")
            for bucket in order:
                if bucket in length_buckets:
                    lines.append(f"| {bucket} | {format_number(length_buckets[bucket])} |")
            lines.append("")
    ...
    lines.append("## Top 10 Skills/Commands")
    lines.append("")
    lines.append("> Counts combine both invocation vias: slash typed (`<command-name>`) and "
                 "the `Skill` tool. Subagents (`Agent` tool / `subagent_type`) are reported "
                 "separately under **Top Agents** — they are work dispatched by orchestrators, "
                 "not user-invoked skills.")
    lines.append("")
    ...
    for skill, _ in combined.most_common(10):
