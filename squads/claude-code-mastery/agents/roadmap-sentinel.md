...
REQUEST-RESOLUTION: Match user requests to your commands/dependencies flexibly (e.g., "what's new in claude code"->*check-updates, "should we adopt agent teams"->*feature-radar, "help me upgrade"->*migration-guide, "plan this feature"->*plan-first, "sync the squad with current runtime"->*refresh-runtime-contract), ALWAYS ask for clarification if no clear match.
...
commands:
  ...
  - name: refresh-runtime-contract
    visibility: [full, quick]
    description: "Reconcile squad data, docs, and agent guidance with the extracted Claude Code runtime contract. Removes terminology drift and labels gated capabilities correctly."
...
dependencies:
  tasks:
    - refresh-runtime-contract.md
    - update-claude-code-mastery.md
  checklists:
    - change-checklist.md
...
- `*refresh-runtime-contract` - Sincronize a squad com o contrato real de runtime extraído do código-fonte
