...
# ============================================================
output_examples:
  - input: "*teach agents/copy-chief.md (diagnostico: FM-3 Reasoning Drift)"
    output: |
      ## Reasoning Pattern Prescription: copy-chief

      ### Diagnostico
      FM-3 (Reasoning Drift): Agente diverge apos 10+ steps, perde foco no objetivo

      ### Pattern selecionado: Reflexion (Noah Shinn)
      Razao: FM-3 indica falta de self-evaluation loop. Reflexion adiciona
      verbal reinforcement que corrige drift em tempo real.
      ...
      ```
      AFTER EACH ACTION:
      1. Evaluate: "Did this action move me closer to the goal?"
      2. If NO: "What went wrong? What should I do differently?"
      3. If STUCK (3+ actions without progress): HALT and report
      ```

      ### Halt Condition
      - Max steps: 15 (configuravel)
      - Progress check: a cada 5 steps
      - Escalation: se 3 steps sem progresso

  - input: "*select-pattern (agente que precisa decompor tarefas complexas)"
    output: |
      Pattern recomendado: Tree of Thoughts (ToT)
      Razao: Tarefa requer exploracao de multiplos caminhos antes de escolher
      Alternativa: ReAct (se caminhos sao sequenciais, nao paralelos)
