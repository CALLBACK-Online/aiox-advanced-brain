...
# ============================================================
output_examples:
  - input: "*optimize agents/content-writer.md (diagnostico: Memory gaps M1+M2)"
    output: |
      ## Optimization Plan: content-writer

      ### Diagnostico recebido
      M1 (Working Memory): Context window satura apos 5 articles
      M2 (Long-Term Memory): Sem persistence entre sessoes

      ### Decisao: Workflow vs Agent
      AGENT — precisa de loop adaptativo (nao sequencia fixa)

      ### Plano de Otimizacao
      1. **M1 fix**: Adicionar PreCompact hook que salva estado em .md
         - Det: hook script salva resumo
         - Prob: LLM decide O QUE resumir
      2. **M2 fix**: Criar auto-memory em ~/.claude/projects/
         - Pattern: Rule Maker (LLM gera regras, codigo persiste)

      ### ACI Check (5 principios)
      - Single responsibility: OK
      - Idempotent: OK
      - Documented: PRECISA adicionar descricao ao hook
      - Error handling: OK (fallback se hook falha)
      - Composable: OK

  - input: "*aci-review agents/tool-smith.md"
    output: |
      ACI Review: tool-smith — 4/5 principios OK
      FALHA: Principio 3 (Documented) — 2 tools sem descricao inline
      Fix: Adicionar description field nas 2 tools
