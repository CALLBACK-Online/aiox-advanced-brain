...
### Phase 3: Optimize (varia por routing — com pesquisa obrigatória)

**IMPORTANTE**: Todas as tasks abaixo agora incluem gates de pesquisa externa obrigatória. Nenhuma otimização é aplicada sem validação de evidência. Isso garante consistência no ciclo: diagnóstico validado → fix validado → medição.

**Se Planning gaps:**

- **Agent**: reasoning-engineer
- **Task**: `tasks/teach-reasoning.md` (inclui Step 2: validação de pattern via pesquisa)
- **Ação**: selecionar/configurar reasoning pattern — validado contra evidência para o tipo de agente

**Se Memory gaps:**

- **Agent**: agent-architect
- **Task**: `tasks/optimize-agent.md` (inclui Step 3: validação de técnicas via pesquisa)
- **Ação**: redesenhar memory strategy — apenas técnicas com evidência de impacto

**Se Tool Use gaps:**

- **Agent**: ecosystem-scout → tool-smith
- **Tasks**: `tasks/search-ecosystem.md` → `tasks/suggest-tools.md`
- **Ação**: encontrar libs existentes ou construir tools (search-ecosystem já é pesquisa nativa)

**Se Failure Modes detectados:**

- **Agent**: autonomy-auditor
- **Task**: `tasks/diagnose-autonomy-failure.md` (inclui Step 5: validação de fixes via pesquisa)
- **Ação**: deep diagnosis com 5 Whys → fixes validados contra evidência
...
| # | Otimização | Classificação | Fonte |
|---|-----------|---------------|-------|
| 1 | {mudança} | Validada/Experimental | {link} |

### Otimizações Descartadas (transparência)
| Candidata | Motivo |
|-----------|--------|
| {técnica} | {sem evidência / incompatível com tipo} |
