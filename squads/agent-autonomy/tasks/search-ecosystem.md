...
Domain: `Tactical`
atomic_layer: Molecule
Input: search_query, resource_type
Output: ecosystem_report
Pre-condition: tipo de recurso e query delimitados
Post-condition: top 3 priorizado com rubric, trade-offs e integração proposta
Performance: avaliar no mínimo 3 opções e registrar score ponderado por dimensão

## METADATA
- **Executor:** ecosystem-scout (Worker)
- **Elicit:** true
...
|------|----------------|
| Framework | `"{domain} agent framework {lang}"` |
| Library | `"{lang} library {capability} for LLM agents"` |
