...
Domain: `Operational`
atomic_layer: Molecule
Input: agent_id_or_spec, tool_type
Output: tool_spec_or_implementation, cross-domain-handoff
Pre-condition: necessidade da tool identificada e agente consumidor definido
Post-condition: ACI 6/6 validado, risco classificado e output testável entregue
Performance: bloquear recomendação com lethal trifecta 3/3 ou sem evidência mínima

## METADATA
- **Executor:** tool-smith (Worker)
- **Elicit:** true
# Task: Suggest Tools
...
Se encontrado: avaliar com rubric (5 dimensões).

#### 2a. Validate Candidates via External Research (BLOCKING)

Para cada tool candidata (encontrada ou a construir), pesquisar:

- Usar Exa (`mcp__exa__web_search_exa`):
  - Query: "[tool/lib name] production reliability issues"
  - Query: "[tool name] vs [alternatives] comparison"
- Min 1 query por candidata

Classificar cada candidata:

- **Validada**: evidência externa confirma estabilidade + compatibilidade
- **Plausível**: sem fonte externa documentada OU evidência parcial — documentar como "testar antes de adotar" (classificação máxima sem pesquisa)
- **Descartada**: evidência externa de risco, incompatibilidade ou refutação explícita — NÃO recomendar
...
- **QG-005**: Tool Quality + External Evidence Validated
