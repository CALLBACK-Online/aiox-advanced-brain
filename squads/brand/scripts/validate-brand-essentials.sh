...
if [ -n "${missing_output_contract}" ]; then
  echo "FAIL workflow/task output contract drift (legacy dialect — EPIC-120 requires L2-tactical):"
...
if [ -n "${bad_output_paths}" ]; then
  echo "FAIL non-canonical output paths (use L2-tactical/brand/ — EPIC-120):"
