...
- `db_url` (string): Database connection string (`SUPABASE_DB_URL` or `DATABASE_URL` env var); required for live schema queries
- `output_dir` (string, optional): Directory where the audit report is saved; defaults to `supabase/docs/`
- `mode` (enum, optional): `full` (all checks) or `security` (security-only via `--security-audit` flag); defaults to `full`
...
- **audit report**: `supabase/docs/schema-audit-{timestamp}.md` with full findings, counts, and recommendations
- **overall score**: `{score}/100` against the schema rubric (100 perfect, <60 critical)
- **issue ledger**: counts of critical issues (🔴), warnings (⚠️), and recommendations (💡)
- **top issues list**: prioritized list of the most impactful findings
- **next-step prompts**: console hints to review report, prioritize fixes, create migrations for P0 issues
...
| Condition | Action |
|-----------|--------|
| `SUPABASE_DB_URL` / `DATABASE_URL` not set or connection refused | Abort with clear message listing the missing env var; suggest running `detect-database.sh` to diagnose connectivity |
| `health-checker.sh` preflight exits non-zero | Display the script output and halt; do not proceed to manual checks until connectivity is confirmed |
| `psql` command not found | Instruct user to install PostgreSQL client tools and re-run |
| `CREATE TEMP TABLE audit_*` fails (e.g. permission denied) | Fall back to inline subquery approach for affected checks; note the degraded mode in the report |
| Report output directory (`supabase/docs/`) does not exist | Create the directory automatically before writing the report |
| Check reveals PII columns without encryption | Include as a Critical finding in the report with remediation SQL; do not auto-apply any changes |

## Post-conditions

After successful execution:
- Full schema inventory captured: tables, columns, types, constraints, indexes, FKs, RLS posture
- Findings classified into Critical / High / Medium severity buckets with remediation SQL per finding
- Unnamed constraints, unencrypted PII, missing FK indexes, and type smells (`TIMESTAMP` without TZ, `VARCHAR > 255`, `FLOAT` for money) explicitly listed
- Audit report persisted under `supabase/docs/` with the timestamp of the run

Acceptance Criteria:
- [ ] Every issue in the report carries a `severity`, an affected `table.column` (where applicable), and a ready-to-run remediation SQL snippet
- [ ] No remediation is auto-applied — the task only emits findings; mutating commands require a separate `*apply-migration` flow
- [ ] Output report directory exists at `supabase/docs/` (created if absent) and the report filename includes the run timestamp
