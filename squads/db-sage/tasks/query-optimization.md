...
- **optimization migration**: `supabase/migrations/{TS}_optimize_query.sql` containing the indexes / statistics changes applied
- **performance benchmark**: before/after execution time + speedup factor + % improvement
- **applied changes list**: indexes created, statistics refreshed (`ANALYZE`), and any query rewrites recommended
- **next-step prompts**: console hints to monitor production, update application code, and review similar patterns
...
| Condition | Action |
|-----------|--------|
| `pg_stat_statements` has no data | Ensure extension is enabled and database has received sufficient traffic; run workload then retry |
| EXPLAIN ANALYZE takes too long or hangs | Cancel query (`Ctrl+C`), analyze with `EXPLAIN` (no ANALYZE) first to inspect plan safely |
| `CREATE INDEX CONCURRENTLY` fails mid-way | Check `pg_stat_progress_create_index`; drop the invalid index and retry |
| Migration file creation fails | Verify write permissions on `supabase/migrations/`; run `mkdir -p supabase/migrations` |
| `ANALYZE` reports permission denied | Requires table owner or superuser role; escalate via `@db-sage` |
| Before/after comparison shows regression | Revert index with `DROP INDEX CONCURRENTLY`; re-run analysis with alternative strategy |
| Database connection fails | Run `*env-check` to validate `SUPABASE_DB_URL` and connectivity |
| Materialized view refresh times out | Increase `statement_timeout` for the session or schedule refresh during off-peak hours |

## Post-conditions

After successful execution:
- Hot-path queries identified from `pg_stat_statements` (mean_exec_time, calls, total_exec_time)
- EXPLAIN (ANALYZE, BUFFERS) executed on candidate queries with index/index-only/bitmap/sequential scans classified
- Optimization actions proposed (covering index, expression index, partial index, materialized view, statistics refresh) tied to specific query nodes
- Before/after benchmark captured for any optimization applied, with regression check before merging

Acceptance Criteria:
- [ ] Every recommendation is paired with the source EXPLAIN node that justifies it (no recommendation without evidence)
- [ ] `CREATE INDEX` recommendations use `CONCURRENTLY` for production-safe rollout
- [ ] After applying any optimization, a re-run of EXPLAIN ANALYZE demonstrates either improved execution time or explicit rollback if regression detected
