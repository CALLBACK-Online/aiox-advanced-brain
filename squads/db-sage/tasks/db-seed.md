...
- **pre-seed snapshot**: `supabase/snapshots/{TS}_before_seed.sql` for rollback
- **seed log entry**: append to `supabase/docs/SEED_LOG.md` with timestamp, file, and result
- **applied seed file**: `{path}` executed against the target database (idempotent via `ON CONFLICT DO NOTHING`)
- **next-step prompts**: console hints to verify data, run smoke tests, and commit the seed file
...
- Pre-seed snapshot persisted at `supabase/snapshots/{TS}_before_seed.sql` before any write
- Seed SQL at `{path}` executed under `ON_ERROR_STOP=1` against the configured database
- Append-only log entry written to `supabase/docs/SEED_LOG.md` with timestamp, file, environment, and user
- Idempotency gate enforced — destructive (`TRUNCATE`/`DELETE FROM`) or non-`ON CONFLICT` seeds require explicit user confirmation

Acceptance Criteria:
- [ ] Re-running the same seed file produces the same final row counts (idempotent via `ON CONFLICT` or `WHERE NOT EXISTS`)
- [ ] `supabase/docs/SEED_LOG.md` contains an entry whose `Date` matches the run and `File` matches `{path}`
- [ ] If user aborts on a destructive-pattern warning, the database remains unchanged (no partial seed)
