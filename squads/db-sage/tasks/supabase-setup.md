...
- `setup_path` (enum): Setup mode — `new` (create new project), `existing` (link existing project), or `local` (local-only dev environment)
- `project_ref` (string, optional): Supabase project reference ID; required when `setup_path=existing`
- `project_name` (string, optional): Human-readable project name; used in generated `.env.local` template
- `region` (string, optional): Supabase region (e.g. `us-east-1`); defaults to `us-east-1` for new projects
...
- **supabase project link**: project linked or created via Supabase CLI (`supabase init` + `supabase link`)
- **folder structure**: `supabase/{migrations,seeds,tests,docs,snapshots}/` directories created with READMEs
- **environment file**: `.env.local` scaffold (placeholder values, requires manual update)
- **helper scripts**: shell helpers placed under `scripts/` (e.g. `db-connect.sh`)
- **enabled extensions**: list of PostgreSQL extensions activated (uuid-ossp, pgcrypto, etc.)
...
| Condition | Action |
|-----------|--------|
| Supabase CLI not installed | Block setup and display install instructions (`https://supabase.com/docs/guides/cli`); exit step 1 with clear message |
| `supabase login` fails or token expired | Ask user to re-authenticate and re-run; do not proceed to project creation |
| `supabase link` fails with invalid project ref | Ask user to verify the ref via `supabase projects list` and re-enter |
| Docker not running when `setup_path=local` | Display Docker install/start instructions; halt until Docker is available |
| Connection verification (`psql ... -c "SELECT version()"`) fails after env vars set | Walk user through the Troubleshooting section (SSL, firewall, pooler port 6543); do not mark setup as complete |
| `psql` extension `CREATE EXTENSION` fails (extension not available on Supabase tier) | Log the failure as a warning, skip that extension, and continue with the remaining ones |

## Post-conditions

After successful execution:
- Supabase CLI authenticated, linked to the target project ref, and `supabase status` reports a healthy connection
- Connection strings (`SUPABASE_DB_URL`, pooler / direct) materialized in the user's local `.env`/`.env.local` (gitignored)
- Round-trip connectivity verified via `psql "$SUPABASE_DB_URL" -c "SELECT version()"`
- Required extensions enabled (or warned-and-skipped when unavailable on the tier)

Acceptance Criteria:
- [ ] `supabase status` exits 0 and prints API URL, DB host, and pooler info for the linked project
- [ ] `.env.local` is present, gitignored, and contains both pooler and direct connection strings with `sslmode=require`
- [ ] No connection string with embedded credentials is echoed in plaintext to stdout during setup (redaction enforced)
