...
- `domain_description` (string, optional): High-level description of the business domain to model; if omitted the agent elicits via the 8-question discovery flow
- `existing_schema` (boolean, optional): Set to `true` to run the preflight `schema-context-loader.sh` before proposing any entities; defaults to `true`
- `output_path` (string, optional): Directory for generated docs (schema-design.yaml, ERD, migration file); defaults to `docs/`
...
- **schema design document**: `docs/schema-design.yaml` with entities, attributes, relationships, and rationale
- **initial schema migration**: `supabase/migrations/{TS}_initial_schema.sql` ready to apply via `*apply-migration`
- **entity-relationship diagram**: `docs/erd.md` markdown ERD for human review
- **entity inventory**: list of entities with count and primary relationships printed to console
...
| Condition | Action |
|-----------|--------|
| `schema-context-loader.sh` fails or DB is unreachable | Warn user, skip preflight, and proceed with manual domain modeling; mark schema grounding as unavailable |
| User provides contradictory relationship cardinality (e.g. 1:1 and M:N for same pair) | Flag the conflict, present both interpretations, and ask user to clarify before designing the table |
| Migration file cannot be written to `supabase/migrations/` (path missing or no write access) | Prompt user for an alternative output directory |
| Generated SQL contains a syntax error (e.g. malformed constraint) | Display the offending clause, explain the issue, and propose a corrected version before proceeding |
| Entity name conflicts with an existing table in the live schema | Warn user, suggest a disambiguated name, and ask whether to extend the existing table or create a new one |

## Post-conditions

After successful execution:
- Domain ubiquitous-language glossary captured (entities, value objects, aggregates, bounded contexts)
- Relational schema authored with explicit cardinalities, FKs, constraints, and indexes derived from access patterns
- RLS posture defined per table (tenant isolation, audit trail, or owner-based) with policy templates ready to materialize
- Generated SQL migration scaffolded in `supabase/migrations/{TS}_{entity}_*.sql` ready for `*verify-order` + `*dry-run`

Acceptance Criteria:
- [ ] Each modeled entity lists its identity field, owning aggregate, and at least one access pattern that justifies its indexes
- [ ] Every table proposed has an explicit RLS posture (tenant / audit / owner / public) recorded — no table left implicit
- [ ] Generated migration SQL passes `*verify-order` (Extensions → Tables → Functions → Triggers → RLS → Views)
