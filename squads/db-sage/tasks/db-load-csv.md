...
- **imported rows**: `{count}` rows persisted into `{table}` from `{csv_file}`
- **validation report**: console assertions for NULLs in required columns, duplicate keys, and data-type coverage
- **timing metric**: import duration in seconds
- **post-import hint**: console prompt to run `ANALYZE {table};` to refresh statistics
- **exit code**: `0` on success, non-zero if any validation or `COPY` fails
...
| CSV file not found at `{csv_file}` | Exit with `❌ File not found: {csv_file}` |
| Target table does not exist | Exit with `❌ Table '{table}' not found` |
| `\copy` fails with encoding error | Report `invalid byte sequence for encoding "UTF8"`; advise `iconv` conversion |
| `\copy` fails with unterminated quoted field | Report delimiter/quote mismatch; advise adjusting `DELIMITER`/`QUOTE` in COPY options |
| Validation finds NULL in required columns | Abort merge with `FAIL: NULL ids found`; leave data in staging for inspection |
| Validation finds duplicate keys | Abort merge with `FAIL: Duplicate ids found`; leave data in staging for inspection |
| `INSERT ... ON CONFLICT` fails with type cast error | Report cast failure with column and value; advise fixing data types in staging SELECT |
| User responds `no` to confirmation prompt | Exit cleanly with `Aborted`; staging table is dropped automatically (TEMP) |

## Post-conditions

After successful execution:
- TEMP staging table `{table}_staging` created with the same column structure as `{table}` and populated via `\copy`
- Validation gate executed (NULL-in-required, duplicate keys, data-type castability) before any write to the target table
- Rows merged into `{table}` via `INSERT ... ON CONFLICT (id) DO UPDATE` inside a single transaction (idempotent)
- Staging table dropped at the end of the session

Acceptance Criteria:
- [ ] Final row count in `{table}` matches `pre_count + new_rows` (or equals `pre_count` if all rows were UPDATE-on-conflict)
- [ ] If validation fails, no row is merged into `{table}` and the staging table is left intact for inspection
- [ ] The `\copy` source path is the local CSV (`{csv_file}`) — no server-side `COPY FROM '/absolute/path'` runs without explicit user opt-in
