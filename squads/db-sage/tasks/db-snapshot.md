...
- **snapshot file**: `supabase/snapshots/{TS}_{label}.sql` — full pg_dump used for rollback
- **metadata sidecar**: `supabase/snapshots/{TS}_{label}.meta` with label, timestamp, size, and content counts
- **summary metrics**: console report of file size and counts (tables / functions / policies captured)
- **restore command**: console hint `*rollback supabase/snapshots/{TS}_{label}.sql`
...
| Condition | Action |
|-----------|--------|
| `SUPABASE_DB_URL` not set | Exit with `❌ Snapshot failed` and instruct user to run `*env-check` |
| `pg_dump` command not found | Exit with error; advise installing PostgreSQL client tools |
| Database connection refused | Report `❌ Snapshot failed: connection failed`; prompt `*env-check` |
| `pg_dump` exits non-zero | Remove partially written file; exit with `❌ Snapshot failed` |
| Resulting snapshot file is empty (0 bytes) | Exit with `⚠️ Snapshot file is empty`; check DB has tables and connectivity |
| Insufficient disk space | Report OS error from `pg_dump`; advise clearing old snapshots |
| Permission denied creating `supabase/snapshots/` | Report filesystem error; ask user to check write permissions on project root |

## Post-conditions

After successful execution:
- Snapshot file persisted at `supabase/snapshots/{TS}_{label}.sql` via `pg_dump --schema-only --clean --if-exists --no-owner --no-privileges`
- Metadata sidecar persisted at `supabase/snapshots/{TS}_{label}.meta` with timestamp, label, redacted DB URL, and restore command
- Snapshot validated as non-empty and reporting counts of tables / functions / policies

Acceptance Criteria:
- [ ] Snapshot file exists, is non-empty, and contains at least one `CREATE TABLE` or `CREATE FUNCTION` statement
- [ ] Metadata file exists at the matching `_{label}.meta` path with no full connection string leaked (URL must be redacted)
- [ ] `pg_dump` exit code is 0 (partial dumps are deleted, not left on disk)
