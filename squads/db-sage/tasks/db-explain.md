...
- **execution plan**: `EXPLAIN (ANALYZE, BUFFERS, VERBOSE)` output for `{sql}` printed to console
- **timing metrics**: planning time, execution time, and total time in milliseconds
- **buffers report**: shared hit / shared read / temp read+written counts (cache vs disk vs temp file usage)
- **row-estimate accuracy**: estimated vs actual row counts per plan node
- **node-type classification**: Seq Scan / Index Scan / Nested Loop / Hash Join detected with cost values
...
| Condition | Action |
|-----------|--------|
| `SUPABASE_DB_URL` not set | Exit with connection error; prompt user to set the variable |
| Database connection refused | Report connection failure and stop |
| `EXPLAIN ANALYZE` on a write query (INSERT/UPDATE/DELETE) without transaction wrapper | Warn `⚠️ EXPLAIN ANALYZE on write query will execute it — wrap in BEGIN/ROLLBACK` |
| Query syntax error | Print PostgreSQL error verbatim; do not interpret |
| `pg_stat_statements` not available for slow-query lookup | Note extension is disabled; recommend `CREATE EXTENSION pg_stat_statements` |
| Execution time exceeds 30s | Warn that query may be too expensive for interactive EXPLAIN; suggest using `--format json` and offline analysis tools |

## Post-conditions

After successful execution:
- `EXPLAIN (ANALYZE, BUFFERS, VERBOSE, FORMAT TEXT)` executed against the user-supplied `{sql}`
- Execution time, planning time, total cost, and buffer breakdown (shared hit / read / temp) extracted and presented
- Detected anti-patterns (Seq Scan on large tables, missing FK indexes, temp-file sorts, row-estimate skew, slow RLS) surfaced with concrete remediation SQL when applicable
- Write queries explicitly wrapped or rejected (no unintended INSERT/UPDATE/DELETE side effects)

Acceptance Criteria:
- [ ] Output report includes Execution Time, Planning Time, and Buffer counts as discrete metrics (not just raw EXPLAIN dump)
- [ ] For any Seq Scan on a table >10MB, output emits a `CREATE INDEX` recommendation tied to the filtered column
- [ ] Write queries are either wrapped in `BEGIN/ROLLBACK` automatically or refused with the warning specified in Error Handling
