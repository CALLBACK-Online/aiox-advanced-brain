...
- **execution result**: `{count}` rows affected from the executed SQL (`file` or `inline` mode)
- **transaction mode**: `auto` / `manual` / `read` flag recorded in the summary
- **captured output**: SQL stdout/stderr captured at `/tmp/dbsage_sql_output.txt`
- **duration metric**: total execution time in milliseconds
- **exit code**: `0` on success; non-zero (and rolled-back transaction) on any error
...
- SQL executed under the user-selected transaction mode (`auto` BEGIN/COMMIT, `manual` no wrapper, `read` BEGIN TRANSACTION READ ONLY)
- Destructive operations (`DROP TABLE`, `TRUNCATE`, `WHERE 1=1`) gated behind `I UNDERSTAND THE RISKS` confirmation
- Full stdout/stderr captured at `/tmp/dbsage_sql_output.txt` for audit
- Rows-affected and execution time extracted from the captured output when available

Acceptance Criteria:
- [ ] In `auto` or `read` modes, any error triggers a transaction rollback — final DB state matches pre-execution state
- [ ] In `read` mode, no INSERT/UPDATE/DELETE/DDL statement succeeds (PostgreSQL enforces read-only)
- [ ] The execution summary surfaces exit code, mode, rows affected, and duration in the final report
