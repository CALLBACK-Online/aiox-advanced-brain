...
- Target schema/tables identified
...
- **RLS status table**: per-table status (`✓ ENABLED` / `❌ DISABLED`) for every public table
- **policy coverage table**: per-table policy count + operations covered (SELECT / INSERT / UPDATE / DELETE)
- **incomplete-coverage list**: tables with RLS enabled but zero policies (full lockout) or partial granular coverage
- **remediation hints**: console suggestions to run `*policy-apply {table} kiss` for tables without policies
- **exit code**: `0` on successful audit, non-zero if database queries fail
...
| Condition | Action |
|-----------|--------|
| `SUPABASE_DB_URL` not set | Exit with `❌ Missing SUPABASE_DB_URL or DATABASE_URL` |
| Database connection refused | Report connection error and prompt user to run `*env-check` |
| `pg_policies` query returns no rows | Warn that no policies exist — does not block audit, report as security risk |
| Table with RLS enabled but zero policies | Flag as `CRITICAL` in output — table is completely inaccessible |
| Permission denied on `pg_tables` | Report insufficient privileges; advise using a role with `pg_read_all_settings` |

## Post-conditions

After successful execution:
- Every public-schema table classified as `✓ ENABLED` or `❌ DISABLED` for RLS
- Policy coverage per table reported (count + operations: SELECT/INSERT/UPDATE/DELETE)
- Tables with RLS enabled but zero policies flagged as CRITICAL (full lockout)
- Remediation hints emitted for `❌ DISABLED` tables (`*policy-apply {table} kiss`)

Acceptance Criteria:
- [ ] Output lists 100% of public-schema tables (no table omitted from the audit)
- [ ] Summary totals (`total_tables`, `rls_enabled`, `rls_disabled`) sum to the same `total_tables` value
- [ ] Every table flagged with `policy_count = 0` is explicitly marked CRITICAL in the report
