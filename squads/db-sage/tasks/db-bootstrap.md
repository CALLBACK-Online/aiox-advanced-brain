...
- `project_name` (string): Name of the Supabase project (e.g., "my-project")
- `profile` (enum: `minimal` | `standard` | `full`): Scaffold profile — minimal creates directories only, standard adds READMEs and config, full adds baseline migration and smoke test
...
- **directory tree**: `supabase/{migrations,seeds,tests,rollback,snapshots,docs}/` created at repo root
- **README files**: per-directory READMEs documenting naming conventions and workflow (profiles `standard` + `full`)
- **baseline migration**: example migration scaffolded under `supabase/migrations/` (profile `full` only)
- **baseline smoke test**: example smoke-test SQL under `supabase/tests/` (profile `full` only)
- **profile flag echo**: chosen profile (`minimal` / `standard` / `full`) and `{project_name}` echoed in console summary
...
| Condition | Action |
|-----------|--------|
| Target directory already exists | Warn user, offer merge or abort |
| Insufficient file permissions | Check project root with write access |
| Config conflicts with existing Supabase | Keep existing config, use bootstrap for organization only |
| No database connection available | Skip live schema detection, scaffold empty structure |
| pg_dump not available | Skip initial snapshot, warn user to install PostgreSQL tools |

## Post-conditions

After successful execution:
- Canonical `supabase/` skeleton present: `migrations/`, `seeds/`, `tests/`, `rollback/`, `snapshots/`, `docs/`
- Profile-specific files materialized (READMEs + `config.toml` for standard; baseline migration + smoke test for full)
- `.gitignore` written under `supabase/` excluding env, branches, temp, OS junk
- Initial `supabase/docs/migration-log.md` created with baseline entry

Acceptance Criteria:
- [ ] All six required directories exist directly under `supabase/`
- [ ] Profile contract honored: `minimal` creates only directories; `standard` adds READMEs + `config.toml`; `full` adds `00000000000000_baseline.sql` and `tests/smoke_test.sql`
- [ ] No file overwrites a pre-existing user file silently (existing `supabase/` triggers explicit merge-or-abort prompt)
