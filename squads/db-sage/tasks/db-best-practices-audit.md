...
- `db_url` (string, optional): Database connection URL — defaults to `$SUPABASE_DB_URL` or `$DATABASE_URL` environment variable
...
- **audit score**: weighted score out of 100 across 7 categories (Connection 15%, FK Indexes 15%, RLS 20%, Schema 15%, Query Perf 15%, Indexes 10%, Vacuum 10%)
- **issue classification**: list of findings tagged CRITICAL / HIGH / MEDIUM with remediation context
- **SQL fix snippets**: copy-pasteable `CREATE INDEX`, `ENABLE ROW LEVEL SECURITY`, `VACUUM` statements generated from audit findings
- **audit-trail report**: pgaudit and grants status summary printed to console
- **exit code**: `0` on success, non-zero if any audit query fails
...
| Database connection refused | Report connection error and stop; do not emit partial scorecard |
| `pg_stat_statements` extension not installed | Record as FAIL for Query Performance section; note in scorecard as HIGH severity |
| Permission denied on system catalog (`pg_stat_activity`, `pg_stat_statements`) | Skip that section, mark as `N/A (insufficient privileges)` |
| `pg_relation_size` returns error for dropped/temp tables | Skip affected rows silently and continue audit |
| Score below critical threshold (< 40%) | Append `⚠️ CRITICAL: Score below acceptable threshold — review CRITICAL items immediately` to output |

## Post-conditions

After successful execution:
- All 8 audit sections executed (connection, FK indexes, RLS, schema, query perf, indexes, vacuum, audit trail) or marked `N/A` with reason
- Weighted scorecard computed across 28 checks mapped to 30 Supabase Agent Skills rules
- Issues classified into CRITICAL / HIGH / MEDIUM severity buckets with remediation SQL templates
- Output captured in a single audit transcript ready to attach to a story or maintenance log

Acceptance Criteria:
- [ ] Final output contains the score table (Category / Weight / Checks / Passed / Score) with all 7 weighted categories populated
- [ ] Every FK-without-index, table-without-RLS, and seq-scan-on-large-table is listed individually (not just counted)
- [ ] On scores below 40%, output explicitly displays the CRITICAL banner described in Error Handling
