...
- `squad_name` (string): Name of the target squad to analyze (e.g. `course-creator`, `innerlens`); used to locate `squads/{squad_name}/config.yaml`
- `include_schema` (boolean, optional): When `true`, runs `squad-integration-preflight.sh --include-schema` to capture a live schema baseline before design; defaults to `false`
- `db_url` (string, optional): Database connection string for live schema inspection; falls back to `SUPABASE_DB_URL` / `DATABASE_URL` env vars
...
- **database-integration-report**: `squads/{pack-name}/database-integration-report.yaml` — main consolidated report
- **data-flow audit**: `squads/{pack-name}/data-flow-audit.yaml` — findings from step 2
- **schema design**: `squads/{pack-name}/schema-design.yaml` — proposed schema from step 4
- **integration guide**: `docs/{pack-name}/database-integration.md` — usage documentation
- **schema definition**: `docs/{pack-name}/database-schema.yaml` — canonical schema reference
- **migration plan**: `docs/{pack-name}/migration-plan.yaml` — phased migration strategy with rollback
- **migration files**: `supabase/migrations/2025MMDD_NNN_{squad}_*.sql` — ready to apply via `*apply-migration`
...
| Condition | Action |
|-----------|--------|
| `squads/{squad_name}/config.yaml` not found | Halt and inform user the squad does not exist; list available squads under `squads/` |
| `squad-integration-preflight.sh` fails or DB connection unavailable | Skip live schema step; proceed with manual schema review (Step 3); mark schema baseline as unavailable in the report |
| KISS Gate validation fails (database not justified) | Halt the integration design and present the KISS Gate findings to the user; recommend filesystem-only approach |
| Proposed table name conflicts with an existing table | Warn user, propose a namespaced alternative (e.g. `{squad_prefix}_{table}`), and ask for confirmation before proceeding |
| Migration file generation fails (output directory missing) | Create `supabase/migrations/` directory if absent; if still failing, prompt user for an alternative path |
| `vector` extension not available in target DB (needed for embeddings) | Remove the `embedding` column from schema proposal and note it as an optional upgrade requiring `pgvector` extension |

## Post-conditions

After successful execution:
- Squad data flows inventoried (inputs / outputs / state / relationships) into `squads/{squad_name}/data-flow-audit.yaml`
- Schema proposal authored at `squads/{squad_name}/schema-design.yaml` with RLS, indexes, FKs, and enums named
- KISS Gate verdict captured (database justified / not justified) before any migration file is scaffolded
- Migration plan + ready-to-apply SQL produced at `docs/{squad_name}/migration-plan.yaml` and `supabase/migrations/{TS}_{squad}_*.sql`
- Consolidated integration report persisted at `squads/{squad_name}/database-integration-report.yaml`

Acceptance Criteria:
- [ ] Every proposed table includes an explicit RLS policy and indexes covering the documented access patterns
- [ ] KISS Gate section in the report explicitly answers "is the database needed?" with rationale and existing-table alternatives considered
- [ ] No migration file is scaffolded if KISS Gate verdict is "database not justified"
