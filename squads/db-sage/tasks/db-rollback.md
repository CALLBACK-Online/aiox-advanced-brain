...
- **emergency snapshot**: `supabase/snapshots/{TS}_emergency_before_rollback.sql` captured before applying rollback
- **rollback applied**: schema restored from `{target}` against the configured database (advisory-locked)
- **post-rollback verification**: console echo of table/policy/function counts after restore
- **status flag**: success / failure echoed; on failure the emergency snapshot path is surfaced for recovery
- **lock release**: `pg_advisory_unlock` confirmed on successful or failed completion
...
| Condition | Action |
|-----------|--------|
| Emergency snapshot creation fails before rollback | Abort rollback immediately; display error and disk/connectivity diagnostics; do not proceed |
| Rollback target file not found | Display error with expected path; list available snapshots in `supabase/snapshots/` and rollback scripts in `supabase/rollback/` |
| Target file does not contain valid SQL (`CREATE`/`DROP`/`ALTER` absent) | Reject target with explanation; ask user to verify the correct file |
| `pg_try_advisory_lock` returns false (another operation in progress) | Halt with message "Another DB Sage operation is running — retry after it completes" |
| `psql -f "$TARGET"` exits non-zero mid-rollback | Attempt restore from emergency snapshot; if that also fails, display "DATABASE MAY BE INCONSISTENT — manual intervention required" with the emergency snapshot path |
| Post-rollback validation shows 0 tables | Treat as critical failure; attempt emergency snapshot restore and alert user |

## Post-conditions

After successful execution:
- Emergency snapshot created at `supabase/snapshots/{TS}_emergency_before_rollback.sql` before any destructive action
- Advisory lock `hashtext('dbsage:rollback')` acquired and released
- Target rollback script (snapshot or rollback SQL) applied; schema restored to target state
- Post-rollback snapshot persisted at `supabase/snapshots/{TS}_post_rollback.sql`
- Rollback event appended to `supabase/rollback/rollback.log` with target and duration

Acceptance Criteria:
- [ ] `_emergency_before_rollback.sql` exists and is non-empty before the destructive `psql -f` step runs
- [ ] Post-rollback validation reports `tables > 0`, `functions > 0` (no zero-object database state)
- [ ] User explicitly typed `ROLLBACK` to confirm before the destructive step (interactive confirmation logged)
