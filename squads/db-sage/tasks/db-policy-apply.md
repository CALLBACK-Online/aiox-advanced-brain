...
- **RLS migration file**: `supabase/migrations/{TS}_rls_{mode}__{table}.sql` containing the policy(ies) applied
- **policies created**: list of policy names installed on `{table}` (single FOR ALL or per-operation set)
- **mode flag**: `kiss` or `granular` recorded in the migration filename + summary
- **next-step prompts**: console hints to run `*impersonate`, `*rls-audit`, commit migration
...
- RLS enabled on `{table}` via `ALTER TABLE {table} ENABLE ROW LEVEL SECURITY`
- Policies installed per mode: `kiss` = single `FOR ALL` policy; `granular` = one policy per SELECT/INSERT/UPDATE/DELETE
- All `auth.uid()` calls wrapped in `(select auth.uid())` for cached-per-statement performance
- Migration file persisted at `supabase/migrations/{TS}_rls_{mode}__{table}.sql` and applied via the apply-migration pipeline

Acceptance Criteria:
- [ ] `pg_class.relrowsecurity` for `{table}` returns `true` after task completion
- [ ] `pg_policies` shows exactly the expected policy set for `{table}` (1 policy for kiss, 4 policies for granular)
- [ ] No policy references `raw_user_meta_data` (client-mutable); only `auth.uid()` or `app_metadata`-based predicates are allowed
