...
- **JWT claims context**: session-scoped `request.jwt.claims` set to `{user_id}` with `role=authenticated`
- **active psql session**: interactive psql session opened with impersonated context for ad-hoc testing
- **verification echo**: `auth.uid()` and `current_setting('role')` returned to confirm impersonation is active
- **reset instructions**: console hint to run `RESET ALL;` (or close session) to drop impersonation
...
| Condition | Action |
|-----------|--------|
| `SUPABASE_DB_URL` not set | Exit with connection error; advise user to configure `SUPABASE_DB_URL` |
| Database connection refused | Report connection failure and stop |
| `user_id` not a valid UUID format | Report `❌ Invalid user_id format — expected UUID (e.g., xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx)` |
| `auth.uid()` returns NULL after setting claims | Report `❌ Claims not applied correctly`; show current `request.jwt.claims` for diagnosis |
| User is still seeing all data (RLS not enforcing) | Warn `⚠️ Verify RLS is enabled and connection is not using service role key` |
| `set_config` permission denied | Report that the current role lacks permission to set session parameters |

## Post-conditions

After successful execution:
- Session-local `request.jwt.claims`, `request.jwt.claim.sub`, and `role` set to impersonate `{user_id}` as `authenticated`
- `auth.uid()` confirmed to return the impersonated `{user_id}` after configuration
- Settings remain session-scoped (transaction-local) and never persist beyond the psql session

Acceptance Criteria:
- [ ] After setup, `SELECT auth.uid()` returns exactly `{user_id}` for the active session
- [ ] No production application code path is modified — only session/transaction-scoped settings via `set_config(..., true)` (the `true` `is_local` flag is mandatory)
- [ ] User is reminded that `RESET ALL;` or session close exits impersonation
