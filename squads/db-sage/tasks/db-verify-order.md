...
- **DDL inventory**: per-section counts for Extensions / Tables / Functions / Triggers / RLS / Views extracted from `{path}`
- **ordering verdict**: PASS / FAIL flag with offending pattern (e.g. "Functions before tables", "RLS before tables")
- **section ordering report**: `/tmp/ddl_order.txt` with line numbers and category for each DDL statement
- **next-step hint**: console suggestion to run `*dry-run` and `*apply-migration` when PASS
- **exit code**: `0` on PASS, `2` on heuristic violations, non-zero on parse failure
...
| Condition | Action |
|-----------|--------|
| Migration file not found at `{path}` | Exit with `❌ File not found: {path}` |
| File is empty or contains no DDL statements | Warn `⚠️ No DDL statements detected` and exit without error |
| Functions detected before tables | Exit with code 2 and message `❌ Functions appear before tables. Reorder recommended.` |
| RLS commands detected before table creation | Exit with code 2 and message `❌ RLS commands before table creation. Reorder required.` |
| Triggers detected before their functions | Emit warning `⚠️ Triggers before functions. May fail if trigger calls function.`; do not block |
| `awk` not available on the system | Fall back to manual review instructions; do not silently skip |

## Post-conditions

After successful execution:
- DDL sections (Extensions / Tables / Functions / Triggers / RLS / Views) extracted from `{path}` with line numbers
- Recommended order (1. Extensions → 2. Tables → 3. Functions → 4. Triggers → 5. RLS → 6. Views) compared against actual order
- Heuristic checks executed: functions-before-tables, RLS-before-tables, triggers-before-functions
- Verdict emitted: PASS (exit 0), WARN (exit 0 with warning), or FAIL (exit 2)

Acceptance Criteria:
- [ ] Output prints both the recommended order and the actual order present in `{path}`
- [ ] Exit code is non-zero exactly when a blocking ordering violation is detected (functions-before-tables or RLS-before-tables)
- [ ] No database connection is opened (this is a static lint — runs offline)
