...
- **hot-path report**: `supabase/docs/performance-analysis-{timestamp}.md` — markdown report with queries analyzed, findings, and recommended actions
- **index recommendations**: list of `CREATE INDEX` statements ready to be turned into a migration
- **analysis summary**: console summary with `{count}` queries analyzed, key findings, and next-step commands
- **exit code**: `0` on successful analysis, non-zero if database queries fail
...
| Condition | Action |
|-----------|--------|
| `pg_stat_statements` extension not enabled | Run `CREATE EXTENSION IF NOT EXISTS pg_stat_statements;` in Dashboard SQL Editor, then retry |
| No rows returned from `pg_stat_statements` | Ensure sufficient database activity has occurred; run representative workload first |
| `index_advisor` extension unavailable | Feature requires Supabase Pro+; skip Step 4 and proceed with manual index analysis |
| EXPLAIN ANALYZE times out or hangs | Cancel query, reduce scope to a specific table or shorter time window |
| `index_advisor` returns no recommendations | Query may already be optimal; verify statistics are up to date with `ANALYZE {table}` |
| Report file directory does not exist | Run `mkdir -p supabase/docs` before generating the report |
| Database connection fails | Run `*env-check` to validate `SUPABASE_DB_URL` and connectivity |

## Post-conditions

After successful execution:
- `pg_stat_statements` (and `index_advisor` when available) confirmed enabled before query analysis
- EXPLAIN (ANALYZE, BUFFERS, VERBOSE, COSTS, TIMING) executed on the selected hot queries
- Performance report persisted at `supabase/docs/performance-analysis-{timestamp}.md` with findings, BUFFERS interpretation, and recommended `CREATE INDEX` statements
- Index recommendations surfaced (from `index_advisor` or manual heuristics) ready to convert into a migration

Acceptance Criteria:
- [ ] Report file exists at `supabase/docs/performance-analysis-{timestamp}.md` and lists every analyzed query with its EXPLAIN output
- [ ] At least one of: actionable index recommendation, statistics refresh advice, or explicit "no optimizations identified" verdict per analyzed query
- [ ] No write/DML statement issued against the database (analysis is read-only)
