...
No layer conflicts with another. The apparent tension between shadcn "copy" and `@synkraai/ds-core` "reference" is resolved by recognizing they operate at different tiers.

## Consistency Model by Deliverable Kind

| Deliverable Kind | Model | Bundler | Token Strategy | Enforcement |
|------------------|-------|---------|----------------|-------------|
| `component` | **reference-first** via `@synkraai/ds-core` | SWC / Next.js | CSS vars via `@theme inline` | TypeScript + ESLint + token validator |
| `page` | **reference-first** via `@synkraai/ds-core` | SWC / Next.js | CSS vars | TypeScript + ESLint |
| `email` | **reference-first** + email-safe subset | SWC / MJML | inline styles (email constraint) | email client matrix + MJML validator |
| `dashboard` | **reference-first** via `@synkraai/ds-core` | SWC / Next.js | CSS vars | TypeScript + ESLint + RLS tests |
...
- External references:
  - `squads/design-ops/rules/design-system-generation.md` (`@synkraai/ds-core` registry)
