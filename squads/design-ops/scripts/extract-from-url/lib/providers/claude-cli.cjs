...
function invoke(promptText, options = {}) {
  ...
  // maxBuffer: Node's spawnSync default is 1MB. Long multi-turn sessions with
  // large cache_read context can emit JSON stdout exceeding that, which would
  // kill the subprocess mid-fetch and surface as a truncated/garbled result.
  // Default to 64MB (parity with codex-cli.cjs / scaffold-edit.cjs); override
  // via DESIGN_MD_MAX_BUFFER. Preventive hardening (diagnose-socket-closed-error
  // workflow, 2026-05-29).
  const envMaxBuffer = parseInt(process.env.DESIGN_MD_MAX_BUFFER || "", 10);
  const maxBufferDefault = Number.isFinite(envMaxBuffer) && envMaxBuffer > 0
    ? envMaxBuffer
    : 64 * 1024 * 1024;
  const { maxTurns = maxTurnsDefault, timeoutMs = timeoutDefault, maxBuffer = maxBufferDefault, cwd, model, designMdPath } = options;
