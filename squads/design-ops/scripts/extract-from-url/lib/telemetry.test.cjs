...
// ── parseClaudeCliStdout surfaces in-band API errors (socket-closed) ──
// Regression guard for diagnose-socket-closed-error workflow (2026-05-29):
// the claude CLI can exit 0 while the result JSON carries is_error:true with
// api_error_status:null — a server-side socket drop reported in-band. The
// pipeline relies on these fields to engage the transient-retry path instead
// of proceeding with a half-written DESIGN.md.

test("parseClaudeCliStdout: surfaces is_error/api_error_status/result_text on socket-closed", () => {
  ...
  const m = parseClaudeCliStdout(body);
  assert.equal(m.is_error, true);
  assert.equal(m.api_error_status, null);
  assert.ok(m.result_text.startsWith("API Error: The socket connection was closed"));
});

test("parseClaudeCliStdout: clean success has is_error false and null api_error_status", () => {
  const body = JSON.stringify({
    type: "result",
    subtype: "success",
    is_error: false,
...
test("parseClaudeCliStdout: empty stdout returns is_error false defaults", () => {
  const m = parseClaudeCliStdout("");
  assert.equal(m.is_error, false);
  assert.equal(m.api_error_status, null);
  assert.equal(m.result_text, null);
});

test("parseClaudeCliStdout: captures non-null api_error_status (5xx)", () => {
  const body = JSON.stringify({
    type: "result",
    is_error: true,
  ...
  const m = parseClaudeCliStdout(body);
  assert.equal(m.is_error, true);
  assert.equal(m.api_error_status, 529);
});
