...
const {
  ...
  computeQualityScore, buildExtractionLog, validateConsumerContract,
  appendCustomLintFindings, classifyLintResult, formatLintSummary,
...
test("computeQualityScore recognizes proprietary namespaced CSS variable systems", () => {
  const tokens = {
    colors: { primary: "#000000", surface: "#ffffff", text: "#111111" },
    typography: { h1: { fontFamily: "Inter", fontSize: "48px", fontWeight: 700 }, "body-md": { fontFamily: "Inter", fontSize: "16px" }, h2: {}, h3: {} },
    rounded: { sm: "4px", md: "8px", lg: "12px" },
  ...
  const log = { confidence_summary: { high: 1, medium: 12, low: 8 } };
  const lint = { ran: true, errors_count: 0, warnings_count: 0 };
  const cssVars = Array.from({ length: 24 }, (_, i) => ({
    name: `--ids_color_token_${i}`,
    value: i % 2 === 0 ? "#111111" : "var(--ids_color_base)",
    is_alias: i % 2 !== 0,
  }));
  const result = computeQualityScore(tokens, log, lint, cssVars, []);
  assert.ok(result.categories.tokenization.score >= 80);
  assert.match(result.categories.tokenization.value, /--ids\*/);
});
...
test("computeQualityScore caps promotion grade when lint or accessibility block", () => {
  const tokens = {
    colors: { primary: "#777777", surface: "#777777", text: "#777777" },
    typography: { h1: { fontFamily: "Inter" }, h2: { fontFamily: "Inter" }, h3: { fontFamily: "Inter" }, "body-md": { fontFamily: "Inter" } },
    rounded: { sm: "4px", md: "8px", lg: "12px" },
    spacing: { xs: "4px", sm: "8px", md: "16px", lg: "24px" },
  };
  const log = { confidence_summary: { high: 10, medium: 0, low: 0 } };
  ...
  assert.equal(result.categories.accessibility.grade, "F");
  assert.equal(result.gates.promotion_ready, false);
  assert.ok(result.gates.blockers.includes("lint_execution_failed"));
  assert.ok(result.gates.blockers.includes("accessibility_below_aa_floor"));
...
test("validateConsumerContract requires golden-standard downstream contract", () => {
  const missing = validateConsumerContract(`---
name: Test
colors:
  primary: "#000000"
---
...
## 1. Visual Theme & Atmosphere`);
  ...
  assert.match(missing.errors[0].message, /Missing required consumer_contract/);

  const valid = validateConsumerContract(`---
name: Test
consumer_contract:
  standalone: true
  goal: "Generate Test-style interfaces from this file alone."
  priority_order:
    - "Use semantic tokens first."
    - "Apply component recipes before inventing variants."
    - "Respect Do's and Don'ts over generic defaults."
  mode_rule: "Light default."
  color_rule: "Black is identity; blue is support only."
  font_rule: "Use Inter with system fallback."
  asset_rule: "Do not require proprietary assets."
  accessibility_rule: "Ship WCAG AA contrast."
colors:
  primary: "#000000"
---
...
test("appendCustomLintFindings merges custom contract errors into lint counts", () => {
  ...
  assert.equal(merged.warnings_count, 1);
  assert.equal(merged.custom.consumer_contract.errors, 1);
});
