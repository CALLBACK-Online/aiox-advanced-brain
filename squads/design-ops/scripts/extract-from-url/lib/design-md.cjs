...
function buildExtractionLog({ url, designMd, tokens, cssVars, fontFaces, usageGraph, cssMeta, lintResult, artifactType = null, pageRole = null }) {
  const log = {
    url,
    extracted_at: new Date().toISOString(),
    extracted_artifact_type: artifactType || "tokens_only",
...
function scoreNativeTokenNamespaces(cssVars = []) {
  ...
  const strongest = namespaces[0] || null;
  const total = cssVars.length || 0;
  const primitive = cssVars.filter((entry) => !entry.is_alias).length;
  const alias = total - primitive;
  const strongestShare = strongest && total > 0 ? strongest.count / total : 0;

  let scoreValue = 40;
  if (strongest?.count >= 50 && strongestShare >= 0.25) scoreValue = 95;
  else if (strongest?.count >= 25 && strongestShare >= 0.2) scoreValue = 88;
  else if (strongest?.count >= 10 && strongestShare >= 0.15) scoreValue = 82;
  else if (total >= 20) scoreValue = 72;
  else if (total >= 10) scoreValue = 62;

  if (primitive >= 10 && alias >= 5) scoreValue = Math.min(100, scoreValue + 5);

  return {
    score: scoreValue,
    strongest,
    namespaces: namespaces.slice(0, 6),
    total,
    primitive,
    alias,
  };
...
function classifyLintResult(parsed, exit_code, stdout, stderr) {
  if (parsed) {
    ...
    const executionError = exit_code !== 0 && errors.length === 0 && warnings.length === 0;
    return {
      ran: true,
      exit_code,
      errors_count: executionError ? -1 : errors.length,
      warnings_count: warnings.length,
      errors,
      warnings,
      raw: parsed,
      ...(executionError ? lintFailureMetadata(stdout, stderr, exit_code) : {}),
    };
  }
  const errCount = (stdout.match(/\b(\d+)\s+error/i) || [])[1];
  const warnCount = (stdout.match(/\b(\d+)\s+warning/i) || [])[1];
  const executionError = exit_code !== 0 && !errCount && !warnCount;
  return {
    ran: true,
    exit_code,
    errors_count: errCount ? parseInt(errCount, 10) : (executionError ? -1 : 0),
...
function lintFailureMetadata(stdout = "", stderr = "", exit_code = null) {
  ...
  return {
    ...
    failure_message: `lint command exited ${exit_code ?? "non-zero"} without structured findings`,
  };
}

function isLintExecutionFailure(lintResult) {
  if (!lintResult || lintResult.ran !== true) return false;
  if (lintResult.execution_error === true) return true;
  if (Number(lintResult.errors_count) < 0) return true;
  const exitCode = lintResult.exit_code;
  const hasExit = typeof exitCode === "number";
  const errors = Number(lintResult.errors_count) || 0;
  const warnings = Number(lintResult.warnings_count) || 0;
  return hasExit && exitCode !== 0 && errors === 0 && warnings === 0;
}

function formatLintSummary(lintResult) {
  if (!lintResult || lintResult.ran !== true) {
    return `skipped (${lintResult?.error || "not-run"})`;
  }
  if (isLintExecutionFailure(lintResult)) {
    const kind = lintResult.failure_kind || "execution-error";
    return `failed (${kind}, exit=${lintResult.exit_code ?? "unknown"})`;
  }
  return `${lintResult.errors_count || 0}E ${lintResult.warnings_count || 0}W`;
}
...
const CONSUMER_CONTRACT_REQUIRED_FIELDS = [
  "standalone",
  "goal",
  "priority_order",
  "mode_rule",
...
function validateConsumerContract(designMd) {
  const tokens = parseFrontmatter(designMd || "");
  const contract = tokens && tokens.consumer_contract;
  const errors = [];
  const warnings = [];
  const issue = (message) => ({
  ...
  if (!contract || typeof contract !== "object" || Array.isArray(contract)) {
    errors.push(issue("Missing required consumer_contract frontmatter block."));
    return { errors, warnings };
  }

  for (const field of CONSUMER_CONTRACT_REQUIRED_FIELDS) {
    if (!(field in contract)) {
      errors.push(issue(`consumer_contract.${field} is required.`));
    }
  }

  if (contract.standalone !== true) {
    errors.push(issue("consumer_contract.standalone must be true."));
  }
  if (!Array.isArray(contract.priority_order) || contract.priority_order.length < 3) {
    errors.push(issue("consumer_contract.priority_order must contain at least 3 normative rules."));
  }

  return { errors, warnings };
}

function appendCustomLintFindings(lintResult, customFindings) {
  const result = lintResult || { ran: true, exit_code: 0, errors_count: 0, warnings_count: 0, errors: [], warnings: [] };
  const errors = Array.isArray(result.errors) ? result.errors : [];
  const warnings = Array.isArray(result.warnings) ? result.warnings : [];
  const customErrors = customFindings?.errors || [];
  const customWarnings = customFindings?.warnings || [];

  result.errors = [...errors, ...customErrors];
  result.warnings = [...warnings, ...customWarnings];
  const baseErrors = Number(result.errors_count);
  const baseWarnings = Number(result.warnings_count);
  result.errors_count = Number.isFinite(baseErrors) && baseErrors < 0
    ? baseErrors
    : Math.max(0, Number.isFinite(baseErrors) ? baseErrors : 0) + customErrors.length;
  result.warnings_count = Math.max(0, Number.isFinite(baseWarnings) ? baseWarnings : 0) + customWarnings.length;
  if (customErrors.length || customWarnings.length) {
    result.custom = {
      ...(result.custom || {}),
      consumer_contract: {
        errors: customErrors.length,
        warnings: customWarnings.length,
      },
    };
  }
  return result;
...
function runLint(designMdPath, cwd) {
  ...
  try {
    ...
    const contractFindings = validateConsumerContract(original);
    const first = tryLint(designMdPath, cwd);
    if (!isModelBuildingBug(first)) {
      return appendCustomLintFindings(
        classifyLintResult(first.parsed, first.exit_code, first.stdout, first.stderr),
        contractFindings,
      );
    }
    ...
    try {
      ...
      return appendCustomLintFindings(classified, contractFindings);
    } finally {
...
function computeAccessibilityAudit(colors) {
  let aaaCount = 0;
  let aaCount = 0;
  let totalPairs = 0;
  const failingPairs = [];
  ...
  for (const [textKey, text] of textKeys) {
    for (const [bgKey, bg] of bgKeys) {
      if (!/^#[0-9a-f]{6}$/i.test(text) || !/^#[0-9a-f]{6}$/i.test(bg)) continue;
  ...
  return {
    score: totalPairs > 0 ? Math.round((aaCount / totalPairs) * 100) : 70,
    aa_pairs: aaCount,
    aaa_pairs: aaaCount,
    total_pairs: totalPairs,
    failing_pairs: failingPairs.slice(0, 12),
    checked_pairs: checkedPairs.slice(0, 24),
  };
}
...
function computeQualityScore(tokens, extractionLog, lintResult, cssVars, fontFaces) {
  ...
  const namespaceScore = scoreNativeTokenNamespaces(cssVars);
  const tokenizationScore = Math.max(40, tokenizationPct, namespaceScore.score);
  const tokenizationValue = namespaceScore.strongest
    ? `${Math.round(tokenizationPct)}% high confidence · ${namespaceScore.strongest.count} --${namespaceScore.strongest.namespace}* vars`
    : `${Math.round(tokenizationPct)}% high confidence`;

  const lintRan = lintResult && lintResult.ran === true;
  const lintExecutionFailed = isLintExecutionFailure(lintResult);
  const lintErrors = Number(lintResult?.errors_count);
  const lintWarnings = Number(lintResult?.warnings_count);
  let lintScore = 70;
  let lintValue = "skipped";
  if (lintExecutionFailed) {
    lintScore = 0;
    lintValue = `failed: ${lintResult.failure_kind || "execution-error"}`;
  } else if (lintRan) {
    lintScore = lintErrors > 0 ? 60 : (lintWarnings > 0 ? 90 : 100);
    lintValue = `${Number.isFinite(lintErrors) ? lintErrors : 0}E ${Number.isFinite(lintWarnings) ? lintWarnings : 0}W`;
  }

  const accessibility = computeAccessibilityAudit(colors);
  ...
  const categories = {
    ...
    tokenization: { score: Math.round(tokenizationScore), grade: score(tokenizationScore), value: tokenizationValue, ideal: "≥80% or ≥10 consistent namespaced CSS vars" },
    lint_compliance: {
      score: lintScore,
      grade: score(lintScore),
      value: lintValue,
      ideal: "0E 0W",
      ...(lintExecutionFailed ? { failure_kind: lintResult.failure_kind || "execution-error" } : {}),
    },
    accessibility: {
      score: a11yScore,
      grade: score(a11yScore),
      value: `${accessibility.aa_pairs}/${accessibility.total_pairs} pairs ≥AA`,
      ideal: "100% AA",
      failing_pairs: accessibility.failing_pairs,
      checked_pairs: accessibility.checked_pairs,
    },
  };
  ...
  const blockers = [];
  if (lintExecutionFailed) blockers.push("lint_execution_failed");
  if (a11yScore < 60) blockers.push("accessibility_below_aa_floor");
  const cappedOverall = blockers.length > 0 && average >= 80 ? 79 : average;
  return {
    overall: cappedOverall,
    average,
    grade: score(cappedOverall),
    categories,
    gates: {
      promotion_ready: blockers.length === 0,
      blockers,
      cap_applied: cappedOverall !== average,
    },
  };
}
...
module.exports = {
  ...
  appendCustomLintFindings,
  computeAccessibilityAudit,
  colorDistanceRgb,
  flattenAllColors,
  computeDrift,
  computeQualityScore,
  scoreNativeTokenNamespaces,
