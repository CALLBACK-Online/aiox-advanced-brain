...
function parseClaudeCliStdout(rawStdout) {
  ...
  for (const line of lines) {
    ...
    if (parsed.type === "result") {
      ...
      // Capture API-level error signals. The claude CLI can exit 0 (subprocess
      // success) while the JSON result carries is_error:true — e.g. a server-side
      // socket drop surfaced as {"is_error":true,"api_error_status":null,
      // "result":"API Error: The socket connection was closed unexpectedly..."}.
      // Without surfacing this, the pipeline proceeds with an incomplete DESIGN.md.
      if (parsed.is_error === true) is_error = true;
      if (parsed.api_error_status != null) api_error_status = parsed.api_error_status;
      if (typeof parsed.result === "string") result_text = parsed.result;
      if (parsed.usage) {
