...
OUTPUTS_DIR="$(resolve_outputs_root "$REPO_ROOT")/code-anatomist"
mkdir -p "$OUTPUTS_DIR"
...
show_help() {
  ...
  echo "  outputs/code-anatomist/{slug}/{version}/   ← each version is a separate run"
  echo "  outputs/code-anatomist/{slug}/latest       ← symlink to most recent"
  echo ""
  echo "Environment:"
...
"
    ...
    local raw_output
    raw_output=$(run_llm_prompt "$model" "$PHASE_PROMPT" "$phase_log" "phase-$phase_num") || true
