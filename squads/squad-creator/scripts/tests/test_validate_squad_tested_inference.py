...
RUNTIME_DIR = REPO_ROOT / ".aiox" / "squad-runtime"
...
def _create_test_squad(
    name: str,
    *,
    tested_value: str | None = None,
    create_direct_runtime_evidence: bool = False,
    create_contract_test: bool = False,
    ...
    tested_line = f"tested: {tested_value}\n" if tested_value is not None else ""
    config = f"""name: {name}
version: 1.0.0
...
"""
    ...
    if create_direct_runtime_evidence:
        runtime_dir.mkdir(parents=True, exist_ok=False)
        (runtime_dir / "run.json").write_text('{"status":"completed"}\n', encoding="utf-8")
    if create_contract_test:
        (squad_dir / "tests").mkdir(parents=True, exist_ok=False)
        (squad_dir / "tests" / "runtime-contract.cjs").write_text("// test\n", encoding="utf-8")

    return squad_dir, output_dir, runtime_dir
...
def test_declared_output_path_alone_does_not_infer_tested_status() -> None:
    squad_name = f"tmp-tested-output-{uuid.uuid4().hex[:8]}"
    squad_dir, output_dir, runtime_dir = _create_test_squad(squad_name)
    try:
        result = _run_validator(squad_name)
        payload = _extract_result_json(result.stdout)

        assert result.returncode != 2
        assert payload["production"]["score"] <= 2
    finally:
...
def test_tested_flag_is_inferred_from_direct_runtime_evidence() -> None:
    squad_name = f"tmp-tested-runtime-{uuid.uuid4().hex[:8]}"
    squad_dir, output_dir, runtime_dir = _create_test_squad(
        squad_name,
        create_direct_runtime_evidence=True,
    )
    try:
        result = _run_validator(squad_name)
        payload = _extract_result_json(result.stdout)
        ...
        assert result.returncode != 2
        assert payload["production"]["score"] >= 3
    finally:
        shutil.rmtree(squad_dir, ignore_errors=True)
        shutil.rmtree(output_dir.parent.parent, ignore_errors=True)
        shutil.rmtree(runtime_dir, ignore_errors=True)
...
def test_explicit_tested_false_is_not_auto_promoted() -> None:
    squad_name = f"tmp-tested-false-{uuid.uuid4().hex[:8]}"
    squad_dir, output_dir, runtime_dir = _create_test_squad(
        squad_name,
        tested_value="false",
        create_direct_runtime_evidence=True,
...
if __name__ == "__main__":
    test_declared_output_path_alone_does_not_infer_tested_status()
    test_tested_flag_is_inferred_from_direct_runtime_evidence()
    test_explicit_tested_false_is_not_auto_promoted()
