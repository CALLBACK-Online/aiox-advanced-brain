...
def convert_md_to_claude_skill(content: str, agent_name: str, is_entry_point: bool = True) -> str:
    ...
    For entry_point agents (is_entry_point=True), injects activation_type: "pipeline"
    so hook-runtime.js triggers the UnifiedActivationPipeline deterministically.
    Relay/sub-agents (is_entry_point=False) are excluded per AC7 of Story 117.12.
    """
    ...
    # Story 117.12 AC1: Inject activation_type: "pipeline" for entry_point agents only.
    # This makes hook-runtime.js trigger UnifiedActivationPipeline deterministically.
    # Relay skills (sub-agents without entry_agent status) are excluded (AC7).
    if is_entry_point:
        frontmatter_lines.append('activation_type: "pipeline"')
...
def sync_file(
    ...
    is_entry_point: True (default) injects activation_type: "pipeline" in Claude SKILL.md.
    Set to False for relay/sub-agents to exclude activation_type injection (Story 117.12 AC7).
    """
    ...
    if ide == "cursor" and source.suffix == ".md":
        ...
        content = convert_md_to_claude_skill(content, source.name, is_entry_point=is_entry_point)
...
def sync_component(
    ...
    is_entry_point: when True (default), Claude SKILL.md gets activation_type: "pipeline".
    Set to False for relay/sub-agents (Story 117.12 AC7).
    """
    ...
    for ide in ides:
        ...
        status, message = sync_file(source, dest, ide, force, dry_run, is_entry_point=is_entry_point)
...
def sync_squad(
    ...
    for comp_dir, files in components.items():
        ...
        for source_file in files:
            ...
            # Story 117.12 AC7: Only entry_agent is_entry_point=True; relay agents False.
            is_this_entry_point = (not entry_agent) or (source_file.stem == entry_agent)
            ...
            result = sync_component(
                ...
                target_ide_override,
                verbose,
                is_entry_point=is_this_entry_point
            )
