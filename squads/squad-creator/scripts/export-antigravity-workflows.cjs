...
function collectRequestedAliases(targetRoot) {
  ...
  const claudeAgentAliases = fs.existsSync(path.join(targetRoot, '.claude', 'agents'))
    ...
    : [];
  const codexAliases = listDirNames(path.join(targetRoot, '.codex', 'skills')).map(resolveAlias).filter(Boolean);

  const requested = new Set(
    [...claudeSkillAliases, ...claudeAgentAliases, ...codexAliases].filter(
