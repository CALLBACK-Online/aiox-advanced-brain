...
_get_historical_scores() {
  local history_dir="$OUTPUTS_ROOT/squad-validations/${SQUAD_NAME:-unknown}"
...
extract_tested_flag_state() {
  if grep -qE '^tested:[[:space:]]*(true|yes|1)[[:space:]]*$' "$SQUAD_DIR/config.yaml" 2>/dev/null; then
    echo "true"
  elif grep -qE '^tested:[[:space:]]*(false|no|0)[[:space:]]*$' "$SQUAD_DIR/config.yaml" 2>/dev/null; then
    echo "false"
  elif grep -qE '^tested:[[:space:]]*' "$SQUAD_DIR/config.yaml" 2>/dev/null; then
    echo "present_other"
  else
    echo "absent"
  fi
...
check_structure() {
  ...
  # 1.5b: CC Format Detection (dual-format — EPIC-109 Wave 4)
  # CC format: flat frontmatter with top-level name: + description:
  # Legacy format: nested agent: block in frontmatter
  # During transition (180 days): WARNING only, not ERROR
  log_subsection "1.5b Agent/Task Format Detection (CC vs Legacy)"
  local legacy_count=0
  local cc_count=0
  local total_md_files=0

  if [[ -d "$SQUAD_DIR/agents" ]] || [[ -d "$SQUAD_DIR/tasks" ]]; then
    while IFS= read -r md_file; do
      total_md_files=$((total_md_files + 1))
      # CC format detection: top-level name: without nested agent: block
      if grep -qE "^name:" "$md_file" 2>/dev/null && ! grep -qE "^agent:" "$md_file" 2>/dev/null; then
        cc_count=$((cc_count + 1))
      else
        legacy_count=$((legacy_count + 1))
      fi
    done < <(find "$SQUAD_DIR/agents" "$SQUAD_DIR/tasks" -name "*.md" 2>/dev/null)
  fi

  if [[ "$total_md_files" -gt 0 ]]; then
    if [[ "$legacy_count" -gt 0 ]]; then
      # WARNING only — dual-format transition period (180 days per AD-6)
      log_warn "Format check: $legacy_count/$total_md_files files in legacy format. \
Consider migrating: node squads/squad-creator/scripts/migrate-to-cc-format.js --squad $SQUAD_DIR"
    else
      log_pass "Format check: all $cc_count files in CC format"
    fi
    if [[ "$cc_count" -gt 0 ]]; then
      log_info "CC format: $cc_count files | Legacy format: $legacy_count files"
    fi
  fi
...
check_production() {
  ...
  if [ -d "$runtime_root" ]; then
    local squad_runtime_dir="$runtime_root/$SQUAD_NAME"
    if [ -d "$squad_runtime_dir" ]; then
      local output_count
      output_count=$(find "$squad_runtime_dir" -type f 2>/dev/null | wc -l | tr -d ' ')
      if [ "$output_count" -gt 0 ]; then
        log_pass "Found canonical runtime evidence in .aiox/squad-runtime/$SQUAD_NAME ($output_count files)"
        has_runtime_evidence=true
        direct_runtime_evidence=true
        prod_score=$((prod_score + 2))
      else
        log_warn "Runtime directory exists but has no files: $squad_runtime_dir"
      fi
    else
      local indirect_runtime_dir=""
      indirect_runtime_dir=$(find "$runtime_root" -mindepth 2 -maxdepth 3 -type d -name "$SQUAD_NAME" 2>/dev/null | head -1)
      if [ -n "$indirect_runtime_dir" ] && [ -d "$indirect_runtime_dir" ]; then
        local indirect_count
        indirect_count=$(find "$indirect_runtime_dir" -type f 2>/dev/null | wc -l | tr -d ' ')
        if [ "$indirect_count" -gt 0 ]; then
          log_info "Found indirect runtime artifacts at $indirect_runtime_dir ($indirect_count files), but they do not count as canonical squad runtime evidence"
        fi
      fi
    fi
  fi
  ...
  # Honor explicit tested state first. Only infer when the flag is absent.
  local tested_state=""
  tested_state=$(extract_tested_flag_state)
  case "$tested_state" in
    true)
      ...
      ;;
    false)
      log_info "config.yaml explicitly sets tested: false; no tested credit is awarded"
      ;;
    present_other)
      log_warn "config.yaml has a tested flag with non-standard value; no tested credit is awarded"
      ;;
    *)
      local inferred_tested=false
      if [ "$direct_runtime_evidence" = true ]; then
        log_pass "config.yaml missing tested flag, but canonical runtime evidence is sufficient to infer tested status"
      ...
      if [ "$inferred_tested" = true ]; then
        ...
        log_warn "config.yaml missing tested flag and no canonical runtime or contract test evidence was found to infer it"
      fi
      ;;
  esac
  ...
  # Count only true examples/sample surfaces here; docs/stories are supportive but not production proof.
  ...
  # output_examples in agents help usability, but they are not production evidence.
  ...
  if [ "$output_examples" -gt 0 ]; then
    log_info "Found output examples in $output_examples agent files (documentation signal only)"
  fi

  if [ "$has_examples" = false ] && [ "$operational_story_files" -gt 0 ]; then
    log_info "Found $operational_story_files operational docs/stories for squad '$SQUAD_NAME' (supporting evidence only)"
  fi
...
SCOREEOF
  ...
  # Phase 3.8: AJV Schema Validation (C6 EPIC-109 Wave 3)
  # Only runs when schemas/ directory exists in the target squad.
  # validate-schemas.js: AJV invalid OVERRIDES LLM result (exit code 1 = fail).
  # Graceful degradation: if Node.js or AJV not available, skip with warning.
  runtime_state_phase_start "phase_3_8_ajv_schema_validation"
  local squad_schemas_dir="$SQUAD_DIR/schemas"
  if [ -d "$squad_schemas_dir" ]; then
    local validate_schemas_script="$SCRIPT_DIR/validate-schemas.js"
    if command -v node &>/dev/null && [ -f "$validate_schemas_script" ]; then
      log_subsection "3.8 AJV Schema Pre-check (C6 EPIC-109)"
      local ajv_exit_code=0
      node "$validate_schemas_script" "$SQUAD_DIR" --format text 2>&1 || ajv_exit_code=$?
      if [ "$ajv_exit_code" -ne 0 ]; then
        # AJV invalid OVERRIDES LLM — log as blocking failure
        log_fail "AJV schema validation FAILED (exit $ajv_exit_code). This overrides LLM result."
        TIER2_SCORE=0
        TIER2_PASS=false
      else
        log_pass "AJV schema validation PASSED"
      fi
    else
      log_info "AJV pre-check skipped: Node.js or validate-schemas.js not available (graceful degradation)"
    fi
  else
    log_info "No schemas/ directory in squad — AJV pre-check not applicable"
  fi
  runtime_state_phase_complete "phase_3_8_ajv_schema_validation"
