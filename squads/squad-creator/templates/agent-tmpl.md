# Agent Template — CC Format (EPIC-109 Wave 4)

> **Active format:** CC Format (Squad Protocol v2.0.0)
> Flat frontmatter + markdown prose body.
> Legacy format (YAML-heavy) kept below for reference during dual-format transition (180 days).
> Migration: `node squads/squad-creator/scripts/migrate-to-cc-format.js <file.md>`

## CC Format Template (DEFAULT — use this for new agents)

```markdown
---
name: {agent-name}
description: "{1-2 sentences: when to use this agent and what it does}"
model: claude-sonnet-4
tools: ["Agent", "Read", "Write", "Grep", "Glob", "Edit"]
# effort: high   # Uncomment for decision agents (chiefs, architects, QA)
---

# {Agent Name} — {Professional Title}

{Identity statement in prose. Who you are, what drives you, your primary style.
One or two sentences. This becomes the opening of the system prompt.}

## Scope

{What this agent is responsible for. Use bullet list for specific items.}
...
{Decision rules the agent applies consistently. These are the core intelligence.}

- {Rule 1: clear and actionable}
- {Rule 2: clear and actionable}
- {Rule 3: clear and actionable}

## Vocabulary (Expert agents only)
...
- `*{primary-command}` — {What this does}
- `*{secondary-command}` — {What this does}
- `*exit` — Exit agent

## Collaboration

Delegates to: `{agent-id}` when {condition}.
Hands off to: `{workflow}` for {scenario}.

## Anti-Patterns

- {What this agent must NEVER do}
- {Common mistake to avoid}
- {Scope violation to prevent}
```
...
agents_metadata:
  - id: {agent-name}
    icon: "{emoji}"
    version: "1.0.0"
    phase: planning   # planning | execution | validation
    tier: 0           # 0=orchestrator, 1=specialist, 2=worker

marketplace:
  archetype: "{archetype-name}"
  greeting_levels:
    brief: "{brief greeting}"
    full: "{full greeting}"
```
