# Task Template — CC Format (EPIC-109 Wave 4)

> **Active format:** CC Format (Squad Protocol v2.0.0)
> For new tasks, use CC format below.
> Legacy YAML-heavy format kept for reference during dual-format transition (180 days).

## CC Format Template (DEFAULT — use this for new tasks)
...
- `{field_name}` ({type}, required) — What this field contains
- `{field_name}` ({type}, optional) — Description

## Steps

1. {Concrete step 1 — verb + object}
2. {Concrete step 2}
3. {Apply {heuristic/pattern} to {produce output}}
...
- {Measurable criterion — verifiable yes/no}
...
# Legacy Template (reference only — dual-format transition)
