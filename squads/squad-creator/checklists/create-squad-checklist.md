...
pre_creation:
  ...
  - id: prior-art-search-documented
    check: "Prior-art search documented before any CREATE decision"
    type: blocking
    validation: "prior_art_search.status == PASS AND prior_art_search.search_vectors >= 2"
    required_rows:
      - claim
      - searches_performed
      - matches
...
creation_checks:
  ...
  - id: squad-io-created
    check: "SquadIO contract created"
    type: blocking
    validation: "squads/{squad_name}/squad-io.yaml exists with inputs, outputs, triggers, dependencies, artifact_contracts, data_sources, integrations, observability"
...
integration_checks:
  ...
  - id: pii-policy-declared
    check: "config.yaml declares identity_bound (boolean) per ADR-SQUAD-PII-POLICY"
    type: blocking
    validation: |
      If squad references real person_ids (Human:{id}, person emails, voice DNA),
      identity_bound MUST be true and identity_source array MUST list the people/minds.
      If squad is generic operational, identity_bound:false (default) and NO PII in any file.
    validator_script: "scripts/validate-squad-pii.js"
