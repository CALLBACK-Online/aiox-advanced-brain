...
Input: request::install_skills_finalize
Output: artifact::install_skills_finalize
pre_condition: install-skills-write completed com files escritos em IDEs detectadas
post_condition: instalação validada cross-IDE, índice de skills sincronizado e relatório final gerado
performance: deterministic Worker, < 30s, fail-loud se arquivos esperados ausentes ou inconsistência cross-IDE
Completion Criteria: all expected files verified AND cross-IDE consistency AND skill index synced AND install report emitido
>>>>>>>> origin/oalanicolas:squads/squad-creator/tasks/install-skills-finalize.md
