...
pre_condition: Phase 0 classification concluída (executor type atribuído), executor-matrix-framework.md disponível
post_condition: executor profile completo com guardrails, checkpoints, escalation paths e human review triggers
performance: < 5 min (Agent), structured output, escalate se executor type ambíguo
Completion Criteria: executor_profile definido AND guardrails calibrados ao risk level AND escalation_path configurado
error_handling: escalate to squad-chief on failure, persist error contextcheckpoints:
  - id: PV_PM_001
    phase: executor_design
    description: "Automation Tipping Point — decisão Human vs Agent vs Worker é justificável?"
    threshold: 0.7
    on_fail: REVIEW
