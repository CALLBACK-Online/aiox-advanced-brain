...
pre_condition: Task Anatomy completa (SC_ANA_001 passed) AND executor profile definido AND task-tmpl.md acessível
post_condition: task .md file gerado com todas seções do template preenchidas, usage examples e error handling incluídos
performance: Agent, < 5 min, fail-loud se template section vazia ou campo obrigatório missing
Completion Criteria: task file gerado com all template sections AND usage examples presentes AND error scenarios documentados
