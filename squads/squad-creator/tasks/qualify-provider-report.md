...
pre_condition: all qualification phases completed (preflight, baseline, reliability, compare, optional PT-BR)
post_condition: qualification report consolidado com verdict, model-routing.yaml atualizado se qualified, summary banner
performance: Agent, < 5 min, structured report aggregation com optional routing update
Completion Criteria: report gerado com all evidence AND verdict emitido AND routing updated (se qualified)
...
- Any required upstream phase output (baseline, reliability, compare) is missing -> BLOCK
- `candidate_model` is not found in the external models registry -> BLOCK
- Report generated without PT-BR evaluation data (qualify-provider-ptbr not completed) -> WARN
