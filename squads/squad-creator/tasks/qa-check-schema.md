...
pre_condition: qa-check-structure passed (structure_result.verdict == PASS) AND component files readable
post_condition: schema_report gerado com field validation results, type mismatches e enum violations listados
performance: deterministic Worker, < 30s, fail-loud em schema violation crítica
Completion Criteria: zero BLOCKER schema violations AND all required fields present with correct types
...
### Step 0: AJV Pre-check (C6 — EPIC-109 Wave 3)

**MANDATORY before LLM evaluation when `schemas/` exists in the squad.**

```yaml
ajv_pre_check:
  enabled_when: "squad has schemas/ directory with *.schema.json files"
  script: "squads/squad-creator/scripts/lib/ajv-validate.js"
  cli: "node squads/squad-creator/scripts/validate-schemas.js {squad-path} --format json"
  graceful_degradation: >
    If AJV/Node.js is not available, skip with WARNING and proceed LLM-only.
    Log: "AJV pre-check skipped (graceful degradation) — LLM-only validation."
  override_rule: >
    AJV invalid → result is FAIL, REGARDLESS of LLM evaluation.
    AJV valid   → proceed to LLM evaluation (AJV valid ≠ semantically correct).
    This is NON-NEGOTIABLE. Do not allow LLM to override a deterministic AJV FAIL.
  output:
    inject_into: schema_result.ajv_pre_check
    fields: [valid, errors, schemaId, dataFile]
```

**Execution:**

1. Check if `schemas/` directory exists with `*.schema.json` files
2. If yes: run `node scripts/validate-schemas.js {squad-path} --format json`
3. Parse JSON output → extract `verdict` and `checks`
4. If `verdict == "FAIL"`: set `schema_result.verdict = "FAIL"` immediately
   and skip LLM evaluation for schema check (it cannot override AJV)
5. If `verdict == "PASS"` or `verdict == "SKIP"`: proceed to LLM evaluation
