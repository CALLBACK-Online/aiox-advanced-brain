...
pre_condition: componente criado por creation task (create-squad/agent/task/workflow/template) AND path acessível no filesystem
post_condition: QA pipeline completo com 6 checks executados, final report gerado com score e verdict
performance: < 15 min (Hybrid — 6 atomic checks sequenciais + human review), short-circuit em ABORT/FAIL
Completion Criteria: all 6 QA checks executed AND final_score calculado AND verdict emitido (PASS/CONDITIONAL/FAIL)
...
- `created_component` path does not exist on the filesystem -> BLOCK
- `component_type` is not one of the valid enum values (squad, agent, task, workflow, template) -> BLOCK
- Early pipeline phase (qa-check-structure or qa-check-schema) emits FAIL -> BLOCK (short-circuit remaining phases)
