...
pre_condition: task_purpose e squad_name fornecidos AND squad target existe com config.yaml válido
post_condition: task .md validado por SC_TSK_001, salvo em squads/{squad_name}/tasks/ e registrado em config.yaml
performance: Composed (6 sub-tasks), < 15 min total, veto em quality gate bloqueia pipeline
Completion Criteria: task file gerado com Task Anatomy completa AND SC_TSK_001 PASS AND registrado em config.yaml
...
- Task id already exists in `squads/{squad}/tasks/` (name collision) -> BLOCK
