...
pre_condition: upgrade-squad-apply completed com upgrade log AND backups em .backup/ AND before_score disponível
post_condition: upgrade report com before/after score comparison, rollback instructions se score piorou
performance: deterministic Worker, < 30s para re-validation + score comparison, fail-loud se validação quebra
Completion Criteria: after_score calculado AND after_score >= before_score AND upgrade report final emitido
