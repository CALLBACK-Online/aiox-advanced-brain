...
pre_condition: workflow_purpose e squad_name fornecidos AND squad target existe AND ai-first-governance.md aplicado
post_condition: classification com workflow vs task decisão, identity fields (name, id, duration, scope) definidos
performance: < 15 min (Hybrid — elicitation + classification), redirect para *create-task se não qualifica como workflow
Completion Criteria: operação classificada como workflow (3+ phases, multiple agents) AND identity fields preenchidos
