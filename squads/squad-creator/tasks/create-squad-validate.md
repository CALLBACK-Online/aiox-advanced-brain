...
pre_condition: create-squad-build completed AND squad-checklist.md e quality-dimensions-framework.md disponíveis
post_condition: squad validado com quality_score, governance compliance verificada, issues corrigidos via retry cycles
performance: < 15 min (Hybrid — validate + fix cycles + optimize), max 3 retry cycles antes de escalate
Completion Criteria: quality_score >= 8.5 AND governance compliance PASS AND zero BLOCKER issues AND workflows optimized
