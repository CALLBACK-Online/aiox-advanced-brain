...
pre_condition: upgrade plan approved AND selected phases definidas AND dry-run completado se VETO-UPGRADE-001 aplica
post_condition: structural fixes aplicados com backup, cada change validado e upgrade log gerado
performance: < 15 min (Hybrid — backup + apply + validate per change), rollback se validação falhar
Completion Criteria: selected phases aplicadas AND cada fix validado AND backup disponível em .backup/ AND upgrade log emitido
