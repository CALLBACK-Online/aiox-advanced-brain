...
pre_condition: squad_name fornecido AND squad-analytics.py disponível AND squad directory existe
post_condition: metrics JSON gerado via script, analysis e quality insights interpretados pelo Agent
performance: < 15 min (Hybrid — script deterministic metrics + Agent interpretation), VETO se script falhar
Completion Criteria: preflight-analytics.json gerado AND metrics computed pelo script AND analysis com quality insights
error_handling: fail-loud, VETO on quality gate failure, escalate to squad-chief

## Inputs

- request::squad_analytics## ⛔ MANDATORY PREFLIGHT: Run Worker Script FIRST
...
- `squad-analytics.py` script does not exist at `squads/squad-creator/scripts/squad-analytics.py` -> BLOCK
- Script execution returns non-zero exit code -> BLOCK
- `squad_name` resolves to a non-existent directory under `squads/` -> BLOCK
- Analytics JSON output fails JSON parsing -> BLOCK
