...
Input: request::install_skills_prepare
Output: artifact::install_skills_prepare
pre_condition: squad_name fornecido AND squad tem config.yaml e agents/ com estrutura mínima
post_condition: IDEs detectadas, diretórios de destino criados, conflitos de arquivos resolvidos e plano de execução pronto
performance: deterministic Worker, < 30s, fail-loud se squad structure inválida ou IDE não detectada
Completion Criteria: IDEs detectadas AND diretórios criados AND conflitos resolvidos AND execution plan emitido
>>>>>>>> origin/oalanicolas:squads/squad-creator/tasks/install-skills-prepare.md
