...
pre_condition: task_purpose e squad_name fornecidos AND executor-decision-tree.md e executor-matrix-framework.md acessíveis
post_condition: classification output com Task vs Workflow decisão, target squad validado e executor type atribuído
performance: < 15 min (Hybrid — elicitation + classification), escalate se Task vs Workflow ambíguo
Completion Criteria: classification definida (Task ou Workflow) AND executor type atribuído AND target squad validado
