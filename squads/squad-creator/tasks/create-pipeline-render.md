...
pre_condition: create-pipeline-design completed com pipeline_design_contract (fases, componentes, layout)
post_condition: render bundle completo com scripts/{squad}.sh, prompt.md, lib/, handlers/ e agent summaries
performance: < 15 min (Hybrid — rendering + review), VETO se scripts/{squad}.sh ou prompt.md ausentes
Completion Criteria: shell runner renderizado AND prompt template gerado AND lib/ files criados AND VETO items presentes
