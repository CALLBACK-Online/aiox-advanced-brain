...
pre_condition: squad_name fornecido AND squad config.yaml parseável AND greeting-script-tmpl.cjs acessível
post_condition: generate-{squad}-greeting.cjs criado, integrado com entry agent STEP 3.5 e validado com 3 cenários
performance: Composed (5 subtasks), < 15 min total, veto se script não executa ou Next Action ausente
Completion Criteria: script executa com exit 0 AND Next Action presente AND greeting-script-checklist PASS
