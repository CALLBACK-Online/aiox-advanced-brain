...
pre_condition: task file validado (SC_TSK_001 PASS) AND squad config.yaml existe e writable
post_condition: task registrado em config.yaml, agent dependencies atualizados e README task list refreshed
performance: deterministic Worker, < 30s, fail-loud em config.yaml parse error ou write failure
Completion Criteria: task entry em config.yaml AND agent dependencies atualizados AND README reflects new task
