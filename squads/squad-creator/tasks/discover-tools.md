...
pre_condition: domain e use cases definidos AND squad target existe AND WebSearch/EXA tool disponível
post_condition: discovery report, capability map e integration plan gerados com tools priorizados
performance: Agent, 5 subtasks sequenciais, < 15 min total, escalate se WebSearch indisponível
Completion Criteria: capability gaps identificados AND tools scored/ranked AND integration plan com quick wins emitido
...
- `domain` or `use_cases` not provided (cannot scope discovery) -> BLOCK
- Internal tool scan returns zero results AND external search is unavailable -> BLOCK
- Tool recommendation without IDS gate check (REUSE > ADAPT > CREATE) -> BLOCK
