...
pre_condition: create-agent-generate completed AND agent file existe AND agent-quality-gate.md acessível
post_condition: agent validado por SC_AGT_001, registrado em config.yaml e README.md, handoff para publish emitido
performance: Agent, < 5 min incluindo até 2 iterações de auto-fix, fail-loud se score < threshold após 2 fixes
Completion Criteria: quality_score >= 8.5 across 6 dimensions AND agent registrado em config.yaml AND zero BLOCKER findings
