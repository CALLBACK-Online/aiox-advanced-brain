...
pre_condition: qa-check-structure e qa-check-schema passaram AND squad directory acessível para cross-file lookups
post_condition: references_report gerado com todas cross-references validadas, dangling pointers listados
performance: deterministic Worker, < 30s, fail-loud em referência quebrada para arquivo inexistente
Completion Criteria: zero dangling references AND all handoff_to targets, task refs e template refs resolvem
