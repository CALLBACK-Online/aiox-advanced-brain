...
pre_condition: SOP file existe e readable (.md ou .ml.yaml) AND squad target existe AND workflow-tmpl.yaml acessível
post_condition: workflow .yaml gerado a partir do SOP com phases, agents e checkpoints mapeados, auto-QA via analyze-workflow
performance: Agent, < 5 min, escalate se SOP structure não mapeável para workflow schema
Completion Criteria: workflow .yaml gerado AND RACI roles mapeados para agents AND analyze-workflow auto-QA PASS
