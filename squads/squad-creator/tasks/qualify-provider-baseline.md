...
pre_condition: qualify-provider-preflight PASS AND Opus model disponível para baseline execution
post_condition: Opus baseline com output, token counts, latency e cost capturados como ground truth
performance: Agent, < 5 min, fail-loud se Opus execution falhar (sem baseline = sem comparação)
Completion Criteria: Opus output captured AND metrics (tokens, latency, cost) registrados AND baseline válido
