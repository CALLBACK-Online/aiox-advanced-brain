...
Input: request::sync_ide_skills
Output: artifact::sync_ide_skills
pre_condition: squad_name ou agent_id fornecido AND IDEs configuradas no projeto AND sync-ide-skills.py disponível
post_condition: agents/squads sincronizados para todas IDEs suportadas com relatório de sync emitido
performance: deterministic Worker, < 30s, script-driven (sync-ide-skills.py), fail-loud em sync error
Completion Criteria: arquivos de destino existem em todas IDEs AND relatório final emitido AND sync verificado
error_handling: fail-loud, persist evidence, escalate if unrecoverable
>>>>>>>> origin/oalanicolas:squads/squad-creator/tasks/sync-ide-skills.md
## Purpose
