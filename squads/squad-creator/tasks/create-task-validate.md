...
pre_condition: task file content gerado AND quality-dimensions-framework.md acessível AND write permissions em squads/{squad_name}/tasks/
post_condition: task file validado por SC_TSK_001, blocking issues corrigidos (max 2 iterações) e file salvo
performance: Agent, < 5 min incluindo até 2 fix iterations, fail-loud se quality gate falhar após fixes
Completion Criteria: SC_TSK_001 PASS AND task file salvo em squads/{squad_name}/tasks/{task_id}.md AND zero BLOCKER findings
