...
pre_condition: domain e purpose fornecidos pelo usuário, WebSearch disponível, squad-type-definitions.yaml acessível
post_condition: viability assessment concluído, squad type selecionado, agent roster inicial definido com tier assignments
performance: < 15 min (Hybrid — inclui elicitation e pesquisa), escalate se viability_score < 6
Completion Criteria: viability_score >= 6 AND squad_type selecionado AND agent_roster definido AND overlap_check executado
error_handling: escalate to squad-chief on failure, persist error contextcheckpoints:
  - id: PV_BS_001
    phase: discovery
    description: "Future Back-Casting — visão de end-state do squad é clara e defensável?"
    threshold: 0.7
    on_fail: REVIEW
...
Before declaring `create_new`, `missing_capability`, `missing_task`, `missing_agent`, or `missing_script`, execute and document prior-art search. This prevents fossilizing false gaps into the architecture.

```yaml
prior_art_search:
  required: true
  reference_rule: ".claude/rules/prior-art-search.md"
  minimum_search_vectors: 2
  search_vectors:
    - name_grep:
        paths: ["squads/", "scripts/", ".aiox-core/", ".claude/"]
        query: "{domain OR proposed_squad_name OR proposed_artifact_name}"
    - semantic_grep:
        paths: ["squads/", "scripts/", ".aiox-core/", ".claude/"]
        query: "{function_description_keywords}"
    - functional_glob:
        paths:
          - "squads/*/scripts/*.{py,js,cjs,mjs,sh}"
          - "squads/*/tasks/*.md"
          - "squads/*/workflows/*.yaml"
    - registry_check:
        files:
  ...
  required_output:
    ...
    search_vectors: 2
    rows:
      - claim: "{what might need to be created}"
        searches_performed: ["name_grep", "semantic_grep"]
        matches: "{count + inspected candidates}"
        verdict: "REUSE | ADAPT | CREATE | ABORT_DUPLICATE"
        evidence_paths: ["{path}"]
  blocking_rule:
    - "If status != PASS: block discovery"
    - "If a candidate covers >=80% of requested use cases: decision must be extend, not create_new"
    - "If partial overlap exists: document integration plan before Step 0.3"
```
...
existing_squad_check:
  ...
  output:
    ...
    rationale: "..."
    prior_art_search: "{prior_art_search.required_output}"
```
...
- [ ] Prior-art search status is PASS with at least 2 search vectors documented
...
- **VETO-SQD-007:** Prior-art search missing, incomplete, or contradicted by discovered reusable/extendable assets
