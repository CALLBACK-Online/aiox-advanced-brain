...
pre_condition: briefing, outputs_defined e skills_detected fornecidos para análise de sinais
post_condition: classificação OPERATIONAL/TEXTUAL emitida com infrastructure contract quando operacional
performance: Composed (5 sub-tasks), < 10 min total, deterministic signal collection + classification
Completion Criteria: mode classificado AND evidências de sinais documentadas AND infrastructure contract emitido (se OPERATIONAL)
