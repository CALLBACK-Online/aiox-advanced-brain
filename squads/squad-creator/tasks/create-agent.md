...
pre_condition: agent_purpose, domain e squad_name fornecidos AND squad target existe em squads/{squad_name}/
post_condition: agent .md gerado com >= 300 linhas, validado por SC_AGT_001, registrado em config.yaml e publicado via IDE sync
performance: Agent, < 5 min por fase (6 fases), escalate se quality gate falhar após 2 iterações de fix
Completion Criteria: agent file existe AND quality_score >= 8.5 AND config.yaml atualizado AND IDE sync executado
