...
pre_condition: create-pipeline-qualify PASS AND pipeline_qualification_contract disponível
post_condition: pipeline design contract fechado com fases, criticalidade, timeout, cache e estratégia inline vs handlers
performance: < 15 min (Hybrid — elicitation de fases + design decisions), escalate em conflito de criticalidade
Completion Criteria: todas fases com num/name/critical/timeout/cacheable definidos AND estratégia de handlers decidida
...
- `pipeline_qualification_contract` is missing or its `decision` field is `veto` -> BLOCK
- Fewer than 3 phases defined and no explicit override from operator -> WARN
- Phase dependency graph contains cycles -> BLOCK (must be a DAG)
