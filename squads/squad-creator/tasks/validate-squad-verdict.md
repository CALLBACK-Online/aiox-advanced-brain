...
pre_condition: preflight, classification e deep review (ou skip) completed com scores disponíveis
post_condition: final validation report (JSON + markdown + console) com PASS/FAIL verdict e veto gates avaliados
performance: deterministic Worker, < 30s, reconcile + veto + report em pipeline linear
Completion Criteria: all scores reconciled AND veto gates avaliados AND final PASS/FAIL verdict emitido com report
