...
pre_condition: preflight PASS AND type classified AND reviewer CLI disponível (claude -p ou codex exec)
post_condition: semantic quality scores para prompt quality, pipeline coherence, checklist actionability e documentation depth
performance: Agent, < 5 min, optional (quick mode skips), escalate se reviewer CLI indisponível
Completion Criteria: semantic dimensions scored AND review findings documentados OR skip_reason emitido (quick mode)
