...
pre_condition: qa-check-structure passed, componente gerado acessível, governance framework e security checklist disponíveis
post_condition: compatibility_report gerado com scores de governance, security (SEC-001 a SEC-018) e ecosystem integration
performance: deterministic Worker, < 30s, fail-loud em security violation
Completion Criteria: compatibility_score >= 8.0 AND zero CRITICAL security findings AND ecosystem_ready = true
error_handling: fail-loud, persist evidence, escalate if unrecoverablecheckpoints:
  - id: PV_PA_001
    phase: validation
    description: "Systemic Coherence Scan — componente é coerente com o ecossistema existente?"
    threshold: 0.7
    on_fail: REVIEW
