...
pre_condition: componente gerado existe no filesystem, schema definitions disponíveis para validação
post_condition: structure_report gerado com scores de completeness, syntax validity e metadata presence
performance: deterministic Worker, < 30s, fail-loud em missing required files
Completion Criteria: structure_score >= 8.5 AND zero BLOCKER findings AND all required files present
error_handling: fail-loud, persist evidence, escalate if unrecoverablecheckpoints:
  - id: META_AXIOMAS
    phase: quality_gate_final
    description: "Validação axiomática — structure+schema passam threshold de Verdade?"
    threshold: 7.0
    on_fail: VETO
