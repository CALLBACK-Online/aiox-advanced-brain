...
pre_condition: squad directory existe em squads/{squad_name}/ AND config.yaml presente
post_condition: component inventory completo com categorização por tipo, line counts e metadata extraída
performance: deterministic Worker, < 30s, fail-loud se squad directory não existe
Completion Criteria: todos componentes categorizados (agents, tasks, workflows, templates) AND version extraída de config.yaml
