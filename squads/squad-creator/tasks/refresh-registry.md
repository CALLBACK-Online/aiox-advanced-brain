...
pre_condition: squads/ directory acessível AND refresh-registry.py disponível AND registry path resolvível
post_condition: ecosystem-registry.yaml atualizado com scan completo de todos squads, enrichments preservados
performance: deterministic Worker, < 30s, script-only (zero LLM), fail-loud em YAML write error
Completion Criteria: all squads scanned AND registry merged com enrichments preservados AND YAML escrito AND summary printed
