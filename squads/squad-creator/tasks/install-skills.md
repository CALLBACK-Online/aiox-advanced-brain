...
Input: request::install_skills
Output: artifact::install_skills
pre_condition: squad_name fornecido AND squad tem config.yaml e agents/ AND IDEs detectadas no projeto
post_condition: agents publicáveis instalados nas superfícies de IDE suportadas via sync-ide-skills.py
performance: deterministic Worker, < 30s, script-driven sync com overwrite policy
Completion Criteria: skill files escritos em todas IDEs detectadas AND verificação de consistência PASS AND report emitido
>>>>>>>> origin/oalanicolas:squads/squad-creator/tasks/install-skills.md
