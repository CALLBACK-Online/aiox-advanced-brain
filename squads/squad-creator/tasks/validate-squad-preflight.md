...
pre_condition: squad directory existe em squads/{squad_name}/ AND config.yaml presente AND worker scripts disponíveis
post_condition: preflight results com structural signals, inventory, naming checks, dependency validation e blocking conditions
performance: deterministic Worker, < 30s, fail-loud em blocking condition (structure, security, refs)
Completion Criteria: all worker scripts executed AND blocking conditions avaliadas AND preflight-results.yaml gerado
