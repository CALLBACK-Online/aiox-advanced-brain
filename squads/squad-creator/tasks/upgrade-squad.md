...
pre_condition: squad_name fornecido AND squad directory existe com config.yaml AND AIOX structural standards acessíveis
post_condition: squad upgraded com gap fixes aplicados, scores before/after comparados e upgrade report gerado
performance: < 15 min (Hybrid — 5 subtasks sequenciais com approval gate), rollback disponível se score piorou
Completion Criteria: all approved upgrade phases applied AND after_score >= before_score AND upgrade report emitido
