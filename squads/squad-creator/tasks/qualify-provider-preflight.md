...
pre_condition: task_id e candidate_model fornecidos AND candidates registry acessível AND LLM Router disponível
post_condition: inputs validados, model disponibilidade confirmada via health ping, test directory criado
performance: Agent, < 5 min, fail-loud se model indisponível ou task não registrada em candidates
Completion Criteria: task exists in registry AND model allowed AND health ping PASS AND test directory created
