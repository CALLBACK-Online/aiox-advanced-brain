...
pre_condition: create-squad-validate concluída com PASS AND config.yaml com entry_agent e slashPrefix AND chief existe
post_condition: activation surfaces publicadas (aiox-sync, IDE skills, Codex, CLAUDE.md) AND smoke test passed
performance: deterministic Worker, < 30s para publish + < 2 min para smoke test, fail-loud em publish error
Completion Criteria: todas activation surfaces atualizadas AND smoke test PASS AND squad ativável via slash command
