...
**Agregação única (OBRIGATÓRIO):** os caps aplicam UMA vez, sobre `Base + Σ(todos os
clusters)` — NUNCA por cluster. Cap por cluster foi a causa-raiz da divergência numérica
da run de 2026-05-21 (portfolio_intelligence_capability).

### Saturação de Cap — Tie-Break e Leitura (precedente 2026-05, codificado 2026-07-06)

12 lentes × ±15 = ±180 de ajuste possível sobre bases de 5-80: saturação em 95%/5% é
ESPERADA em decisões com vencedores/perdedores claros — não é erro, mas exige protocolo:

1. **Coluna Σ bruto pré-cap é OBRIGATÓRIA em todo ranking** — é o tie-break oficial
   entre futuros saturados (precedente: F1 +106 / F7 +106 / F4 +84, run de maio).
2. **Saturação múltipla no TETO é sinal**, não empate a forçar: os saturados são
   frequentemente VARIANTES DO MESMO CAMINHO sob condições diferentes — a decisão real
   vira faseamento/execução, não escolha excludente (maio: F1 contém F7, decisão virou
   D→B; julho: F1/F4/F7 eram a mesma estratégia em 3 climas).
3. **Saturação múltipla no PISO é sinal simétrico**: família de futuros condenados pela
   MESMA razão profunda — nomeie a razão (vira failure factor da síntese).

### Pesos por Tipo de Decisão — Ponto de Aplicação (OBRIGATÓRIO)

A tabela de pesos (abaixo) aplica-se **multiplicando o ajuste de cada lente ANTES da
soma** (`ajuste_ponderado = ajuste × peso`, arredondar para inteiro), nunca pós-soma.
Apresentar SEMPRE os rankings ponderado E não-ponderado; se divergirem no topo, tratar
como contradição a resolver. Decisão de tipo misto → usar não-ponderado e declarar.
(Nenhuma run havia aplicado os pesos até 2026-07-06 porque este ponto era ambíguo.)
