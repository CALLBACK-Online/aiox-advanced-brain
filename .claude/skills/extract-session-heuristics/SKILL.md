...
description: "Extrai heurísticas operacionais de sessões de trabalho usando metodologia Pareto ao Cubo + GAH calibrado."
...
/extract-session-heuristics --scope project "allfluence"
...
2. usuário/operador ativo da sessão com namespace em `minds/`
3. ownership inferido do contexto da sessão/projeto
4. perguntar uma vez, apenas se continuar ambíguo

Regras:

- Nunca assumir `@oalanicolas` como default só porque existe um exemplo legado.
- Sempre escrever no namespace do owner resolvido: `minds/{heuristic_owner_slug}/heuristics/`.
...
- Exemplo concreto: sessões do Pedro PV devem resolver para `pedro-valerio` e usar `PV_KE` como família default.
...
3. **Read the canonical heuristic template** at `squads/squad-creator-pro/templates/session-heuristic-tmpl.md`
4. **Use KE as the default target family** for session extraction unless the user explicitly requests a checkpoint family
...
7. **Read existing decision cards** at `minds/{heuristic_owner_slug}/heuristics/decision-cards.yaml`
...
**Objetivo:** filtrar candidatas que NÃO são heurísticas (são pipeline_instance/task/checklist/rule/archive disfarçados).

Para CADA candidata bruta de Phase 1, executar 4 testes semânticos estruturados. LLM produz output estruturado obrigatório (não pode escapar com "depende"). Output: 1 audit YAML por candidata em `outputs/minds/gah-audits/{owner_slug}/{candidate-slug}-{YYYYMMDD}.yaml`.

Template: `squads/squad-creator-pro/templates/gah-audit-tmpl.yaml`. Detalhes completos dos 4 testes: `squads/squad-creator-pro/tasks/an-extract-session-heuristics.md#phase-15-gah`.

| Teste | Eixo filtrado | Verdict |
|-------|---------------|---------|
| 1. Vocabulary Strip | Agnosticidade lexical (heurística sobrevive sem vocab stack-específico?) | PASS / FAIL |
| 2. Cross-Domain Instances | Largura de aplicação (≥3 domínios literais sem reescrita?) | PASS (≥3) / FLAG (==2) / FAIL (<2) |
| 3. Other-Human Test | Utilidade externa (outro humano em outro projeto encontra valor?) | PASS / FLAG / FAIL |
| 4. Anti-Test | Tensão sistêmica (existe contexto onde fazer o oposto é certo?) | PASS / FLAG (vago) / FAIL (tautologia) |

**Decisão matrix (aggregate):**

| pass | flag | fail | Verdict | Próximo passo |
|-----:|-----:|-----:|---------|---------------|
| 4 | 0 | 0 | **ADMIT** | Procede pra Phase 2 |
...
| 0-2 | * | 2-4 | **REJECT** | Reclassificar destination: pipeline_instance / task / checklist / rule / archive |

**Mode v1: FLAG only.** Audit é gerado, verdict é calculado, mas TODAS candidatas seguem para Phase 2. Operador revisa audit YAML e decide ações manuais (skip/rewrite/reclassify) ANTES de Phase 5 PERSIST.

**Quando promover para BLOCK mode:** após 3 sessões consecutivas onde GAH verdict bate com decisão final do operador (calibração). Versão atual: v1.0.0 FLAG.
...
cat minds/{heuristic_owner_slug}/heuristics/decision-cards.yaml
```
...
Use o template canônico em `squads/squad-creator-pro/templates/session-heuristic-tmpl.md`.
...
```bash
ls minds/{heuristic_owner_slug}/heuristics/{heuristic_id_prefix}_*.md | sort | tail -1
```
...
### Phase 5.5: CALIBRATE — GAH Verdict vs Operator Decision (2 min)

**Objetivo:** alimentar o calibration log que decide quando o GAH pode promover de FLAG para BLOCK.

Para CADA candidata bruta processada em Phase 1.5 (não só as persistidas), append uma linha JSON em:

```
outputs/minds/gah-audits/{owner_slug}/calibration.jsonl
```

**Schema da linha (gah-calibration-v1):**

```json
{
  "ts": "<ISO-8601>",
  "session": "<session slug or handoff path>",
  "owner": "<owner_slug>",
  "candidate": "<heuristic_id se persistida, candidate slug caso contrário>",
  "gah_verdict": "ADMIT | ADMIT_WITH_REWRITE_LIGHT | REWRITE_REQUIRED | REJECT",
  "operator_decision": "persisted | skipped | rewritten | updated_existing | reclassified | archived",
  "matches": true | false,
  "audit_path": "outputs/minds/gah-audits/{owner_slug}/{candidate}-{YYYYMMDD}.yaml"
}
```

**Regra de `matches`:**

| GAH verdict | Operator action que MATCH | Operator action que MISS |
|-------------|---------------------------|--------------------------|
| `ADMIT` | `persisted`, `updated_existing` | `skipped`, `rewritten`, `reclassified` |
| `ADMIT_WITH_REWRITE_LIGHT` | `persisted`, `rewritten`, `updated_existing` | `skipped`, `reclassified` |
| `REWRITE_REQUIRED` | `rewritten` | `persisted` (without rewrite), `skipped` |
| `REJECT` | `skipped`, `reclassified`, `archived` | `persisted` |

**Regras de append:**

1. Append-only — NUNCA editar linhas anteriores. NUNCA deletar linhas para inflar match-rate.
2. Uma linha por candidata bruta de Phase 1, não só as que sobreviveram.
3. `session` deve ser único por execução do skill (slug + timestamp).
4. Se Phase 1.5 foi pulada (skill < v3.3), append nada — a linha pressupõe audit YAML existente.

**Promotion check (executar AO FIM da Phase 5.5):**

1. Ler todo o `calibration.jsonl` do owner.
2. Agrupar por `session` (DISTINCT).
3. Para cada sessão, calcular `match_rate = sum(matches==true) / count(linhas)`.
4. Considerar apenas as 3 sessões mais recentes que tenham ≥ 5 candidatas (TKN-ESH-THR-002).
5. Se as 3 mais recentes têm `match_rate ≥ 0.80` → emitir output:

```
GAH PROMOTION CANDIDATE
- Last 3 sessions match-rate: <r1>, <r2>, <r3>
- All ≥ 0.80 threshold
- Action: review and flip `gate_admissibilidade_heuristica.mode` from FLAG to BLOCK
  in .claude/skills/extract-session-heuristics/config.yaml
```

6. Caso contrário: emitir o status atual (`<N>/3 sessions calibrated, current match-rate <r>`).

**Mode v1 FLAG:** este check é advisory. Operador decide a flip manualmente; não auto-edita config.yaml.
...
```
minds/{heuristic_owner_slug}/heuristics/
...
|- {candidate-slug-or-id}-{YYYYMMDD}.yaml   <- 1 audit per candidate (Phase 1.5)
|- retroactive-{YYYYMMDD}.md                <- (advisory) full audit of existing cards
```
...
Heurísticas consolidadas podem ser promovidas para `.claude/rules/` ou SINKRA tokens.
...
Heurística criada (Phase 5)
  ...
  → Ao atingir 3 sessions:
      ...
      3. Human gate: Pedro Valério (ou @squad-chief) aprova
      4. SE aprovado:
         ...
         c. SE zone=genialidade: candidata a SINKRA token (token-registry.yaml)
```

### Mapeamento Heurística → SINKRA

| Campo heurística | Target SINKRA | Mapeamento |
|-----------------|---------------|-----------|
...
promoted_to_token: null        # Token ID após promoção SINKRA
```
...
1. Skill no Hub (`synkra-hub/.claude/skills/`) é SOT para estrutura base
2. Spoke (allfluence) pode adicionar scopes (project/global) como extensão
