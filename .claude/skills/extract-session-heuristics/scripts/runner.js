 ...
 *
 * Related: STORY-MIG-1.13, AN_KE_152, AN_KE_147
 */
...
const {
  scanNamespace,
  checkDrift,
  scanDecisionCardsConsistency,
} = require('./lib/heuristic-scanner');
...
// Ensures .aiox/active-operator.yaml exists and is valid.
// If missing: runs cascade fallback + write-back via sinkra-operator setter.
// ================================================================
...
function cmdBootstrap() {
  try {
    ...
    printJson({
      ok: false,
      error: err.message,
      hint: 'Run: npm run sinkra:operator -- list  (and then) -- set <slug>',
    });
    return 1;
  }
}
...
function cmdScan() {
  ...
  const consistency = scanDecisionCardsConsistency(nsPath);
  ...
  printJson({
    ...
    drift: {
      ...
      deltas: drift.drift,
      hint: drift.hasDrift
        ? 'Run: npm run sinkra:operator -- refresh'
        : 'operator state matches filesystem',
    },
    decision_cards_consistency: {
      ok: consistency.ok,
      declared_total: consistency.declared_total,
      card_count: consistency.card_count,
      file_count: consistency.file_count,
      files_without_cards: consistency.files_without_cards,
      cards_without_files: consistency.cards_without_files,
      total_matches_cards: consistency.total_matches_cards,
    },
  });
  return 0;
}
...
// batch.json schema:
//   {
//     "heuristic_files": ["abs/path/AN_KE_143.md", ...],
//     "new_cards": [ { id: "AN_KE_143", name: "...", ... }, ... ],
//     "dry_run": false
//   }
// ================================================================
  ...
  if (!load.ok) {
    printJson({
      ...
      hint: load.reason === 'file_missing'
        ...
        : 'Run: npm run sinkra:operator -- set <slug>',
    });
    return 1;
  }
...
// ================================================================
// Subcommand: verify-cards
// Fails when decision-cards.yaml and L3 files diverge.
// ================================================================

function cmdVerifyCards() {
  const load = loadOperator();
  if (!load.ok) {
    printJson({ ok: false, error: `operator not loaded (${load.reason})` });
    return 1;
  ...
  printJson({
    ...
    operator: load.operator.slug,
    namespace: load.operator.heuristic_namespace,
    decision_cards: path.relative(REPO_ROOT, consistency.path),
    declared_total: consistency.declared_total,
    card_count: consistency.card_count,
    file_count: consistency.file_count,
    files_without_cards: consistency.files_without_cards,
    cards_without_files: consistency.cards_without_files,
    total_matches_cards: consistency.total_matches_cards,
    error: consistency.ok ? null : 'decision-cards.yaml and L3 heuristic files diverge',
  });
...
function cmdHelp() {
  ...
  console.log('  bootstrap                  Phase 0.5 — ensure .aiox/active-operator.yaml is loaded');
  ...
  console.log('  verify-cards               Fail if decision-cards.yaml and L3 files diverge');
  ...
  console.log('');
  console.log('Related: STORY-MIG-1.13, AN_KE_152 (persistent ownership), AN_KE_147 (enforcement mechanism)');
  console.log('');
