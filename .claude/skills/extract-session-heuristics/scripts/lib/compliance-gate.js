...
const MANDATORY_CHECKS = [
  ...
  },
  {
    ...
    test: (content) => {
      ...
      const hasSeEntaoNunca = /\bSE\b[^a-zA-Z]/.test(content)
        && /ENT[ÃA]O[^a-zA-Z]/.test(content)
        && (/\bNUNCA\b[^a-zA-Z]|NEVER:/i.test(content));
      const hasStructuredRule = /^\s*rule:\s*$/mi.test(content)
        && /^\s*if:\s*["|]/mi.test(content)
        && /^\s*then:\s*["|]/mi.test(content)
        && /^\s*never:\s*["|]/mi.test(content);
      return { ok: hasSeEntaoNunca || hasStructuredRule };
    },
  },
  {
    id: 'sys_tension',
  ...
  },
  {
    ...
    test: (content) => {
      const hasAntiPattern = /anti[-_\s]?pattern\s*:?/i.test(content);
      const hasNever = /NEVER:|^\s*never:\s*/im.test(content);
