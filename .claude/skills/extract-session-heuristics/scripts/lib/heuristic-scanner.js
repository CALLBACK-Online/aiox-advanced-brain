...
/**
 * Scan a heuristics namespace directory (e.g. minds/alan_nicolas/heuristics).
...
function scanNamespace(namespacePath) {
  ...
  // Match both 2-letter (AN_KE) and longer prefixes (PV_KE, etc)
...
/**
 * Compare L3 heuristic files with L2 decision-card entries.
 * This catches the drift that plain operator scans miss: files can exist while
 * decision-cards.yaml is missing the runtime card, or vice versa.
 */
function scanDecisionCardsConsistency(namespacePath) {
  const scan = scanNamespace(namespacePath);
  const cardsPath = path.join(namespacePath, 'heuristics', 'decision-cards.yaml');

  if (!fs.existsSync(cardsPath)) {
    return {
      ok: false,
      reason: 'decision_cards_missing',
      path: cardsPath,
      declared_total: null,
      card_count: 0,
      file_count: scan.count,
      files_without_cards: scan.files.map(f => f.replace(/\.md$/, '')),
      cards_without_files: [],
      total_matches_cards: false,
    };
  }

  const data = readDecisionCards(cardsPath);
  const cardIds = new Set((data.cards || []).map(card => card.id));
  const fileIds = new Set(scan.files.map(f => f.replace(/\.md$/, '')));

  const filesWithoutCards = [...fileIds].filter(id => !cardIds.has(id)).sort();
  const cardsWithoutFiles = [...cardIds].filter(id => !fileIds.has(id)).sort();
  const declaredTotal = data.total ?? null;
  const cardCount = data.cards.length;
  const totalMatchesCards = declaredTotal === cardCount;

  return {
    ok: filesWithoutCards.length === 0 && cardsWithoutFiles.length === 0 && totalMatchesCards,
    reason: null,
    path: cardsPath,
    declared_total: declaredTotal,
    card_count: cardCount,
    file_count: scan.count,
    files_without_cards: filesWithoutCards,
    cards_without_files: cardsWithoutFiles,
    total_matches_cards: totalMatchesCards,
  };
}
