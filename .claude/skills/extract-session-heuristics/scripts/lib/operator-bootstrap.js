 ...
 *
 * Canonical file: .aiox/active-operator.yaml (LOCAL, gitignored — bootstrapped per checkout).
 * Schema: .aiox/active-operator.example.yaml.
 * Setter: scripts/sinkra-operator.js (run via `npm run sinkra:operator -- set <slug>`).
 *
 * Implements AN_KE_152 (Runtime Ownership via Persistent State) + STORY-MIG-1.13.
 */
...
const OPERATOR_FILE = path.join(REPO_ROOT, '.aiox', 'active-operator.yaml');
const OPERATOR_EXAMPLE = path.join(REPO_ROOT, '.aiox', 'active-operator.example.yaml');
const MINDS_ROOT = path.join(REPO_ROOT, 'minds');
const SINKRA_OPERATOR_SCRIPT = path.join(REPO_ROOT, 'scripts', 'sinkra-operator.js');

/**
 * Phase 0.5 fast path — read .aiox/active-operator.yaml and validate.
 ...
 *   1. Env override: AIOX_ACTIVE_OPERATOR
...
function cascadeFallback() {
  // 1. Env override
  const envOverride = process.env.AIOX_ACTIVE_OPERATOR;
...
/**
 * Write-back via sinkra-operator setter. More robust than writing YAML manually
 * because the setter scans the filesystem and populates counts/next_id automatically.
 * Returns { success, stdout, stderr }.
 */
function writeBackViaSetter(slug) {
  try {
    const stdout = execSync(`node "${SINKRA_OPERATOR_SCRIPT}" set ${slug}`, {
      cwd: REPO_ROOT,
      encoding: 'utf-8',
    });
