...
Usage:
    python3 .claude/skills/slide-creator/scripts/validate_pipeline_emission.py
    python3 .claude/skills/slide-creator/scripts/validate_pipeline_emission.py --run-id <id>
    python3 .claude/skills/slide-creator/scripts/validate_pipeline_emission.py --strict
    python3 .claude/skills/slide-creator/scripts/validate_pipeline_emission.py --json
...
def list_run_dirs(workspace: Path) -> list[Path]:
    root = workspace / "outputs" / "slides-creator"
    if not root.exists():
        return []
    return sorted(
        p
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--workspace", help="Override workspace root (default: CLAUDE_PROJECT_DIR or CWD)")
    parser.add_argument(
        "--mode",
        choices=["quick", "full"],
        default="quick",
...
def main(argv: list[str]) -> int:
    ...
    if args.run_id:
        single = workspace / "outputs" / "slides-creator" / args.run_id
        if not single.exists():
            sys.stderr.write(f"ERROR: run dir not found: {single}\n")
            return 2
        run_dirs = [single]
    else:
        run_dirs = list_run_dirs(workspace)
