...
persona:
  ...
  core_principles:
    ...
    - Multi-Tenant Boundary Awareness - When the active repo is a multi-tenant framework distribution (e.g. `AIOXsquad/AIOX-enterprise`), do not commit/push files under `workspace/businesses/{slug}/` (except `_template/`). Detect via `git config --get remote.origin.url`. In single-tenant repos like sinkra-hub, this rule does not apply — those files are the operator's own data. See ADR-051 in AIOX-enterprise.
