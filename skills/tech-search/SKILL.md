...
constraints:
  forbidden_actions:
    ...
    - NEVER write to .claude/agents/, skills/, squads/, app/, lib/
...
tool_hierarchy:
  ...
  deep_read:
    1_preferred: "WebFetch with prompts/page-extract.md prompt"
    2_enhanced: "Scrapling MCP — use when WebFetch returns empty/blocked content"
    routing: |
      1. Try WebFetch first (zero overhead, always available)
      2. IF WebFetch returns empty, blocked, 403, Cloudflare challenge, or JS-only content:
         → Use scrapling.get(url) for static HTML extraction
         → Use scrapling.fetch(url) for JS-rendered content
         → Use scrapling.stealthy_fetch(url) for anti-bot protected sites
      3. Scrapling pre-filters by CSS selector before returning content,
         reducing token consumption vs raw WebFetch
    detection: |
      Try ToolSearch("scrapling") at Phase 3 start.
      If found: set scrapling_available=true.
      If not found: set scrapling_available=false, use WebFetch only.
    note: "No ETL, no Bash, no external scripts. WebFetch + Scrapling MCP only."

  bulk_extraction:
    tool: "Scrapling MCP (scrapling.bulk_get or scrapling.bulk_fetch)"
    when: "5+ URLs need deep-read in a single worker — faster than sequential WebFetch"
    detection: "Only if scrapling_available=true"
    ...
    3_parallel_search:
      ...
      execution: |
        1. PRE-CHECK MCP AVAILABILITY (main model, before dispatch):
           ...
           - Try Scrapling: ToolSearch("scrapling")
             → If found: scrapling_available = true
             → If not found: scrapling_available = false
        ...
        2. DISPATCH WORKERS:
           ...
           MCP AVAILABILITY: exa={exa_available}, context7={context7_available}, scrapling={scrapling_available}
           ...
           3. Deep-read top 1-3 results — TOOL SELECTION:
              a) FIRST try WebFetch for each URL with this prompt:
              ...
              b) IF WebFetch returns empty/blocked/403/Cloudflare AND scrapling available:
                 → Use ToolSearch("scrapling") to load Scrapling tools
                 → Use scrapling.get(url) for static content
                 → Use scrapling.stealthy_fetch(url) for anti-bot protected sites
                 → Scrapling supports CSS selector pre-filtering to reduce tokens

              c) IF 3+ URLs need deep-read AND scrapling available:
                 → Use scrapling.bulk_get(urls) for parallel extraction (faster)
           ...
           4. Return results as JSON (no other text):
              {
                ...
                "sources": [
                  {"url": "...", "title": "...", "snippet": "first 200 chars...",
                   "credibility": "HIGH|MEDIUM|LOW", "tool_used": "WebSearch|Exa|Context7|Scrapling"}
                ],
           ...
           - Track tool_used for each source (WebFetch, Scrapling, Exa, Context7).
           ```
...
Query → Auto-Clarify → Decompose (ultrathink, MAIN MODEL)
              ...
              (search+read)  (search+read)       (search+read)
                   |              |                   |
                   |         Tool Selection:
                   |         ├─ Exa → WebSearch (search)
                   |         ├─ WebFetch (deep read, 1st try)
                   |         └─ Scrapling (fallback: blocked/anti-bot/bulk)
                   |              |                   |
                   +------+-------+-------+-----------+
                          |
...
- **Scrapling is OPTIONAL** — skill works 100% without it (falls back to WebFetch)
