...
**Reference test:** `skills/wave-execute/__tests__/full-sdc-standalone-mode1.test.js`
...
Agent(
  ...
  prompt: "[ACTION REQUIRED: validate-story-draft]
    ...
    Read skills/validate-story-draft/SKILL.md and execute the
...
Agent(
  ...
  prompt: "[ACTION REQUIRED: develop-story]
    ...
    Read skills/develop-story/SKILL.md and execute the COMPLETE
...
SendMessage(
  ...
  message: "[ACTION REQUIRED: review-story]
    ...
    Read skills/review-story/SKILL.md and execute the COMPLETE
    review protocol for:
...
WHILE qg_iteration <= max_qg_iterations AND gate == FAIL:
  ...
  SendMessage(
    ...
    message: "[ACTION REQUIRED: apply-qa-fixes]
      ...
      Read skills/apply-qa-fixes/SKILL.md and execute the protocol:
      - Fix each issue
...
>
> **Bootstrapping note:** `skills/full-sdc/SKILL.md` is self-modified by AC-5 of Story 115.S2.O11. NÃO invocar `/full-sdc` recursivamente para desenvolver O11. Verificar on-disk diretamente. Precedente: O6b CG-8.
...
Agent(
  ...
  prompt: "[ACTION REQUIRED: deploy-story → verify-deploy]
    ...
    Read skills/deploy-story/SKILL.md and execute the deploy protocol for:
    Story: {story_path}
    ...
    Read skills/verify-deploy/SKILL.md and execute the verify protocol
    for the same story.
...
`skills/close-story/SKILL.md` CHK-0 deliberately.
...
SendMessage(
  ...
  message: "[ACTION REQUIRED: close-story]
    ...
    Read skills/close-story/SKILL.md and execute the COMPLETE
  ...
  IF retry_count <= 2:
    SendMessage(
      ...
      message: "[ACTION REQUIRED: complete-missing-artifacts]
        ...
        These are REQUIRED by skills/{skill_name}/SKILL.md.
...
**INDEX.json registration (R39 — NON-DEFERRABLE):** `.aiox/worktrees/registry.json` MUST be registered in `.aiox/registries/INDEX.json` (schema `aiox-index-v1`) when the file is first created. See §INDEX.json Registration below.
...
When `.aiox/worktrees/registry.json` is first created (AC-3), register it in `.aiox/registries/INDEX.json` using schema `aiox-index-v1`:
