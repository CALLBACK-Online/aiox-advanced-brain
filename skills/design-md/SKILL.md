...
Legacy snapshot files may remain under `skills/design-md/` during transition, but they are not authoritative. Do not edit or call them for extractor behavior.
...
- Do not maintain a second implementation inside `skills/design-md/`; the launcher must delegate to `squads/design-ops/scripts/extract-from-url/`.
- Do not add browser automation to this extractor. Visual diffing and screenshot validation are downstream workflows.
- Do not write canonical business data to `docs/` (substituto local; outputs/ é private enterprise distribution) from this skill; extracted runs live under `outputs/design-ops/url-extracts/`.
