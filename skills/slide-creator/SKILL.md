...
This skill is self-contained. Do not require `squads/slides-creator`, external agents, AIOX tasks, or private repo files. Use bundled templates and references only when needed.
...
The skill respects 7 inviolable principles authored canonically in the squad `slides-creator`. See `references/principios-invioláveis.md` (mirror) and `squads/slides-creator/data/principios-invioláveis.md` (SoT). Violation of any P1-P7 = QA FAIL automatic:

- **P1 Fidelidade aos dados** — every claim has source_ref
- **P2 Especificação obsessiva** — hex/grid_coord/pt_size resolved
- **P3 Pyramid + MECE** — governing thought → supporting → details
- **P4 Action titles** — propositions, never topics
- **P5 Enumeração universal** — every visual has ID
- **P6 Citação APA** — 5 fields per citation
- **P7 Prompts IA completos** — 10 components per LLM call

## SoT Manifest

Per-artifact source-of-truth between skill and squad declared in `references/sot-manifest.yaml`. Highlights:
- 7 slide archetypes → squad SoT (skill imports copy)
- Principios P1-P7 + AI slop blacklist → squad SoT (skill references)
- Wireframes (60) + runtime contracts (31) + Python validators (17) → skill SoT
- Pipeline execution log schema → squad SoT (skill emits using squad schema)
...
5. **Select canonical templates**
   Load `templates/index.yaml` first. Select workflow mode, deck playbook, deck template, slide-function templates, visual templates, theme profile, research route, import pipeline, runtime job, rendered-eval contract, and QA gates before drafting content. For key slides, produce a template-selection report with rejected runners-up and deck-specific rejection reasons.
   For consulting-grade decks, also consult `templates/slide-archetypes/` (7 McKinsey archetypes: title, executive-summary, framework, data, timeline-roadmap, financial, appendix) — these are the canonical CONTENT patterns paired with wireframe LAYOUT patterns.
   Use `references/template-selection-guide.md`, `references/roteiro-template-library.md`, and `references/slide-structure-library.md` only as explanatory backup.
...
11. **Package + emit pipeline log (Mandamento 10)**
   ...
   Emit a pipeline execution log to `outputs/slides-creator/{run_id}/pipeline-execution-log.yaml`. The **declarative emission matrix** lives in `templates/runtime/phase-emit-matrix.yaml` (skill mirror of the squad workflow `phase_emit_matrix`). Read that file FIRST to know which events to emit per step + per gate.

   Minimum events: `workflow_started` (step 1), `gate_verdict` (one per gate at steps 9 + 10), `workflow_completion` (end of step 11). Full mode also emits `phase_transition` between every step boundary.

   Emission path:
   - **Squad runtime available:** `node squads/slides-creator/scripts/emit-pipeline-log.js --run-id <id> --event <event> --payload '<json>'`
   - **Standalone (Codex / external):** emit YAML directly per `references/pipeline-log-emission.md` Path 2

   Validate emission before declaring the run complete:
   - **Python validator (always available):** `python3 skills/slide-creator/scripts/validate_pipeline_emission.py --run-id <id>`
   - **Squad CI gate (when monorepo present):** `node squads/slides-creator/scripts/validate-pipeline-emission.js --run-id <id>`

   Both validators read the same log path and apply the same schema (workflow_started + gate_verdict + workflow_completion required; min 3 records quick, 8 records full).
...
- If defining visual direction, load `templates/runtime/design-direction-contract.yaml` (includes the `tasteskill_dials` section: `DESIGN_VARIANCE` / `MOTION_INTENSITY` / `VISUAL_DENSITY` 1-10 with `per_workflow_mode_defaults` for executive/board/dashboard/pitch/marketing/etc).
- If the workflow_mode is set, READ `templates/runtime/design-direction-contract.yaml#tasteskill_dials.per_workflow_mode_defaults.<mode>` to load default dial values; honor `dials_override` from briefing if provided.
- If the deck has SaaS dashboard slides, feature sections, or "Vercel-core meets Dribbble-clean" aesthetic moments, load `templates/visual/bento-archetypes.yaml` and pick from the 5 named archetypes (Intelligent List, Command Input, Live Status, Wide Data Stream, Contextual UI Focus) — but only when `MOTION_INTENSITY >= 5`; otherwise use static variants.
- If the deck imports a source style via `/design-md` (URL → DESIGN.md pipeline), READ `outputs/design-ops/url-extracts/{company}/dial-inference.yaml` and seed `design-direction-contract.tasteskill_dials` with the inferred dials.
- If the source URL has an `ai-fingerprint-report.json` with `ai_slop_score > 60`, REFUSE to replicate that style — emit a warning slide and recommend rebrief instead of importing AI-slop signatures.
- If the deck register is `brand|pitch|marketing|creative` (per `briefing.register`), enforce font allowlist (`Geist | Outfit | Cabinet Grotesk | Satoshi`) in `render-lock.yaml`; `Inter`/`Mona Sans`/`Plus Jakarta Sans` are BLOCKED in those registers per `squads/design-ops/data/font-allowlist.yaml`.
- If running the runtime validator, expect `scripts/validate_design_100_runtime.py` to include `font_monoculture`, `eight_bans`, `jane_doe`, and `persona_walkthrough` scorers in addition to the original 5 — cross-fit bench absorption 2026-05-19.
...
- If the user asks for "an API for this", "an MCP for this", "expose this as a service", or "run this in CI/batch", route to `apps/squad-engine` REST (`/api/v1/slides/*`), `slides-mcp` tools (`slides.generate|review|export|edit|show`), or `scripts/aiox/slides-cli.mjs` — all three share `slides-core` so parity is mandatory.
...
- `templates/deck/copy-derived.yaml`: offer, VSL, cohort launch, and investor pitch templates absorbed from AIOX copy and pitch squads.
