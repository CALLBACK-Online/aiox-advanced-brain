...
| Mode | AIOX Mode | Purpose | Verdict Model | Agent Limit |
|------|------------|---------|---------------|-------------|
...
domain_to_agents:
  ...
  aiox_framework:
    keywords: [AIOX, composition-rules, token-registry, process, pipeline, "v3.1", "v3.2", organism, molecule, atom, flywheel, mission-pattern]
    agents: ["@aiox-chief"]

  architecture:
    keywords: [architecture, ADR, package, monorepo, Hub, Spoke, migration, "@aiox/core", dependency, exports, build]
    agents: ["@architect"]
  ...
  c_level:
    ...
    agents: ["@vision-chief", "@human-orchestrator"]
```
...
| `governance_review` | @cso, @aiox-master, @aiox-chief | review |
...
| `final_approval` | @cso, @vision-chief, @human, @cto, @aiox-chief, @squad-chief, @architect, @devops, @infra-chief, @aiox-master | review (extended) |
| `architecture_decision` | @architect, @cso, @devops, @infra-chief, @aiox-chief | decision |
| `gap_discovery` | @squad-chief, @aiox-chief, @cso, @human, @architect | gap_analysis |
| `state_investigation` | @cso, @architect, @qa, @db-sage, @aiox-master, @aiox-chief | investigation |
| `strategic_planning` | @vision-chief, @cso, @human, @cto, @pm, @architect | strategic |
...
| @aiox-master | AIOX Master | Framework compliance, Constitution | Story template, agent assignments, AIOX v3.1/v3.2, Hub×Spoke compliance, Constitution, missing stories |
| @aiox-chief | AIOX Chief | Composition rules, tokens, processes | Composition rules, token families, process mapping, organism patterns, flywheel/mission alignment |
...
| @human-orchestrator | COO | Operações, processos, equipe | Operational feasibility, resource availability, process efficiency |
...
2. **Analisar à luz de:** Constitution, AIOX composition rules, ADRs existentes, precedentes, epistemic standards
...
Esta skill emite `orchestration_events[]` no learning log ao final de cada execução.

Vocabulário canônico, schema v1.0, Invariants I-1/I-2/I-3 e persistência canônica em
`.synapse/metrics/orchestration-friction.json` estão definidos em:

> `.claude/rules/orchestration-telemetry.md` — **não copiar** o vocabulário aqui.

**Recipients autorizados (I-2):** `[membros do council declarados na sessão]` (dinâmico — enum fechado por sessão)

**Nota ACK (I-1):** `/roundtable` opera em `context: conversation` sem worktree isolado.
O fallback message-ACK (via SendMessage de resposta com TTL explícito) é o mecanismo
disponível até on-disk ACK ser viabilizado para skills conversation-context.

## Post-Execution Learning (schema v1.1)

Ao final de cada execução, criar log em `.aiox/learning/logs/roundtable/roundtable-{topic}-{timestamp}.yaml`

Schema v1.1 — inclui `orchestration_events[]`:

```yaml
schema_version: "1.1"
skill_id: roundtable
run_id: "{topic}-{timestamp}"
# ... demais campos ...
orchestration_events:
  - type: "<canonical enum — ver .claude/rules/orchestration-telemetry.md>"
    phase: <int>
    severity: "low|medium|high"
    skill: roundtable
    run_id: "{topic}-{timestamp}"
    timestamp: "<ISO8601>"
    detail: "<descrição do evento>"
    resolution: "<resolução ou null>"
