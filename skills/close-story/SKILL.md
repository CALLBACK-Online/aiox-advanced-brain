...
5. If not found → **read `.aiox/config/skill-defaults.json` → `dynamic_agent_binding.fallback_agent`**, load `.claude/agents/{fallback_agent}.md`. If fallback agent file also not found, HALT with error: "Quality gate agent and fallback agent both missing. Check .claude/agents/ directory."
...
- Coverage score (target: `registry_governance.coverage_target` from `.aiox/config/skill-defaults.json`, TK-RG-003)
...
**If coverage < coverage_target (read from `.aiox/config/skill-defaults.json` → `registry_governance.coverage_target`):**
...
- Read `squads/aiox-squad/data/ecosystem-registry.yaml`
...
**For each new skill file matching `skills/*/SKILL.md`:**
- List existing skill directories in `skills/`
...
3. **Read the policy digest** at `policy/digests/at-close.md` and apply it during closure.
   If `POLICY_DIGEST_OFF=1`, skip `at-close.md`, read `policy/digests/_safety.md` when
   available, and log `policy_digest_off=1` in the Closure Summary. This is runtime
   steering, not background documentation. If the digest is missing, log
   `policy_digest_missing` in the Closure Summary and continue; do not invent policy content.
4. **Read the AIOS task file** at `.aiox/aiox-canon/development/tasks/po-close-story.md`
5. **Execute the closure protocol** defined in the task file:
...
Before final response, classify whether any `policy/digests/at-close.md` card was
load-bearing, violated, or contradicted in this closure. The LLM/operator decides the
classification; `policy/scripts/log-event.mjs` only transports the event.

For each event, run:

```bash
npm run policy:log-event -- --card <card-id> --event cited|vetoed|violated|contradicted|overridden \
  --origin meta|product \
  --session "<story-id>" --note "<short evidence>"
```

Use `origin: meta` for work about policy/skills/governance, and `origin: product` for
real product or operational delivery.
If `POLICY_DIGEST_OFF=1`, prefix the session with `ablation-`, for example
`--session "ablation-<story-id>"`.

If no card materially changed the closure decision, write `Policy telemetry: none` in the
Closure Summary. Do not log decorative events.

### Section 4.5: Demand-Driven Policy Proposals

Before final response, inspect the closure context for new heuristic candidates. This is
an inbox only; NEVER create or edit `policy/cards/*.md` from this step.

1. Harvest every literal operator marker shaped as `#regra: ...` from the conversation,
   QA Results, Dev Agent Record, or Closure Summary context. For each marker, append:

```bash
npm run policy:propose -- --source operator-marker --session "<story-id>" \
  --note "#regra: <literal marker>" --evidence "<literal surrounding context>" \
  --suggested-id "<slug>" --trigger-moment "<moment>" [--absorbs AN_KE_NNN,...]
```

2. Also review operator corrections/vetos from the closure. If a correction does not
   semantically match any loaded policy card, propose a candidate with source
   `llm-close-detection`. Include suggested id, `SE -> THEN -> NEVER` rule text, the
   anti-pattern with literal evidence, and default `review_after: +30d`.
3. Before adding `absorbs`, search the active owner archive/source files
   (`minds/alan_nicolas/heuristics/AN_KE_*.md` and relevant owner sources). If an archived
   card covers the proposal, include its id via `--absorbs`; otherwise leave `absorbs: []`.
4. Human gate is absolute: proposals stay in `policy/telemetry/proposals.jsonl` until the
   operator explicitly accepts/edits/discards them. With `CONDUCTOR_ACTIVE=true`, append
   proposals to the inbox without asking.
5. Write `Policy proposals: <N> appended` or `Policy proposals: none` in the Closure
   Summary.

### Section 5: Epic Development Log Entry (MANDATORY)
...
- [ ] Coverage score checked against `registry_governance.coverage_target` from `.aiox/config/skill-defaults.json`
...
- [ ] New `skills/*/SKILL.md` files checked against existing skill directories (IDS duplicate detection)
...
1. **File path:** Read `.aiox/config/skill-defaults.json` → `learning.base_path`, then append `logs/close-story/close-story-{story-id}-{YYYYMMDD}-{HHmmss}.yaml` to form the full path.
