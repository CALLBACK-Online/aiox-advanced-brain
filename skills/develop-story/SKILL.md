...
Port all task logic from `.aiox/aiox-canon/development/tasks/dev-develop-story.md` into this skill so `/develop` is equivalent to running the full 925-line task. This skill covers: 3 execution modes, constitutional gates, CodeRabbit self-healing, decision logging, Code Intelligence Check, story file restrictions, blocking conditions, deploy awareness, and post-execution learning.
...
3. **Completion**
   ...
   - Execute `.aiox/aiox-canon/product/checklists/story-dod-checklist.md`
...
### Policy Digest Runtime Injection (before-edit/on-error)

Applies to YOLO, Interactive, and Pre-Flight modes without changing decision logging or mode-specific prompts.

1. **Ablation branch:** If `POLICY_DIGEST_OFF=1`, skip `before-edit.md` and `on-error.md`, read `policy/digests/_safety.md` when available, and record `policy_digest_off=1` visibly in the Dev Agent Record/decision context. Never make ablation silent.
2. **Before first edit:** Before the first code/config/story mutation of an execution, Read `policy/digests/before-edit.md` unless ablation is active. If the file is missing or unreadable, log a visible WARN in the Dev Agent Record/decision context and continue; this digest is steering, not a dependency.
3. **On fix cycle entry:** When entering any fix cycle (failed test/validator, QG finding, CodeRabbit issue, repeated error, or a user-reported bug), Read `policy/digests/on-error.md` unless ablation is active before proposing or applying the fix. If the file is missing or unreadable, log a visible WARN in the Dev Agent Record/decision context and continue.
4. **Decision logging:** In YOLO mode, record whether each digest was loaded, gracefully skipped or ablated in the decision log when decision logging is available. In Interactive and Pre-Flight modes, include the same fact in the visible checkpoint/plan or Dev Agent Record.
5. **Scope:** Do not add telemetry writes here. The digests are read-only policy context and must not change existing completion, CodeRabbit, ACK, or learning-log contracts.
...
7. **`.aiox/aiox-canon/product/checklists/story-dod-checklist.md` executed**
...
- `.aiox/aiox-canon/product/checklists/story-dod-checklist.md` — Definition of Done checklist
...
- [ ] `.aiox/aiox-canon/product/checklists/story-dod-checklist.md` executed in full
