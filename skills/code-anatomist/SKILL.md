...
- Use esta skill como **porta de entrada** do squad `code-anatomist` quando a missão for a dor coberta pela aula do curso.
...
- Só quer estudar anatomia sem copiar o pacote: leia a aula e o `squads/code-anatomist/` sem ativar runtime.

## Aula do curso (como usar de verdade)

`cursos/AIOX-Advanced-Squads/aulas/03-code-anatomist.md`

Lá estão: quando usar/evitar, briefing, ativação, evidência e limites de maturidade.


Squad: `squads/code-anatomist/` · entry agent: `decoder-chief` em `squads/code-anatomist/agents/decoder-chief.md`.
...
### E11: Baseline Immutability + Delta Re-Analysis
Existing extraction outputs are baselines. Never overwrite completed phase artifacts just because the same slug is analyzed again.

Before writing any file under `outputs/decoded/{slug}/`, inspect:
- `outputs/decoded/{slug}/00-extraction-index.yaml`
- Existing phase directories
- Existing `deltas/` and `_history/` directories

Decision rules:
1. **No existing extraction** → write the canonical phase outputs directly under `outputs/decoded/{slug}/`.
2. **Interrupted extraction** (`current_phase` incomplete or required phase files missing) → resume only the missing/incomplete phase. Do not replace completed phase files unless the user explicitly asks to repair that phase.
3. **Completed extraction exists and user asks to re-run, validate deeper, or analyze delta** → preserve the canonical baseline and write the new work under:
   `outputs/decoded/{slug}/deltas/{YYYY-MM-DD}-{reason_slug}/`
4. **Replacement requested explicitly** → first archive the existing canonical artifacts under:
   `outputs/decoded/{slug}/_history/{YYYY-MM-DD-HHMMSS}/`
   then write `replacement-decision.md` explaining why replacement is valid.

Required delta artifacts:
- `baseline-ref.yaml` — baseline path, prior index checksum if available, source path/URL, source commit when available, prior completed phases.
- `as-is-delta.md` — what changed since baseline, by phase/capability, with closed gaps, still-open gaps, superseded assumptions, new risks, and recommended follow-up stories.
- Phase-specific addenda only for new findings, e.g. `phase-3-fusion/boundary-depth-addendum.md`; do not rewrite the original `phase-3-fusion/c4-container.md` unless replacement was explicitly approved.

`*compare` and `*audit` remain valid delta-producing commands, but they do not authorize overwriting the canonical extraction baseline.
...
```
0. Resolve persistence mode:
   - canonical_new: no previous extraction exists
   - resume: previous extraction is incomplete; fill only missing/incomplete phase files
   - delta: completed baseline exists; write to outputs/decoded/{slug}/deltas/{date}-{reason}/
   - replace: explicit user-approved replacement; archive baseline first

1. Create output directory:
   mkdir -p {OUTPUT_ROOT}/{phase_dir}/

2. Execute the phase (spawn agent, read sources, extract/analyze)

3. Write output file(s):
   Write tool → {OUTPUT_ROOT}/{phase_dir}/{filename}

4. Run phase gate:
   Bash: ls {OUTPUT_ROOT}/{phase_dir}/
   Bash: wc -c {OUTPUT_ROOT}/{phase_dir}/*

5. Update index:
   canonical_new/resume/replace → Edit tool → outputs/decoded/{slug}/00-extraction-index.yaml
   delta → Write/Edit delta index → outputs/decoded/{slug}/deltas/{date}-{reason}/delta-index.yaml
...
Reference checklist: `skills/code-anatomist/checklists/phase-gates-G0-G4.md`
...
Templates in `skills/code-anatomist/templates/` and `squads/code-anatomist/templates/`:
