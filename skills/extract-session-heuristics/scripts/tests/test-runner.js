 ...
 *
 * Usage: node skills/extract-session-heuristics/scripts/tests/test-runner.js
 */
...
const NAMESPACE = path.join(REPO_ROOT, 'minds/alan_nicolas/heuristics');
...
test('T2 status returns valid operator state', () => {
  ...
  assertEq(data.operator.slug, 'alan_nicolas', 'slug should be alan_nicolas');
  assertEq(data.operator.prefix, 'AN_KE', 'prefix should be AN_KE');
...
test('T3 scan matches filesystem state', () => {
  ...
  assertEq(data.operator, 'alan_nicolas', 'operator should be alan_nicolas');

  // Count the actual files in the namespace
  const actualFiles = fs
    .readdirSync(NAMESPACE)
    .filter(f => /^AN_KE_\d+\.md$/.test(f));
  ...
  const lastNum = parseInt(lastFile.match(/AN_KE_(\d+)/)[1], 10);
  const expectedNext = `AN_KE_${String(lastNum + 1).padStart(3, '0')}`;
...
test('T4 bootstrap uses persistent_state when file exists', () => {
  const operatorFile = path.join(REPO_ROOT, '.aiox/active-operator.yaml');
  if (!fs.existsSync(operatorFile)) {
    throw new Error('TEST PREREQ: .aiox/active-operator.yaml must exist for T4');
  }
  ...
  assertEq(data.operator.slug, 'alan_nicolas', 'operator should be alan_nicolas');
});
...
test('T5 validate AN_KE_152 passes 6/6 mandatory', () => {
  const file = 'minds/alan_nicolas/heuristics/AN_KE_152.md';
...
// ================================================================
// T7 — validate subcommand on a known contiguous current batch
// ================================================================

test('T7 all AN_KE_147-153 pass compliance gate', () => {
  const ids = [147, 148, 149, 150, 151, 152, 153];
  const results = {};
  for (const id of ids) {
    const fname = `AN_KE_${id}.md`;
    const fpath = path.join(NAMESPACE, fname);
    if (!fs.existsSync(fpath)) {
      throw new Error(`TEST PREREQ: ${fname} should exist in namespace`);
    }
    const rel = `minds/alan_nicolas/heuristics/${fname}`;
...
// ================================================================
// T8 — check-overlap with rich keywords finds AN_KE_152
// ================================================================

test('T8 check-overlap detects existing AN_KE_152 for active operator keywords', () => {
  const payload = JSON.stringify({
    keywords: ['active', 'operator', 'owner', 'cascata', 'deterministic'],
  });
  ...
  assertContains(ids, 'AN_KE_152', 'AN_KE_152 should be in top matches');
});
...
test('T10 persist dry-run validates batch without side effects', () => {
  ...
  const batch = {
    heuristic_files: [
      path.join(NAMESPACE, 'AN_KE_147.md'),
      path.join(NAMESPACE, 'AN_KE_148.md'),
    ],
    new_cards: [
...
test('T13 runner scripts contain no semantic classification/generation', () => {
  const scriptsDir = path.join(REPO_ROOT, 'skills/extract-session-heuristics/scripts');
  const files = [
...
test('T14 active-operator.example.yaml exists and parses', () => {
  const examplePath = path.join(REPO_ROOT, '.aiox/active-operator.example.yaml');
...
test('T15 active-operator.yaml matches gitignore rule', () => {
  try {
    execSync('git check-ignore --no-index .aiox/active-operator.yaml', {
      cwd: REPO_ROOT,
      stdio: 'pipe',
    });
    // exit 0 means it matches an ignore rule, even if currently tracked
  } catch (err) {
    throw new Error('.aiox/active-operator.yaml should match a gitignore rule (git check-ignore returned non-zero)');
  }
});
...
test('T16 active-operator.example.yaml is NOT gitignored', () => {
  try {
    execSync('git check-ignore .aiox/active-operator.example.yaml', {
      cwd: REPO_ROOT,
      stdio: 'pipe',
    });
    throw new Error('.aiox/active-operator.example.yaml should NOT be gitignored');
  } catch (err) {
...
// ================================================================
// T17 — aiox-operator npm script works
// ================================================================

test('T17 npm run aiox:operator -- list works', () => {
  try {
    const stdout = execSync('npm run aiox:operator -- list 2>&1', {
      cwd: REPO_ROOT,
      encoding: 'utf-8',
    });
    assertContains(stdout, 'alan_nicolas', 'list should mention alan_nicolas');
    assertContains(stdout, 'active', 'list should mark one as active');
  } catch (err) {
    throw new Error(`npm run aiox:operator -- list failed: ${err.message}`);
  }
});
...
test('T18 decision-cards.yaml structure + file/card consistency', () => {
  ...
  const LEGACY_GAPS = ['AN_KE_115'];  // pre-existing drift from before STORY-MIG-1.13
