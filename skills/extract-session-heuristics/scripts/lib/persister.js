 ...
 *   3. Invoke `aiox-operator refresh` to update active-operator.yaml counts
 *   4. Report summary
 *
...
const { AIOX_OPERATOR_SCRIPT, REPO_ROOT } = require('./operator-bootstrap');
 ...
 * @param {boolean} params.skipOperatorRefresh  If true, don't run aiox-operator refresh
 *
...
function persistBatch(params) {
  ...
  // Step 4 — refresh active-operator.yaml via aiox-operator
  let refreshed = false;
  let refreshOutput = '';
  if (!skipOperatorRefresh) {
    try {
      refreshOutput = execSync(`node "${AIOX_OPERATOR_SCRIPT}" refresh`, {
        cwd: REPO_ROOT,
        encoding: 'utf-8',
      });
      refreshed = true;
