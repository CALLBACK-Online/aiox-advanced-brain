 ...
 * Setter: scripts/aiox-operator.js (run via `npm run aiox:operator -- set <slug>`).
 *
...
const AIOX_OPERATOR_SCRIPT = path.join(REPO_ROOT, 'scripts', 'aiox-operator.js');
...
/**
 * Write-back via aiox-operator setter. More robust than writing YAML manually
 * because the setter scans the filesystem and populates counts/next_id automatically.
 * Returns { success, stdout, stderr }.
 */
function writeBackViaSetter(slug) {
  try {
    const stdout = execSync(`node "${AIOX_OPERATOR_SCRIPT}" set ${slug}`, {
      cwd: REPO_ROOT,
      encoding: 'utf-8',
    });
