...
// If missing: runs cascade fallback + write-back via aiox-operator setter.
// ================================================================
...
function cmdBootstrap() {
  try {
    ...
    printJson({
      ok: false,
      error: err.message,
      hint: 'Run: npm run aiox:operator -- list  (and then) -- set <slug>',
    });
    return 1;
  }
}
...
function cmdScan() {
  ...
  printJson({
    ...
    drift: {
      ...
      deltas: drift.drift,
      hint: drift.hasDrift
        ? 'Run: npm run aiox:operator -- refresh'
  ...
  if (!load.ok) {
    printJson({
      ...
      hint: load.reason === 'file_missing'
        ...
        : 'Run: npm run aiox:operator -- set <slug>',
    });
    return 1;
  }
