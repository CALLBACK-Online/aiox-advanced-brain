...
- Action: review and flip `gate_admissibilidade_heuristica.mode` from FLAG to BLOCK
  in skills/extract-session-heuristics/config.yaml
```
...
Heurísticas consolidadas podem ser promovidas para `.claude/rules/` ou AIOX tokens.
...
Heurística criada (Phase 5)
  ...
  → Ao atingir 3 sessions:
      ...
      4. SE aprovado:
         ...
         c. SE zone=genialidade: candidata a AIOX token (token-registry.yaml)
```

### Mapeamento Heurística → AIOX

| Campo heurística | Target AIOX | Mapeamento |
|-----------------|---------------|-----------|
...
promoted_to_token: null        # Token ID após promoção AIOX
```
...
1. Skill no Hub (`synkra-hub/skills/`) é SOT para estrutura base
