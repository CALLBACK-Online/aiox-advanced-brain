...
# Tech Research — AIOX-Native Deep Research Pipeline v2.0.0
...
Pattern: aligned with `/processo de mapeamento AIOX`, `/aiox-validate-squad`, `/full-sdc`.
...
This skill does **NOT** spawn specialist subagents per phase (unlike `/processo de mapeamento AIOX`
...
documenter) are **roles**, not full AIOX agents. Each is ~5-10 specific behaviors
...
expert protocols (e.g. `processo de mapeamento AIOX` spawns 7 specialists across 60-120 minutes).
tech-research's 7 molecules complete in ~5-15 minutes inline.

### When inline fallback applies (from processo de mapeamento AIOX)
...
# In practice: read the first 20 lines of skills/tech-research/SKILL.md
# and extract the workflow_version field. Compare with what was loaded.
on_disk_version = Read("skills/tech-research/SKILL.md", limit=20)
...
- **t0+30d:** if zero regression incidents are documented in `outputs/qa/` over the window, @aiox-devops promotes the default by editing this SKILL.md and `skills/research-bench/SKILL.md` to swap `--enforcement warn` → `--enforcement block`. The flip date is recorded in the same Change Log.
...
> (`skills/tech-research/SKILL.md`) as a stable alias; the implementation
...
├── data/tech-research/
│   ├── _skill-config.yaml            # AIOX-native + operational config
