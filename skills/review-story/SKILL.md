...
### Step 5.5: Policy Finding Telemetry (Additive)

After manual findings, CodeRabbit findings, AC gaps, and `top_issues` are consolidated,
compare each finding against loaded policy cards:

1. Read `policy/digests/_always.md`.
2. Read the phase digest that was loaded by the reviewed implementation session when the
   story records it in the Dev Agent Record. If none is recorded, use
   `policy/digests/before-edit.md` as the develop-phase digest. Missing digests are
   graceful: continue with the digests that exist.
3. For each finding, the LLM decides semantically whether the finding matches a card
   `anti_pattern`. Do not use keyword/regex matching. Zero matches is a valid outcome.
4. A match means: the card was available, but the reviewed work still violated its
   anti-pattern. Log a `violated` event without changing the gate verdict:

```bash
npm run policy:log-event -- --card <card-id> --event violated --session <story-id> \
  --origin meta|product \
  --note "QG finding F{NN}: <one-line finding summary>"
```

Use `origin: meta` for reviews of policy/skills/governance, and `origin: product` for
real product or operational delivery.

5. Record the outcome in QA Results under `Policy Finding Telemetry`: list events logged,
   or write `none — no semantic card match`.

This step is transport only. The reviewer owns the semantic classification; the script
only validates and appends the event.
...
### Policy Finding Telemetry

{Events logged with `npm run policy:log-event`, or "none — no semantic card match"}
...
2. **Risk thresholds** (if `risk_summary` present) — read from `.aiox/config/skill-defaults.json → quality_scoring.risk_thresholds`:
...
Before calculating the quality score, read `.aiox/config/skill-defaults.json`:

1. Open `.aiox/config/skill-defaults.json`
...
3. **If `quality_scoring` key is missing or the file does not exist → HALT immediately** with message:
   ```
   HALT: .aiox/config/skill-defaults.json not found or missing quality_scoring.weights.
...
```
fail_deduction    = quality_scoring.weights.fail_deduction    (from .aiox/config/skill-defaults.json)
concerns_deduction = quality_scoring.weights.concerns_deduction (from .aiox/config/skill-defaults.json)
