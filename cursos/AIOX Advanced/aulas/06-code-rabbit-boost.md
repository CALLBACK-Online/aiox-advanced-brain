...
source_path: content/courses/aiox-advanced/aulas/06-code-rabbit-boost/lesson.md
...
- **meta**: fonte=aula-01 + aula-06 + t2-aula-2
...
O "silêncio" não é figura de linguagem. Foi assim que o Pedro apresentou o
reviewer na primeira aula do cohort, na tela, mostrando o core-config:

> **Pedro Valerio Lopez (aula-01 L929)**: Tem vários testes no AIOX que já usam o CodeRabbit silenciosamente. Isso é o self-healing. É o autocura. Então os códigos já estão saindo melhores sem vocês saberem.
...
Na primeira aula, quando o Code Rabbit apareceu no bootstrap, o Alan
traduziu o efeito em uma frase que resume esta aula inteira:

> **Alan Nicolas (aula-01 L907-911)**: Se vocês fizerem esse Setup GitHub, vocês vão estar colocando esse Code Rabbit para funcionar para vocês, que é um cara que aumenta a qualidade do código de vocês nas alturas. Porque, por mais que o Claude Opus 4.5 é muito bom, ele ainda comete vários erros. Esse cara tem até uma função de self-heal, que ele mesmo se cura. Ou seja, ele mesmo corrige os erros e as vulnerabilidades do teu código automaticamente.
...
Essa progressão não é interpretação do curso. É literal na fala do Pedro,
no fechamento do raciocínio de determinismo progressivo da primeira aula:

> **Pedro Valerio Lopez (aula-01 L2917-2919)**: Vai acionar CodeRabbit de novo. Então ele tem uma progressão de CodeRabbit que é trinta, sessenta, noventa: trinta no self-heal, chega em sessenta de acurácia no Review do QA e chega em noventa de acurácia no CI/CD, com CodeRabbit no repositório. Então você vê esse determinismo progressivo? Ele tira a abstração da LLM, travando nesses estágios de Quality Gate.

E a integração com o template de Story também não é figura de linguagem:
no core-config, o bloco de integração aparece com enable true ou false, e
quando uma Story está sendo desenvolvida, obrigatoriamente o template de
Story tem a integração de fazer teste com o Code Rabbit. Tudo está
atrelado: criar Story já é botar o Code Rabbit ali. [SOURCE: aula-01 L863-869]
...
O gate 1 foi aberto ao vivo no cohort. O Pedro projetou a task do @dev e
apontou a instrução literal: execute CodeRabbit self-healing loop. O loop
roda depois que o @dev terminou todas as tasks e subtasks da Story, e antes
do checklist de definition of done: o self-healing acontece entre os dois,
um loop que melhora em trinta o código. Ali ele roda em light mode, porque
um modo mais pesado deixaria os ciclos de desenvolvimento longos demais; o
que muda entre os modos é o número máximo de interações em que o reviewer
entra no código para comentar. Se não há nada crítico, CodeRabbit passed,
prossiga para o checklist; se foram achados problemas, auto fix. [SOURCE: aula-01 L2237-2283]

> **Pedro Valerio Lopez (aula-01 L2285-2287)**: Isso aqui vocês não veem acontecer. Isso é silencioso, porque o self-heal pra mim é o mínimo. Se eu estou trabalhando com inteligência artificial e com automação, o self-heal para mim é o mínimo. É o cara revisando o seu próprio trabalho, e não deixando passar erro ortográfico.
...
> **Pedro Valerio Lopez (aula-06 L819-823)**: Se você estiver com CodeRabbit no seu projeto, ele vai fazer o self-heal loop escondido, sem você ver. Quando voltar do @dev, ele vai voltar com o status já em Review.
...
Esse gate foi demonstrado ao vivo. Na primeira aula, o Alan abriu um PR
real no GitHub para mostrar o Code Rabbit trabalhando lá dentro: o
reviewer disse que o código não passou e listou tudo que podia gerar
problema. O Claude Code tentou convencer que dava para deixar assim ("deixa
isso aí, esse cara nem sabe o que tá falando"); o Alan respondeu mandando
consertar, de novo e de novo, até que, depois de meia hora de interações
entre os dois, o erro foi corrigido de verdade. A conclusão dele: isso te
garante que a possibilidade de o código estar muito bom, muito bem feito, é
muito elevada. [SOURCE: aula-01 L2869-2881]

> **Alan Nicolas (aula-01 L2871)**: Cara, o CodeRabbit é chato pra caramba. Ele revisa tudo. Ele vai olhando.

Na aula seis, o mesmo mecanismo apareceu do lado do time: nos PRs abertos
pela equipe do Pedro, o Code Rabbit fez toda a análise do que aconteceu, o
GitHub Action também, e o Review Required bloqueou a parada toda até os
achados serem resolvidos. É a regra que ele mesmo definiu: ele pode aprovar
sozinho, o time não. [SOURCE: aula-06 L499-513]
...
Na aula seis, no meio de uma demo em que ele mostrava o self-heal segurando
um teste que não passou, o Pedro deu o teste mais direto de todos:

> **Pedro Valerio Lopez (aula-06 L1033)**: Isso aqui é o CodeRabbit self-heal. Ah, como eu sei que o CodeRabbit está funcionando ou não? Primeiro: se não existe um arquivo chamado coderabbit.yaml no seu projeto, você não está com CodeRabbit configurado no projeto.

E quando um aluno perguntou qual task configura o coderabbit.yaml e o Code
Rabbit no repositório, a resposta do próprio @devops, consultado ao vivo,
foi que não existe um comando isolado para isso: a configuração faz parte
do Setup GitHub, que já deixa tudo ligado. [SOURCE: aula-06 L1163-1175]
...
- coderabbit.yaml: Raiz do projeto, um por tech stack | Configura o reviewer para aquele domínio | Ausente: Code Rabbit não está configurado
...
Existe ainda uma distinção que o Pedro fez questão de mostrar quando um
aluno trouxe as duas formas de usar a ferramenta. O CLI e o app do GitHub
não competem: são momentos diferentes do mesmo guardião.

> **Pedro Valerio Lopez (aula-06 L1131-1143)**: A diferença do CLI é que o CLI vai funcionar durante o development cycle workflow, fazendo o self-healing, e na parte de Review do QA. O do GitHub vai funcionar onde? Nos PRs, no GitHub Action. Então são formas diferentes de usar o CodeRabbit. E eu uso todas elas. Para quê? Para proteger o máximo possível da entropia e da alucinação.

No projeto real dele, essa configuração é por tech stack: o design system
tem um Code Rabbit específico, porque a stack do design system é diferente
da stack do worker, que é diferente da do front-end e da do back-end. Um
coderabbit.yaml genérico revisaria todo mundo com a régua errada. [SOURCE: aula-06 L1117-1127]
...
Perguntas e momentos reais das transcrições que geraram esta aula.

**"Tem duas maneiras de usar o CodeRabbit, via CLI e instalando dentro do
GitHub?"** Um aluno da aula seis trouxe a dúvida contando que usa os dois.
O Pedro confirmou: "são momentos diferentes". O CLI cobre o self-healing e
o Review do QA; o app do GitHub cobre os PRs. Quem usa só um deixa metade
do fluxo sem guardião.
*T1 · aula-06 L1091-1097*

**"Qual CodeRabbit é melhor, assertive? Qual a diferença?"** Pergunta do
chat na primeira aula. Resposta do Pedro: o modo assertivo demora mais
porque fica vendo tudo, e por isso as fases do Dev, do QA e do DevOps têm
configurações um pouco diferentes. "Segue o que está ali. Não precisa mexer
em nada disso. Vai embora. Deixa o AIOX fazer o trabalho dele."
*T1 · aula-01 L1009-1013*

**Nem toda Story precisa do reviewer.** Na aula dois, o Pedro lembrou que
existem tipos de Story com templates diferentes: "Stories de investigação
de documentação não precisa de Code Rabbit." O gate protege código; Story
que não produz código usa outro template.
*T1 · aula-02 L2965*

**O gate como padrão do cohort.** Na T2, o Adriano formalizou o mesmo
princípio dentro do artigo Quality First da Constitution: primeiro
qualidade, não velocidade, não economia de token. "Você vai passar por
gates, nós vamos usar CodeRabbit", além de processos de lint e type check.
Para quê? Para que quem pega esse código, sendo um engenheiro de software,
fale: "caramba, o cara que fez isso é um gênio".
*T2 · t2-aula-2 L2329-2337*
