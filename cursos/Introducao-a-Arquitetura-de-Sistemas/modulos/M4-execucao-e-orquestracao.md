---
type: course-module
course: introducao-arquitetura-sistemas
module: M4
sequence: 4
status: canonical
canonical_scope: cursos/Introducao-a-Arquitetura-de-Sistemas
---

# M4 — Execução e orquestração

Transforme etapas em unidades executáveis e saiba onde dividir, sequenciar e reunir o trabalho.

## Resultado

Plano de execução com tasks, workers, pipeline e fan-in explícito.

## Aulas

10. [Processo, task, job, worker e runner](../aulas/10-processo-task-job-worker-runner.md)
11. [Workflow, pipeline, batch e stream](../aulas/11-workflow-pipeline-batch-stream.md)
12. [Concorrência, paralelismo, fan-out e fan-in](../aulas/12-concorrencia-paralelismo-fanout-fanin.md)

## Evidência

Você identifica dependências, ownership e ponto de convergência antes de paralelizar.

## Checkpoint

[Quiz M4](../avaliacoes/Quiz-M4-execucao-e-orquestracao.md) · [Anterior: M3](M3-contratos-e-comunicacao.md) · [Próximo: M5](M5-escala-e-confiabilidade.md)
