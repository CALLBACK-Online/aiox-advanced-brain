...
```mermaid
flowchart TD
  R[Research] --> A[Advisory Board]
  CA[Code Anatomist] --> DD[Domain Decoder]
  DD --> SOP[AIOX SOP]
  SOP --> CU[ClickUp Ops]
  ETL[ETL Ops] --> DATA[Data]
  DATA --> DB[DB Sage]
  BRAND[Brand] --> DS[Design System]
  DS --> DO[Design Ops]
  H[Hormozi] --> COPY[Copy]
  COPY --> SALES[Sales]
  STORY[Storytelling] --> SLIDES[Slides Creator]
  SC[Squad Creator] --> SCP[Squad Creator Pro]
```
