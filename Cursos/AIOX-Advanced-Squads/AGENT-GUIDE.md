...
2. Consultar `agent-router.json`; não selecionar por memória ou apenas pelo nome do squad.
3. Comparar sinais e anti-sinais das rotas mais próximas.
4. Abrir a aula indicada e confirmar `Quando usar — e quando não usar`.
5. Se a ambiguidade mudar a entrega, fazer uma pergunta curta. Caso contrário, declarar a hipótese e avançar.
6. Verificar no projeto se existem o squad, a skill e o agente de entrada.
7. Pedir apenas os campos ausentes do briefing.
8. Informar maturidade e dependências antes de propor execução.
9. Usar somente sintaxe confirmada pelo runtime; caso contrário, usar o prompt genérico.
10. Exigir briefing, decision-log, deliverable e validation como evidência final.
...
Esses casos e a cobertura 1:1 estão em `tests/routing-cases.json` e são verificados por `npm run validate` (validador estrutural + `validate-agent-routing.mjs`).
